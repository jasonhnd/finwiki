...
- `.opinions/**` 被排除在站点 corpus / sitemap / API / AI discovery 之外，但在**本公开 GitHub 仓库中仍可直接阅读**。它是意见隔离区，由 `.opinions/QUARANTINE.md` 与 `bun run opinions:audit` 强制执行。禁止把“未部署”写成“不公开”。
