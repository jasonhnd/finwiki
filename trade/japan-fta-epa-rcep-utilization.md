---
title: "FTA / EPA / RCEP utilization — preferential trade agreements and how Japanese firms actually claim them"
aliases:
  - "trade/japan-fta-epa-rcep-utilization"
  - "japan-fta-epa-rcep-utilization"
  - "FTA EPA RCEP"
  - "経済連携協定"
  - "けいざいれんけいきょうてい"
  - "preferential trade agreement utilization"
  - "EPA utilization rate"
...
status: active
sources:
  - "https://www.mofa.go.jp/policy/economy/fta/"
...
This entry sits under [[trade/INDEX|trade INDEX]] and explains the **preferential-tariff** regime that determines whether a trader pays the MFN duty or a lower (often zero) rate. Its direct same-domain peer is [[trade/japan-customs-tariff-origin-clearance|customs, tariff and certificate of origin]] — that entry covers the *general* tariff machinery; this one covers the **preferential** layer that origin rules unlock. The promotion body that publishes EPA utilization guides is mapped in [[trade/jetro-vs-nexi-vs-jbic-comparison|JETRO vs NEXI vs JBIC]]. For broader policy context, cross into [[policy-finance/INDEX|policy-finance INDEX]].
...
| Term | Scope | Japanese usage |
|---|---|---|
| **MFN (most-favoured-nation)** | WTO baseline — the non-preferential rate every member gets | The fallback if no preference is claimed |
...
| **Specific process rule** | A named manufacturing operation (e.g., chemical reaction, weaving) occurs in-country |
...
This is why **JETRO** (and METI) run EPA-utilization support — origin-rule databases, utilization guides, and consultation — to convert paper preferences into realised duty savings. JETRO's role as the information channel for this is part of why it anchors the [[trade/INDEX|trade]] domain and is profiled in [[trade/jetro-vs-nexi-vs-jbic-comparison|JETRO vs NEXI vs JBIC]].
...
Treat this as the **shape** of Japan's preferential network from public Ministry of Foreign Affairs / METI disclosures — the precise membership and staging schedules evolve and should be checked against [[trade/INDEX|the domain's primary sources]] (MOFA, METI, JETRO, 税関) before relying on any specific corridor.
