...
### 推論 (from 多角化モデル) — 海外比率 ^[inferred]

- **海外比率拡大**: 米州・アジア豪州合算で営業利益の 30〜40% 台（年度変動）。ORIX の開示はセグメント＝事業区分（法人金融・事業投資・投資 等）ベースであり、地域別「営業利益の海外シェア」を直接示す一次開示は確認できず、年度・開示粒度により変動する推計値。検証可能な単一出典がないため本文から分離。
