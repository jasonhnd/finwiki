---
title: "[opinion] 明治安田生命保険相互会社"
source_entry: JapanFG/meiji-yasuda.md
type: opinion-quarantine
moved: 2026-05-29
note: 客観 wiki 本文から分離した主観的内容（推論 / Counterpoints / Open questions）。事実ではないため公開面・監査・計数から除外。将来の分析板块用に保留。
---

# 主観内容の隔離 — 明治安田生命保険相互会社

> 出典エントリー: `JapanFG/meiji-yasuda.md`

## 推論 (from 6. Counterpoints)

- 「相互会社の縛り」観: 上場できず M&A・資本調達自由度は株式会社系（[[life-insurers/dai-ichi-life]] 等）に劣る ^[inferred]
- 「StanCorp 集中リスク」: 米国 1 軸の海外戦略 → 為替・米景気・州規制連動 ^[inferred]
- 「営業職員チャネル老朽化」: 対面型主力 vs ネット完結型新興（ライフネット・楽天生命等）の台頭 ^[inferred]

<!-- ambiguous 整理 (2026-05-29) -->

## Counterpoints (from 6. Counterpoints)

- 「B2C ブランド露出 ROI」: Jリーグ・巨人スポンサーは認知に効くが、新契約獲得への直接寄与は計測困難 ^[ambiguous]
  - 出典本文より隔離。スポンサーシップの新契約獲得への寄与度は計測困難という評価的主張であり、検証可能な出典を持たないため隔離。
