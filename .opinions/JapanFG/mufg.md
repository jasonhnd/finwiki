---
title: "[opinion] 三菱 UFJ FG (MUFG)"
source_entry: JapanFG/mufg.md
type: opinion-quarantine
moved: 2026-05-29
note: 客観 wiki 本文から分離した主観的内容（推論 / Counterpoints / Open questions）。事実ではないため公開面・監査・計数から除外。将来の分析板块用に保留。
---

# 主観内容の隔離 — 三菱 UFJ FG (MUFG)

> 出典エントリー: `JapanFG/mufg.md`

## 推論 (from 4. 規制・政策)

- **G-SIB Bucket 1**: FSB 認定（+1.0% CET1 追加資本要件）^[inferred]

## Counterpoints

- 「海外集中＝個人デジタル弱」観: メガ 3 行で個人デジタル統合が最弱（vs [[megabanks/smfg]] Olive・楽天・PayPay）。GO 戦略は法人寄り ^[inferred]
- 「Krungsri 集中リスク」: タイ向け資産集中 → タイ景気 / バーツ / 政治リスク連動 ^[inferred]
- 「Union Bank 売却の戦略意義」: 米 retail 撤退 vs 「IB に集中」両論。U.S. Bancorp 株 4.39% で関係維持 ^[ambiguous]
- 「Global Open Network Japan 撤退」: Akamai 提携ブロックチェーン決済は 3 年弱で廃業 → ブロックチェーン挫折歴 ^[extracted]
- 「MUFG スタジアム 命名権」: B2C ブランド露出強化 vs スポーツ事業 ROI 疑問 ^[ambiguous]

## Open questions

- Progmat の他 FG（[[megabanks/smfg]] / [[megabanks/mizuho-fg]] / 通信系 FG）採用範囲は？ 単独主導 vs 共同インフラ化?
- アコム持分法（39.6%）→ 完全子会社化シナリオは？
- 6 代半沢社長（2026-04 就任）の戦略方向性は？ Google AI 提携の戦略的位置付け
- 「MUFG スタジアム」命名権（2026-01〜2030-01）の B2C ブランド効果は？
- 三菱 UFJ eスマート証券（旧カブドットコム）の戦略統合は？ MUMSS との重複/分業整理

<!-- ambiguous 整理 (2026-05-29) -->

## 一次確認できず隔離した計数（FY2026 決算表より）

- **FY2026 (2026-03 期) 連結 経常収益** — 精確値を一次出典で確認できなかったため隔離。2026-05-29 再検証の経緯:
  - MUFG 公式決算短信 PDF（`summary2603_ja.pdf` / `summary2603_en.pdf`）は和英いずれもバイナリ/圧縮データで本文抽出に失敗（本 entry 旧版でも「PDF バイナリ抽出失敗」と注記済み）。
  - kabutan・minkabu・官報決算 DB・kabuyoho の各 HTML 集計ページは HTTP 403、Nikkei 適時開示は有料会員限定。
  - 検索サマリ等の二次値は **14,620,843 百万円**（≒14.62 兆）と **15,564,500 百万円**（≒15.56 兆、年度帰属も曖昧）で齟齬。
  - 確実なのは FY2026 3Q 累計（9 か月）経常収益 約 **10 兆 6,438 億円**（YoY +3.6%）と、経常利益 3 兆 4,101 億円・純利益 2 兆 4,272 億円（公式・複数経路一致、本文に保持）のみ。
  - 通期経常収益の確定値は MUFG 決算短信本文（テキスト版）または有価証券報告書を直接参照して要確認。捏造を避けるため本文表からは値を除外。^[ambiguous]
