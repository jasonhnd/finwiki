---
title: "[opinion] 大垣共立銀行 (OKB)"
source_entry: JapanFG/ogaki-kyoritsu-bank.md
type: opinion-quarantine
moved: 2026-05-29
note: 客観 wiki 本文から分離した主観的内容（推論 / Counterpoints / Open questions）。事実ではないため公開面・監査・計数から除外。将来の分析板块用に保留。
---

# 主観内容の隔離 — 大垣共立銀行 (OKB)

> 出典エントリー: `JapanFG/ogaki-kyoritsu-bank.md`

## 推論 (from 1. 会社概要)

**機関設計**：監査役会設置会社 ^[inferred]

## 推論 (from 主要数値 (概数) ^[ambiguous])

> 注: 具体的な決算数値は OKB 公式 IR・有価証券報告書を直接参照のこと。本ノートは時点保証しない。^[inferred]

## 推論 (from 2. 事業セグメント・マップ)

| 公金 | 岐阜県・大垣市・西濃地方市町村 指定金融機関 | ^[inferred] |

## 推論 (from 観光融資)

- 養老の滝・関ヶ原古戦場・大垣城下町・郡上八幡など岐阜観光地への融資 ^[inferred]

## 推論 (from 中期経営計画 (2024〜))

- 地域経済振興 (中京サプライチェーン・観光) ^[inferred]

## 推論 (from 4. 規制・政策)

- **指定金融機関**: 岐阜県・大垣市等 (西濃地方市町村中心) ^[inferred]

## 推論 (from 頭取・経営)

- 創業家・地元有力者との関係維持 ^[inferred]

## Counterpoints

- 「楽しい銀行 ≠ 収益力」観: B2C ブランド露出は強いが、地銀中位の規模で持株会社化していない単独銀行体制が長期的に競争力を保てるかは要観察 ^[ambiguous]
- 「西濃集中リスク」: 岐阜県西濃地方への営業集中 → 中京自動車部品サプライチェーンの構造転換リスク (EV シフト) ^[inferred]
- 「[[regional-banks/juroku-fg]] との力学」: 岐阜県内で十六 FG (FG 化済・PRIME) と並ぶ二大銀行だが、預金量・総資産では十六に劣勢との見方もある ^[ambiguous]
- 「持株会社化見送り」: 地銀再編の流れの中で OKB が単独銀行を貫く戦略意義 — 地域密着 / 経営自由度 vs スケールメリット欠如 ^[ambiguous]
- 「ドライブスルー支店」: 注目度高い vs キャッシュレス化で取扱量は減少傾向 ^[inferred]

## Open questions

- OKB の持株会社化シナリオは? 単独銀行維持 vs [[regional-banks/juroku-fg]] や他地銀との経営統合の可能性?
- 中京自動車産業 EV シフトに伴う西濃製造業融資ポートフォリオの構造変化は?
- 「日本一の女性活躍銀行」KPI の達成度と外部評価は?
- 観光融資 (養老・関ヶ原・大垣) のインバウンド回復インパクトは?
- OKB レンガ通り等地域コミュニティ投資の中長期 ROI は?
- 移動店舗車・ドライブスルー支店の今後の拡張 / 縮小方向は?

<!-- ambiguous 整理 (2026-05-29) -->

## ATM 網の相対評価 (from 2. 事業セグメント・マップ)

- ATM 数が「全国地銀でも上位水準」とする評価 ^[ambiguous]：検証可能な順位ソースなし。本文は事実ベース（365 日年中無休稼動・他行 ATM 提携網、ドライブスルー・コンビニ展開）に置換。コンビニ・ドライブスルー含む台数の全国順位は OKB 公式開示に確たる数字がないため隔離。

## 女性活躍の相対評価 (from 「日本一の女性活躍銀行」)

- 「地銀業界の中でも、女性活躍指標で目立つ取組み」^[ambiguous]：主観評価。OKB 自身の「日本一の女性活躍銀行」標榜は事実として本文に残置するが、業界横断比較での「目立つ」という第三者評価は検証可能ソースなしのため隔離。
