...
## スポンサー付き / 非スポンサー型 DeFi コンポーザビリティ微妙だが戦略上重要なパターンとして、**Ondo の DeFi コンポーザビリティはスポンサー仲介型**であり、パーミッションレスではない。OUSG トークン契約の移転権限に KYC 制約が組み込まれているため、**Ondo の明示的な支援**なしに Aave、Morpho、Compound、Spark へ担保として追加することはできない。これは USDC のパーミッションレスな DeFi コンポーザビリティとは**逆**で、BUIDL のモデルに近い。
...
- [[fintech/blackrock-buidl-sc-issuer-adoption|BUIDL stablecoin-発行会社 adoption matrix]]
...
- [[fintech/issuer-distributor-incentive-realignment-arc-strategy|発行会社-distributor realignment · Arc strategy]]
