...
> [!info] 要約
> クロスボーダー SC 普及最大の阻害は「銀行業務フロー / AML/CFT との非互換性」。これを解く現代パターンは **SWIFT API を 前面 に置き、ブロックチェーン決済 を 後面 で実行する ハイブリッド構造**。Project Pax(Progmat + Datachain · 2024-09)と BIS Project Agorá がこの構造の代表実装。**TD(Tokenized Deposit) は SWIFT API 互換性で先行**、**SC は §501(d) 相互運用ライセンス次第で接続パターンが分岐**。
...
| 理由 | 内容 |
|---|---|
| **既存業務フロー 保護** | 銀行内部システム / corporate ERP は SWIFT 前提で 50 年運用 |
...
| 互操作パートナー | 米系 banking パートナー 既存 | アジアパートナー 確定不足 |
| 利息 | 預金利息銀行帰属 | 信託受託報酬 |
...
| Layer | 構成要素 | 提供者 |
|---|---|---|
| Application | SWIFT API mock + corporate ウォレット | Datachain |
| 決済 instruction | Progmat Coin contract | Progmat + Datachain |
