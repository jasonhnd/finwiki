...
| 設計選択 | 詳細 | 含意 |
|---|---|---|
| **二層仲介型** | CBN → Deposit Money Banks（DMBs）+ Mobile Money Operators（MMOs）→ 消費者ウォレット | [[fintech/cbdc-multi-tier-architecture-three-paradigms|three paradigms]] の二層モデルと同じ仲介型。 |
...
| **Speed Wallet（消費者）+ Speed Merchant（加盟店）** | 二つのアプリを使う構成。 | 加盟店側に追加のオンボーディング手順が生じる。 |
...
| 流通モデル | DMBs + MMOs（二層型） | Authorized Financial Institutions（AFIs） | Authorized wallet providers（Lynk ほか） | 10  の指定運営機関（六つの国有銀行 + 四つの決済大手） |
