...
本エントリは、日本・韓国・香港・シンガポール・EU・米国・UAE・英国という 8 つの法域を、現行規制の確認入口として並べる。制度が単一モデルへ収斂したとは扱わず、法源、license class、資産分別、AML / Travel Rule、stablecoin、cross-border、marketing、enforcement、開示、例外を法域ごとの一次資料で確認する。各極の詳細は [[exchanges/jp-vasp-regulatory-timeline]] / [[exchanges/eu-mica-casp-regime-overview]] / [[exchanges/us-crypto-licensing-multi-layer-system]] / [[exchanges/hk-sfc-vasp-licensing-overview]] / [[exchanges/sg-mas-dpt-licensing-overview]] を起点に。
...
- **比較対象**: 8 法域は網羅的な世界ランキングではなく、主要な制度設計を横断確認するための選定
- **比較方法**: cold-storage 比率、資本、閾値、施行日を固定ランキングにせず、法人・活動・例外・確認日をそろえて各当局の現行 rule を読む

### A. 法源・規制当局・施行時期

出典: 各当局の現行規制入口（[日本](https://www.fsa.go.jp/en/policy/virtual_currency/)、[韓国](https://www.fsc.go.kr/eng/)、[香港](https://www.sfc.hk/en/Rules-and-standards/Suitability-requirements/Virtual-asset-related-products-and-services)、[シンガポール](https://www.mas.gov.sg/regulation/payments)、[EU](https://www.esma.europa.eu/policy-activities/crypto-assets)、[米国 NYDFS](https://www.dfs.ny.gov/virtual_currency_businesses)、[UAE](https://www.vara.ae/)、[英国](https://www.fca.org.uk/firms/cryptoassets)）。

| 極 | 主な公開監督入口 | 本表の確認範囲 |
|---|---|---|
| **日本** | FSA / JVCEA | 暗号資産交換業登録と自主規制 |
| **韓国** | FSC / FIU | 仮想資産事業者の監督 |
| **香港** | SFC | VATP 制度 |
| **シンガポール** | MAS | Payment Services / DPT 制度 |
| **EU** | ESMA / 各国 NCA | MiCA / CASP 制度 |
| **米国** | 連邦当局 / 州当局 / NYDFS | 連邦・州の複層制度 |
| **UAE** | VARA / ADGM-FSRA 等 | 首長国・金融自由区別制度 |
| **英国** | FCA | cryptoasset 登録・規制 |

### B. ライセンス区分・階層

出典: ライセンス名称と活動範囲は A 節の各当局現行ページで確認する。

| 極 | 制度の読み方 | 更新時の確認点 |
|---|---|---|
| **日本** | 暗号資産交換業と金融商品取引の法的区分を分ける | FSA 登録一覧と業務範囲 |
| **韓国** | FIU 登録と関連する情報セキュリティ要件を分ける | 現行法・監督資料 |
| **香港** | SFC の VATP 要件を参照する | 認可条件と顧客範囲 |
| **シンガポール** | Payment Services Act 上のサービス区分を参照する | licence class と DPT 条件 |
| **EU** | MiCA の CASP サービス認可を参照する | NCA 認可と passporting |
| **米国** | 連邦・州・活動別ライセンスを別々に確認する | 州差と連邦業法 |
| **UAE** | VARA、ADGM、DIFC 等の管轄を分ける | 活動別許可と所在地 |
| **英国** | FCA の現行登録・認可範囲を参照する | 施行済み要件と提案を分離 |

### C. 活動カバレッジ (spot / derivatives / カストディ / staking / lending / NFT marketplace)

出典: 活動可否は一律の記号ではなく、商品・顧客・法人・管轄ごとに A 節の各当局で確認する。

| 活動 | 比較時に分ける論点 | 一次資料で確認する事項 |
|---|---|---|
| **Spot** | 仲介・販売・取引所運営 | 認可サービスと顧客範囲 |
| **Derivatives** | VASP 規制と証券・商品規制 | 別ライセンスの要否 |
| **Custody** | 自己保管・第三者保管・信託 | 資産分別と受託資格 |
| **Staking / lending** | 交換業付随サービスと金融商品性 | 商品別条件と開示 |
| **NFT** | 交換可能性・決済性・投資性 | トークンの機能別判断 |

### D. 資本要件・分別管理

出典: 資本・分別管理・保管要件は法改正や licence class により変わるため、A 節の各当局の現行規則を参照する。

| 要件軸 | 比較方法 | 固定値を置かない理由 |
|---|---|---|
| **資本** | 法人・活動・license class ごとに確認 | 区分と通貨が異なり改定される |
| **顧客金銭** | 信託、分別口座、銀行保全を区別 | 法域ごとに法的構造が異なる |
| **暗号資産保管** | cold / hot、自己保管、第三者保管を区別 | 比率・例外・補償条件が異なる |
| **保険・保証** | 法定要件と任意契約を分ける | 保険金額・免責が契約ごとに異なる |

### E. Fit-and-proper / 出資金来源

fit-and-proper、management、qualifying holding、資金源の要件は法域・license class・申請主体で異なる。比較時は A 節の各当局ページから現行 application / authorization rule に進み、対象者、審査項目、届出時点、継続義務を個別に確認する。

### F. AML / Travel Rule 実装

出典: AML / Travel Rule は A 節の各当局の現行規則と FATF 実装状況で確認する。

| 確認軸 | 一次資料で確認する内容 | 注意点 |
|---|---|---|
| **適用対象** | VASP / CASP / 金融機関の定義 | 法域間で対象が一致しない |
| **情報項目** | originator / beneficiary 情報 | 送付先種別で要件が異なる |
| **閾値・例外** | 現行法令・ガイダンス | 通貨換算や例外が改定される |
| **伝達方式** | 法的要件と業界プロトコル | 特定ベンダー利用を法定要件と混同しない |

### G. ステーブルコイン発行ルール (cross-link [[fintech/global-stablecoin-regulatory-five-pole-matrix]])

stablecoin の発行・取扱規制は VASP 制度と別の銀行、決済、e-money、信託、証券 rule にまたがる。施行済み rule と proposal を分け、発行主体、準備資産、償還、販売・仲介の要件を各当局の dated primary source で確認する。法域別の詳細は [[fintech/global-stablecoin-regulatory-five-pole-matrix]] を参照する。

### H. クロスボーダー solicitation / passporting

cross-border solicitation と passporting は、顧客所在地、active solicitation / reverse solicitation、商品、提供法人、license scope で結論が変わる。単一の国数や「外資排除」といった固定評価を置かず、対象時点の当局 guidance と register を確認する。日本の警告確認手順は [[exchanges/fsa-foreign-exchange-warning-system]] を参照する。
...
- **UAE**: proof-of-reserves / reporting の現行頻度と対象は VARA Rulebook で確認する
- **英国**: MLR 下の現行 disclosure と FSMA 拡張の施行状況は FCA の日時付き公式資料で確認する
...
拠点選択や market access を評価するときは、法人ごとの authorization、顧客範囲、passporting / solicitation、資本・custody、税務を同一時点で比較する。本項目は、特定都市の役割分担、license cost、企業の本社移転理由、将来の発行 hub を一次資料なしに確定しない。
