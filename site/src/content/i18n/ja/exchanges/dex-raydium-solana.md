...
status: machine
fidelity: ok
title: "Raydium — Solana 系老舗 AMM / memecoin 流動性ハブ概要"
translated_at: 2026-06-02T12:19:04.238Z
---
﻿
# Raydium — Solana 系老舗 AMM / memecoin 流動性ハブ概要


## ウィキ上の位置づけ

本項目は [[exchanges/INDEX|exchanges index]] の配下に位置する。Solana 内のピア文脈は [[exchanges/solana-ecosystem-dex-comparison|Solana エコシステム DEX 群比較]]、EVM との対比は [[exchanges/global-dex-major-five-comparison|global DEX major five comparison]]、より広いシステム・設計境界は [[exchanges/amm-design-evolution|AMM 設計進化系譜]] とあわせて読む。

> 2021-02 ローンチ · RAY トークン · 定数積 (x*y=k) AMM · 旧 Serum integration legacy · 現在は memecoin 流動性ハブ

## 1. プロトコル概要

- **チェーン**：Solana L1（SVM 環境）
- **モデル**：定数積 AMM（x*y=k）+ 集中流動性（CLMM）+ 旧 Serum/OpenBook orderbook ハイブリッド（legacy）
- **ローンチ**：2021-02-21
...
- **現在**：標準 AMM + CLMM + OpenBook hook の 3 層併存
...
|---|---|---|---|
| 設計 | 定数積 + CLMM + orderbook hook | 集中流動性（Whirlpools） | DLMM（Dynamic Liquidity） |
| 主用途 | memecoin + 一般 spot | リテール向けスワップ | memecoin + 高度 LP |
| token | RAY | ORCA | MET（未発行 / 公開情報なし） |
| TVL 規模 | $1.5–2.5B | $300–500M | $400–800M |
| 強み | memecoin migration、aggregator 経由量 | UX、安定 LP | LP 効率、動的手数料 |
...
- **RAY**：総供給 555M、staking で 手数料分配 受給
- **fee 構造**：標準 0.25%（うち 0.22% LP、0.03% RAY staker への buy-back）
- **CLMM プール**：手数料 tier 選択可（0.01% / 0.05% / 0.25% / 1%）
