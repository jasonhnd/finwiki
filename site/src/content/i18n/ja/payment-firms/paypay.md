---
source: payment-firms/paypay
source_hash: 2f822b411f4370be
lang: ja
status: machine
fidelity: ok
title: "PayPay株式会社"
translated_at: 2026-06-20T09:33:00.420Z
---

# PayPay株式会社


## ウィキ上の位置づけ

この項目は [[payment-firms/INDEX|payment-firms INDEX]] の配下に位置する。ピア比較・対比の文脈では [[payment-firms/paypal-pte-ltd-japan|PayPal Pte. Ltd. Japan service boundary]]、より広い制度・規制上の境界では [[payments/INDEX|payments index]] とあわせて読む。

## 要約

PayPay株式会社は [[megabanks/paypay-fg|PayPay金融グループ]] の consumer wallet / code-payment operator。公式会社概要はモバイルペイメント等の電子決済サービスを主業とし、前払式支払手段、クレジットカード番号等取扱契約締結事業者、資金移動業、電子決済等代行業、賃金デジタル払い指定資金移動業者などの登録を列挙している。

## 1. 法人・ライセンス境界

| 項目 | 読み方 |
|---|---|
| 法人名 | PayPay株式会社 |
| グループ | [[megabanks/paypay-fg|PayPay FG]] / SoftBank-LY ecosystem |
| 主要レーン | コード決済、前払式支払手段、資金移動、加盟店受入、賃金デジタル払い隣接領域 |
| 金融庁レジストリ上のルート | 金融庁 資金移動業者一覧：関東財務局長 第00068号、PayPay株式会社。 |

## 2. 事業上の役割

- 消費者向けアプリは、PayPay 残高、コード決済、個人間送金、ポイント施策、PayPay Credit / PayPay Card への導線、隣接する PayPay 金融サービスの発見のための流通接点である。
- 加盟店側もウォレット側と同じく重要である。受入、決済、販促、アプリ経由の顧客接点があるため、PayPay は単なる QR ブランドではなく、決済主導の流通レイヤーとして読める。
- 公式会社概要は PayPay Card、PayPay Bank、PayPay Securities、PayPay SC、PayPay India、Credit Engine を関連会社として列挙しているため、SoftBank / LY / PayPay エコシステム内のウォレット起点のグループ金融として読むべきである。
- 日本の大規模コード決済プラットフォームを比較し、ウォレットの経済性とカード発行会社の経済性を切り分けるためのアンカーページである。

## 3. 規制・グループ上の位置づけ

| レイヤー | 公開情報からの読み方 |
|---|---|
| 資金移動 | 公式会社概要と金融庁一覧は、PayPay を関東財務局第 00068 号の資金移動業者として示している。 |
| 前払 / 残高 | 会社概要は第三者型前払式支払手段発行者登録も別に示すため、ウォレット残高の分析で前払と資金移動を混同してはいけない。 |
| 加盟店 / カード番号管理 | PayPay の公式登録にはクレジットカード番号等取扱契約締結事業者が含まれるため、加盟店受入境界の一部は純粋なウォレット領域の外にある。 |
| 代理 / 仲介 | 銀行代理業、金融商品仲介業、電子決済等代行業の登録は、提携・仲介チャネルを示すものであり、PayPay 自体が銀行や証券会社であることを意味しない。 |
| 賃金デジタル払い | 公式会社概要は賃金デジタル払い指定も示している。これは特定の給与隣接レーンであり、全てのウォレット残高へ一般化すべきではない。 |

## 4. この単独ページが重要な理由

[[megabanks/paypay-fg|PayPay FG]] は持株会社 / グループ境界を扱う一方、PayPay株式会社はウォレットとコード決済の事業会社である。両者を混同すると、ライセンス、加盟店、消費者保護の各レイヤーが見えにくくなる。

## 関連

- [[megabanks/paypay-fg]]
- [[card-issuers/paypay-card]]
- [[securities-firms/paypay-securities]]
- [[payments/cashless-jp-landscape]]
- [[payments/funds-transfer-vs-prepaid-boundary]]

## 出典

- PayPay 会社概要: https://about.paypay.ne.jp/about/
- 金融庁 資金移動業者一覧: https://www.fsa.go.jp/menkyo/menkyoj/shikin_idou.pdf
