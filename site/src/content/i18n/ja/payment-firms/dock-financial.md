...
この項目は [[payment-firms/INDEX|payment-firms INDEX]] の下に置き、ソフトバンク株式会社（東証 PRIME 9434、コア通信事業）グループの **決済代行・金融サービス子会社 SB ペイメントサービス（SBPS）** の **事業会社アンカー** として扱う。[[megabanks/paypay-fg|PayPay 金融グループ]] は SoftBank 系 payment / fintech クラスター内の兄弟会社、[[megabanks/au-fh|au フィナンシャル HD]] は通信会社内包型金融グループの比較対象、[[megabanks/ndfg|NTT ドコモ FG]] は通信系金融グループのピア、[[payments/INDEX|payments index]] は PSP / 加盟店決済サービス文脈、[[payments/cashless-jp-landscape|キャッシュレス日本ランドスケープ]] は制度上の位置付けとして併読する。
...
- [[payment-firms/INDEX]] — JapanFG 名前空間インデックス
