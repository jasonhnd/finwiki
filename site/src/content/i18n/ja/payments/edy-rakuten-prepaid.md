...
| Original operator | bitWallet（ビットワレット株式会社）── NTT ドコモ + ソニー + 銀行コンソーシアム |
...
Rakuten Edy は、受容が楽天自身のサービスを超えた第三者加盟店で行われるため、[[payments/funds-transfer-vs-prepaid-boundary|Payment Services Act (資金決済法)]] 第 3 章のもとで **第三者型前払式支払手段** として登録されている。主要な規制上の帰結：
...
| Acceptance merchant points | 全国規模で相当数（Rakuten Edy サイトが開示）|
| Outstanding prepaid balance | 数十億円規模（楽天グループ開示）|
...
2. **楽天エコシステム連携** ── 楽天カードのオートチャージ、楽天ポイントのロイヤルティ連携、楽天エコシステムのクロスプロモーション。
3. **レガシーのインストールベース** ── WAON / nanaco 以前のカードベースが残存的な顧客フットプリントを提供。
4. **コード決済の補完であって代替ではない** ── 楽天ペイのコード決済は異なるユースケースに対応；Edy は楽天エコシステム向けの FeliCa タップレールであり続ける。
5. **縮小したが持続的** ── Edy の市場シェアは WAON / nanaco / Suica のピアに比して浸食されたが、FeliCa タップレールと楽天エコシステム連携が有意な取引量を維持している。
