---
source: payments/apple-gift-card-japan
source_hash: 8e674621b040f932
lang: ja
status: machine
fidelity: ok
title: "Apple Gift Card Japan: Apple エコシステムのクローズドループ前払"
translated_at: 2026-06-02T16:33:58.490Z
---
# Apple Gift Card Japan: Apple エコシステムのクローズドループ前払

## ウィキ上の位置づけ

この項目は [[payments/INDEX|payments index]] の下に位置し、[[payments/prepaid-payment-instrument-issuers-japan-index|prepaid payment instrument issuers registry]] の FSA 登録ビューと対になる、日本における Apple Gift Card のクローズドループ前払発行者ページである。[[payments/amazon-gift-card-japan|Amazon Gift Card Japan]]、[[payments/google-play-gift-card-japan|Google Play Gift Card Japan]]（同業のプラットフォーム起点クローズドループ・ギフトカード）、[[payments/quo-card-prepaid|QUO Card]]（複数加盟店型オープンループ・ギフト）、[[payments/visa-gift-card-japan|Visa Gift Card]]（ブランドネットワーク型プリペイド）と比較する。Apple Gift Card は、従来の **iTunes Card** と **Apple Store Gift Card** を 2021, に置き換え、Apple エコシステム（App Store、iTunes、iCloud、Apple Music、Apple TV+、Apple ハードウェア、Apple Store）全体で使える単一の統合ギフトカードに統合した。法的枠組みは [[payments/funds-transfer-vs-prepaid-boundary|funds-transfer vs prepaid boundary]]。

## 要約

**Apple Gift Card（Appleギフトカード）** は、**Apple Inc. のクローズドループ型統合前払ギフトカード**であり、日本では **Apple Japan G.K.（Apple Japan合同会社）** が発行する。[[payments/funds-transfer-vs-prepaid-boundary|Payment Services Act prepaid framework]] のもとで、自家型（クローズドループ / 単一加盟店）前払式支払手段として運営される。統合形態のプロダクトは **2021-09に開始**され、それまでの **iTunes Card**（App Store / iTunes / Apple Music / iCloud 向け支出）と **Apple Store Gift Card**（Apple ハードウェア / Apple Store 小売向け支出）を単一 SKU に統合した。Apple Gift Card 残高は、日本の **Apple エコシステム全体**、すなわち App Store、iTunes Store、Apple Music、Apple TV+、iCloud、Apple Books、Apple Arcade、**および** Apple Store（apple.com/jp/shop）でのハードウェア（iPhone、iPad、Mac、AirPods、アクセサリ）購入に利用できる。流通レールには、コンビニエンスストア（Lawson、FamilyMart、MiniStop、7-Eleven、Daily Yamazaki、ドラッグストア、家電量販店等）での物理カード販売、Apple.com/jp のデジタル eGift、Apple Store 店頭が含まれる。

## 発行者と運用構造

| 観点 | 読み方 |
|---|---|
| 運営会社 | Apple Japan G.K.（アップル・ジャパン合同会社） |
| グループ親会社 | Apple Inc.（US） |
| FSA 前払登録 | 自家型前払式支払手段（クローズドループ）— 単一加盟店の Apple エコシステム |
| 法的分類 | クローズドループ前払（第三者型ではない）— `daisan.xlsx` 第三者型一覧には掲載されない |
| 統合版開始年 | 2021-09 （iTunes Card + Apple Store Gift Card の統合） |
| 前身商品 | iTunes Card（デジタルコンテンツ）、Apple Store Gift Card（ハードウェア小売）— 統合時に終了 |
| 額面 | 固定（¥1,500, ¥3,000, ¥5,000, ¥10,000, ¥25,000）および可変（デジタル eGift で ¥1,500 から ¥50,000 まで選択可能） |
| 流通レール | コンビニ、家電量販店、Apple Store、Apple.com/jp eGift |
| 利用可能先 | Apple エコシステムのみ（App Store、iTunes、Apple Music、Apple TV+、Apple Books、Apple Arcade、iCloud、Apple Store ハードウェア / 小売） |
| 有効期限 | Apple ID にチャージされた残高には有効期限なし |

## 法的根拠: 自家型前払式支払手段（クローズドループ）

Apple Gift Card は、[[payments/funds-transfer-vs-prepaid-boundary|Payment Services Act (資金決済法)]] 第 3章に基づく **自家型前払式支払手段** として登録される。クローズドループ分類は、利用可能範囲が Apple 自身のサービスおよび小売に限定されることを反映している。主な規制上の帰結は次のとおり。

- **FSA 登録が異なる** — クローズドループ発行者は [[payments/prepaid-payment-instrument-issuers-japan-index|`daisan.xlsx`]]（第三者型のみ）には掲載されない。
- **半期ごとの未使用残高供託義務** は、資金決済法上のクローズドループ前払枠組みでも適用される。
- **発行終了時を除き、元の支払者への現金払い戻しは不可** — Apple Gift Card 残高は現金化できない。
- **残高保持期間が長い** — Apple ID にチャージされた後の残高には有効期限がないため、アカウントへチャージ済みの残高レベルでの失効益は小さい。未チャージのカード単位の失効（紛失 / 未使用の物理カード）は別の論点である。

## 統合（2021-09）

2021 以前の商品構成:

| 2021 以前の商品 | 利用面 |
|---|---|
| **iTunes Card** | App Store、iTunes Store、Apple Music、Apple TV+、Apple Books、Apple Arcade、iCloud — デジタルコンテンツ / サービス |
| **Apple Store Gift Card** | Apple Store（apple.com/jp/shop）— ハードウェア（iPhone、iPad、Mac、AirPods） |
| **App Store & iTunes Gift Card（国際版）** | iTunes Card と類似 |

**2021-09 の統合**により、これらはデジタルコンテンツとハードウェア小売の両方で使える単一の **Apple Gift Card** SKU にまとめられた。これは消費者と流通チャネル双方にとって構造を単純化する変更だった。米国、日本、その他主要市場で同時期に展開された、Apple のグローバルな商品変更である。

## 流通レール

| レール | 説明 |
|---|---|
| **コンビニエンスストア** | Lawson、FamilyMart、MiniStop、7-Eleven、Daily Yamazaki、Seicomart — プラスチック包装の物理カード |
| **ドラッグストア** | マツモトキヨシ、ココカラファイン、スギ薬局、ウエルシア、ツルハ — 物理カード |
| **家電量販店** | Yamada Denki、Bic Camera、Yodobashi Camera、Edion、Ksdenki — 物理カード |
| **Apple Store 店頭** | Apple Store Japan（Marunouchi、Shibuya、Omotesando、Ginza 等）— 物理カード + デジタル eGift |
| **Apple.com/jp eGift** | メールによるオンラインデジタル配送、金額とメッセージのカスタマイズ |
| **販促 / キャンペーン配布** | Apple パートナーのキャンペーン報酬 |

**コンビニ + ドラッグストア + 家電量販店の流通** が主要な物理カードレーンであり、Apple 外チャネルでの Gift Card 販売の大部分を担う。

## ギフトカード詐欺 / 悪用懸念

[[payments/amazon-gift-card-japan|Amazon Gift Card]] と同様に、Apple Gift Card は日本の **振り込め詐欺 / 電子マネー型詐欺** エコシステムで多用されてきた。詐欺電話の加害者は、被害者にコンビニで Apple Gift Card を購入させ、コードを電話で読み上げさせる。Apple Japan の対応は次のとおり。

- Apple サポートサイトでの消費者向け警告ページと詐欺注意喚起。
- 高額 Apple Gift Card 購入時のコンビニ店員による注意喚起。
- この詐欺パターンを受け、警察庁や消費者保護当局は Amazon や Google Play と並んで Apple Gift Card を名指しする注意喚起を行っている。

## KPI スナップショット

| 指標 | 読み方 |
|---|---|
| Apple Gift Card（統合前の iTunes Card を含む）の累計発行 | フランチャイズ全体では数兆円規模の重要な水準（Apple は日本セグメント別の Gift Card 数値を公表していない） |
| コンビニ流通浸透 | 主要コンビニチェーンとドラッグストアが Apple Gift Card を取り扱う |
| Apple エコシステム依存 | Apple Gift Card は日本の App Store 購入における **主要なカードレス決済レール** であり、特に未成年者やクレジットカードを持たない利用者のアプリ・アプリ内課金購入で重要である |
| 可変額面 eGift | Apple.com/jp で利用可能。法人一括 B2B も利用可能 |

Apple は日本セグメント別の Gift Card 売上を財務開示で分解していない。数値は業界推計や間接的なチャネル流通指標に依存する。

## 戦略: Apple エコシステムへのキャッシュレス導線

Apple Gift Card Japan の戦略上の役割:

1. **App Store のカードレス導線** — 未成年者、クレジットカードを持たない利用者、ギフト受取者が、クレジットカード登録なしでアプリやアプリ内コンテンツを購入できる。
2. **ハードウェア小売ギフト配布** — 従来の Apple Store Gift Card を置き換えることで、消費者のギフト選択を単純化する。
3. **クローズドループのフロート / 失効益** — 他のクローズドループ型プラットフォーム・ギフトカードと同様、未使用残高のフロートと未チャージカードの失効益を得る。
4. **コンビニレールの活用** — 日本のコンビニ網に広く物理カードを流通させることで、購入アクセスを支える。
5. **統合 SKU の効率性** — 2021 の統合により、小売店の SKU 複雑性と店頭での消費者混乱を減らした。

## 関連項目

- [[payments/INDEX]]
- [[payments/prepaid-payment-instrument-issuers-japan-index]]
- [[payments/amazon-gift-card-japan]]
- [[payments/google-play-gift-card-japan]]
- [[payments/quo-card-prepaid]]
- [[payments/visa-gift-card-japan]]
- [[payments/waon-prepaid-aeon]]
- [[payments/nanaco-prepaid-seven-i]]
- [[payments/funds-transfer-vs-prepaid-boundary]]
- [[payments/japan-payment-scheme-economics-matrix]]
- [[INDEX|FinWiki index]]

## 出典

- Apple Gift Card Japan page (apple.com/jp/shop/buy-giftcard/giftcard).
- Apple support page on Apple Gift Card (support.apple.com/ja-jp/HT213007).
- Apple Japan 公式企業サイト。
- FSA prepaid payment instruments policy page (closed-loop / 自家型 framework).
- FSA `daisan.xlsx`（相互参照 - Apple Gift Card がクローズドループであり、第三者型一覧に掲載されないことを確認）。
- METI cashless policy page.
