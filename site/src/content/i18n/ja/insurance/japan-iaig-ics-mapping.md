...
translated_at: 2026-06-02T14:01:20.936Z
---
...
この項目は [[insurance/INDEX|insurance index]] 配下に置き、IAIS の Internationally Active Insurance Group 指定と、日本本社グループの Insurance Capital Standard 報告が交差する領域の経路ページである。制度比較は [[insurance/global-solvency-framework-comparison-matrix|global solvency framework matrix]]、日本のFSAフレームワークは [[insurance/economic-value-based-solvency|economic-value-based solvency]]、会社別比率の解釈は [[insurance/esr-economic-value-solvency|ESR]]、生命保険側エンティティは [[insurance/japan-life-insurance-big-four|Japan life big four]]、P&C側エンティティは [[insurance/japan-nonlife-big-three|Japan non-life big three]]、インバウンドの外国IAIG視点は [[insurance/foreign-life-affiliate-japan-positioning|foreign-life affiliates positioning]]、ICS報告が捉えるバランスシート要因は [[insurance/japan-life-insurance-alm-overview|Japan life ALM overview]] と併読する。

License-route context is in [[financial-licenses/insurance-license-and-solvency|insurance license and solvency route]].

## 要約

IAIS Insurance Capital Standard Version 2.0  は、2024  年12月に、5年間のモニタリング期間（2020-2024）を経て最終化され、2025  以降 Internationally Active Insurance Groups（IAIGs）の Prescribed Capital Requirement（PCR）となる。IAIG指定は、複数法域にまたがる保険料・資産などの国際活動と規模閾値を含むIAIS基準に基づき、当該グループのグループワイド監督当局が行う。

日本本社の保険グループについては、FSAがグループワイド監督当局として、IAIG指定、ICS報告の徴求、該当する場合の内部モデル承認、国際監督カレッジへの参加を担う。候補または指定IAIGとして通常議論される日本本社グループは、損保側では [[non-life-insurers/tokio-marine|Tokio Marine]]、[[non-life-insurers/msad|MS&AD]]、[[non-life-insurers/sompo|Sompo]]、生保側では [[life-insurers/nippon-life|Nippon Life]]、[[life-insurers/dai-ichi-life|Dai-ichi Life]]、[[life-insurers/sumitomo-life|Sumitomo Life]]、[[life-insurers/meiji-yasuda|Meiji Yasuda]] である。指定IAIGの公開リストはIAISおよび各グループワイド監督当局が維持する。

日本の枠組みでは、国内FSA [[insurance/economic-value-based-solvency|ESR]] 制度の上にグループレベルの上乗せとしてICSを運用する。両制度は概念的に近く調整されているが、同一ではない。
...
| Criterion | Threshold (indicative) | 内容 |
|---|---|---|
| 国際活動 | 3つ以上の法域で保険料を計上し、本国外法域の保険料比率が定義済み閾値を上回る | 連結グループレベルで活動を測定 |
...
正確な閾値はIAIS公開文書で定められ、IAISにより定期的に改定され得る。指定は当該グループのグループワイド監督当局が行い、日本本社グループについてはFSAがその役割を担う。
...
この損保3グループはいずれも、国際フットプリントに基づきIAIGとして広く認識されている。FSAはグループワイド監督当局として、持株会社レベルでICS報告を運用する。

### Life side

| Group | International footprint relevant to IAIG criteria |
|---|---|
| [[life-insurers/nippon-life|Nippon Life]] | 日本最大級の生命保険会社。複数法域にまたがる海外保険・資産運用投資および買収を持つ。フットプリントは意味のある規模だが、総バランスシートに対する比率では損保ピアより小さい。 |
...
各生命保険会社のIAIG指定は、グループワイド監督当局としてFSAが適用する具体的なIAIS基準閾値に依存する。公開IAIGリストはIAISとFSAが維持する。住友生命と明治安田の閾値判定は、年ごとの国際活動測定に左右される。

[[megabanks/sony-fg|Sony Financial Group]] / [[life-insurers/sony-life|Sony Life]] の範囲は [[insurance/sony-life-group-life-operating-model|Sony Life Lifeplanner / group-life operating model]] で議論するが、最終親会社が非保険会社の Sony Group Corporation であり、保険グループ範囲が単独の日本相互会社ピアより狭いため、構造的に異なる。IAIG範囲の分類は、FSAのグループ範囲定義に依存する。
...
日本では、FSAが指定された日本本社IAIGに対してICSをグループレベルPCRとして実装し、国内 [[insurance/economic-value-based-solvency|economic-value-based ESR]] 制度と並行して運用する。同制度は 2025-04 から日本免許エンティティレベルで稼働した。
...
FSAはIAISを介したクロスボーダー対話に参加し、EIOPA、米国州監督当局、FINMA、BaFin、OSFI、MAS、HKIAなど主要保険監督当局と連携する。監督カレッジ構造により、FSAは日本本社IAIGの海外子会社のホスト監督当局、例えば Tokio Marine の HCC と Philadelphia については米国、Lloyd's syndicates については英国PRA、Dai-ichi Life の TAL については豪州APRAなどと調整できる。
...
ICSとESRは概念的に近い。どちらも市場整合的 / 経済価値ベース評価を用い、長期負債には割引カーブ手法（ESRではUFR、ICSではLTFR）を適用し、質基準を伴う階層別適格資本を使い、市場・信用・保険・オペレーショナルリスクモジュールを較正する。ただし数値的に同一ではない。差異には、相関行列の較正、日本国内固有ショックシナリオの扱い、MOCEとリスクマージン計算、移行措置が含まれる。
...
日本本社IAIGについて、FSAはこれらの基準に照らして内部モデル要素を審査する。新規ICS計算のデフォルトは標準手法であり、内部モデル採用は段階的で、モデル成熟度の実証を必要とする。
...
[[insurance/japan-life-insurance-big-four|Japan life big four]] と [[insurance/japan-nonlife-big-three|Japan non-life big three]] のIAIGでは、これらの要件が保険業法に基づく既存の日本FSAグループ監督ルールの上に重ねられる。実務上の効果は、指定IAIGが国内規制提出に加えて、ComFrameに沿ったグループガバナンスとORSA文書を維持することである。
...
[[non-life-insurers/tokio-marine|Tokio Marine]] では、ホスト監督当局の参加者は通常、Philadelphia Consolidated、HCC、Pure Group を所管する米国州監督当局、Lloyd's syndicates を所管する英国PRA、追加のアジア・欧州監督当局を含む。[[life-insurers/dai-ichi-life|Dai-ichi Life]] では、TALを所管する豪州APRA、Protective Lifeを所管する米国州監督当局、一部アジア監督当局が通常含まれる。[[non-life-insurers/msad|MS&AD]] と [[non-life-insurers/sompo|Sompo]] では、ホスト監督当局の組み合わせは各グループ固有の海外P&Cフットプリントを反映する。

監督カレッジの仕組みは、ICSをグループレベルで実装可能にする実務インフラである。母国監督当局とホスト監督当局の間で情報共有が調整されなければ、グループレベルの資本数値は信頼性をもって測定できない。

## Aggregation Method (AM) parallel track

米国主導のAggregation Methodは、単一共通基準を適用するのではなく、各法域の規制資本比率（US NAIC RBC、Solvency II SCR、日本ESRなど）をグループレベルで集約する並行計算である。IAISは、AMが米国本社IAIGにとってICSの実行可能な代替となる「比較可能な結果」を生むかを評価している。

日本本社IAIGにとって、AMは主経路ではない。FSAはICSをグループレベルPCRとして実装する。ただしAMは、日本のグループが米国本社競合（Berkshire Hathaway、Chubb、MetLife、Prudential Financial など）の資本開示をどう読むか、また [[insurance/global-solvency-framework-comparison-matrix|the global solvency framework comparison matrix]] 経由で制度間比較可能性をどう評価するかに影響する。
...
| 6. 監督カレッジの結果 | 公表されている場合のFSAおよびホスト監督当局の公開要約 | クロスボーダー監督対話の論点、再建計画テストの結果 |

これらのレイヤーを突き合わせることで、最も完全な像が得られる。国内ESRだけに依拠するとグループレベルの範囲を見落とし、親グループのグループ資本だけに依拠すると日本免許エンティティの健全性ポジションを見落とす。
...
FSAは外国IAIG子会社に対して一方的にICSを適用しない。それは外国GWSの責任である。一方で、FSAは日本免許子会社にはESRを全面的に適用し、監督カレッジにも参加する。この分担があるため、親会社がグローバルには十分なICSカバレッジを持つ場合でも、[[insurance/foreign-life-affiliate-japan-positioning|foreign-life affiliates positioning]] は子会社レベルでFSA承認を条件とする親グループへの資本上流移転を論じる。
...
- IAIS: Insurance Capital Standard activity and topic ページ.
