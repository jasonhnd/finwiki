...
| 軸 | 収集すべきもの | 理由 |
|---|---|---|
| 親エコシステム | コマース、決済、通信、証券、信託銀行、地方銀行、または独立系グループ | 顧客獲得とクロスセルのコストを説明する。 |
| 口座規模 | 日付付きの口座数または顧客基盤 | 日付付きの場合のみ有用；口座数は急速に動きうる。 |
| 預金フランチャイズ | 預金残高、給与口座、決済口座、またはプロモーション残高 | 資金調達の安定性とメインバンク転換を決定づける。 |
| 貸出構成 | 住宅ローン、カードローン、個人ローン、事業ローン、有価証券担保、または意味のある貸出ブックなし | ROA、信用リスク、資本使用を駆動する。 |
...
[[banking/rakuten-bank|Rakuten Bank]]、[[banking/paypay-bank|PayPay Bank]]、[[banking/au-jibun-bank|au Jibun Bank]] はエコシステム型銀行である。その公的な提供面には、コマース、通信、カード、ウォレット、ポイント、証券、日常の決済行動が含まれる。メインアカウント転換の項目には、給与の入金、定期支払、住宅ローン、カード、高頻度の振込が含まれる。
...
1. 口座数の指標とその出典日付。
2. 親グループ、銀行単体、口座のみ、またはアクティブ顧客のデータカテゴリ。
3. 個別開示がある場合のBaaS口座とメインのリテール銀行口座。
4. 預金規模と貸出収益性。
5. [[banking/japan-baas-operating-models|BaaS operating models]] および [[payments/japan-bank-api-payment-agency-route|bank API payment agency route]] にリンクするAPI／銀行代理業カテゴリ。
6. [[securities/INDEX|securities index]] およびクイック入金経路にリンクする証券フロー。
...
- FSA：免許銀行一覧。
- 楽天グループ／楽天銀行の公開リリースおよび銀行開示。
- PayPay銀行の会社概要およびPayPayの保有関連リリース。
- au じぶん銀行の業績／開示ページ。
- SSNBの会社／NEOBANKページ。
- GMOあおぞらネット銀行の会社ページ。
- ソニー銀行のプロフィール／開示ページ。
