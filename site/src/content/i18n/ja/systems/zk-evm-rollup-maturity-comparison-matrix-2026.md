...
**DA層**: デフォルトは L1  blobs(EIP-4844)。validium モード(DAC 7-of-9 マルチシグ · enterprise プライベートチェーン用)を提供する。AggLayer は Polygon の cross-chain unifier で、複数 zkEVM が settlement を共有できる。
...
**エコシステム**: ~120 dApp · Ekubo · JediSwap · ZkLend · zkLend · Briq · Realms を含む。Argent / Braavos AA wallet はネイティブサポート。機関事例: Visa Aeon · Provenance pilot。
...
- **TVL と ecosystem traction は完全には相関しない**: zkSync は $2.0B の高 TVL を持つが、Type 4 が開発者移行コストを制限する。Linea の $1.2B は MetaMask トラフィック優位を借用する。Scroll / Polygon は Type 2  + 中等 TVL の「balanced choice」である。
...
- Polygon AggLayer は Polygon zkEVM + zkSync(一部)+ Astar + その他を連合し、settlement layer + cross-chain LP を共有する
