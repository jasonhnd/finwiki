...
この項目は [[systems/INDEX|systems index]] 配下に位置する。[[systems/canton-overview|Canton 総覧]] と [[systems/hyperledger-besu-overview|Hyperledger Besu 総覧]] をあわせて読み、2 つのコアな機関チェーンの技術的対位を理解し、[[systems/hyperledger-besu-vs-canton-migration|Besu vs Canton · JPM 移行経路]] と対照して具体的な G-SIB の実際の移行決定を確認する。パブリックチェーンの一極については [[fintech/blackrock-buidl-tokenized-mmf-overview|BlackRock BUIDL]] を参照。

### 1. Canton(Digital Asset + R3 合併陣営)

- **技術**:DAML(関数型 + 義務ベース言語)+ アプリケーションレベルのプライバシー + Global Synchronizer
...
- **典型的ユースケース**:トークン化 MMF のクロス参加者アトミック決済 + collateral mobility + 24/7 repo + tokenized deposit
...
- **優位性**:EVM 互換、Solidity ツールチェーン再利用、Linux Foundation 治理の透明性、許可制 validator セットがシンプル
- **制約**:アプリケーションレベルのプライバシー が Canton より弱い + マルチパーティアトミック決済には hack-y なコントラクト組み合わせが必要
...
## ユースケース軸 · 誰が何のプラットフォームを選ぶか

| ユースケース | プラットフォーム選択 | 理由 |
|---|---|---|
...
| ステーブルコイン決済(リテール) | Public L2(Base / Tron / Polygon PoS) | 低 gas + ユーザーウォレット密度 |
...
## トークン化ステーブルコイン × DLT プラットフォーム・マトリクス
...
- BIS reports — ホールセール CBDC & DLT pilots
