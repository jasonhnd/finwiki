...
- 7 つの主流 Ethereum L2  rollup について、2026-Q2 時点の **TVL · 日次アクティブアドレス · 日次取引量 · sequencer revenue · gas cost · DEX 深度 · 借貸規模 · RWA · 治理成熟度 · 経済圏多様性 · DA 選定** を 11 軸で比較する
...
- 経済圏多様性。DEX、借貸、RWA、stablecoin 流通
...
**RWA**: Centrifuge · Maple · Stripe USDC 決済 · PayPal PYUSD 流通。Coinbase USDC primary 入口により、Base は RWA + stablecoin 流通が最も深い L2 となる。
...
**経済圏多様性**: Consumer-heavy。DEX、meme coin、ソーシャル、ゲームが中心。Aerodrome / Friend.tech / Farcaster / Zora / Coinbase Smart ウォレット など。DeFi と消費アプリのバランスがある。
...
- Stripe / Shopify / Apple Pay 統合。USDC 決済
- Coinbase Smart ウォレット。ネイティブ AA、ETH gas 不要。[[systems/erc-4337-embedded-wallet-adoption|ERC-4337 embedded ウォレット adoption]] を参照
- 極めて低い gas と Aerodrome 深度により、meme coin / 消費 dApp が爆発
- Base 専用プロジェクト(Friend.tech · Farcaster · Zora)が Base に独自経済圏を提供
...
**経済圏多様性**: DeFi-heavy。perp、spot、借貸、yield farming が中心。GMX / Camelot / Pendle / Vela など。消費アプリは少ない。
...
**経済圏多様性**: DeFi + 一部消費。Worldcoin on OP Stack、Zora、Mode、Lyra など。消費 dApp では Base 以外の第二の L2 プラットフォーム。
...
- Retroactive Public Goods Funding(RPGF)は 経済圏 incentive のイノベーション
...
**経済圏多様性**: DeFi-heavy + AA-native dApp。Argent · Braavos AA ウォレット統合、ZK Stack hyperchain(GRVT perpetual · Treasure gaming · Cronos zkEVM)。
...
**経済圏多様性**: DeFi + Consensys 統合。MetaMask · Infura · Truffle ツールチェーンのネイティブサポート。dApp に MetaMask 統合事例が多い。
...
- Consensys ツールチェーン経済圏の深さ。Infura · Truffle · Hardhat など
...
**経済圏多様性**: DeFi + enterprise dApp + identity(Polygon ID)。多元化している。AggLayer 連邦は Polygon zkEVM + 他 zkEVM chain + Astar zkEVM。
...
- AggLayer 連邦により複数 zkEVM chain が 決済 を共有する。ZK rollup 中の modular RaaS フレーム
...
**経済圏多様性**: DeFi-heavy。Aave / Uniswap / Pendle / GMX など主流 fork が中心。独自 native dApp を欠く。Scroll Canvas は onboarding ツール。
...
| Rollup | TVL May 2026  | 日次アクティブ | 日次取引 | Sequencer Revenue | ユーザー Gas | DEX 深度 | 借貸規模 | RWA | 治理成熟度 | 経済圏特色 | DA Layer |
|---|---|---|---|---|---|---|---|---|---|---|---|
| **Base (Coinbase)** | **~$18B (L2 首位)** | **3-5M (首位)** | 10-15M | $30-100M ARR | $0.0005-0.005  | Aerodrome $3-5B · Uniswap · Curve | Aave $2-4B · Morpho $500M-1B | Stripe USDC · PayPal PYUSD · Centrifuge | Stage 1  · SC 7/13  · 7d timelock | Consumer-heavy · meme/social/game · Coinbase Smart ウォレット | L1 blobs only |
...
- **経済圏深度**: Base = consumer + DeFi。Arbitrum = DeFi 専門。Optimism = Superchain 連邦。zkSync = AA native。Linea = MetaMask トラフィック。Polygon zkEVM = enterprise pilot。Scroll = EVM 純度
...
- Coinbase Smart ウォレット(ネイティブ AA。[[systems/erc-4337-embedded-wallet-adoption|ERC-4337 embedded ウォレット adoption]] を参照)はデフォルトで Base に deploy される。ETH gas 不要で、ユーザー体験は Web2 に近い

**Stripe / Shopify / Apple Pay 統合**:

- Stripe は 2024-2025 年に「USDC on Base」加盟店決済 を発表し、cross-border payment コストを大幅に削減
- Shopify 加盟店の pay-with-USDC のデフォルトは Base
- Apple Pay は Coinbase ウォレット経由で Base と統合
...
- Farcaster(分散型ソーシャル)は主に Base に deploy される。Frames(Farcaster 内蔵 mini-app)経済圏が爆発
...
- **OP Stack 統合**: Base は OP Superchain の一員であり、OP 経済圏の溢出を享受しつつ独立ブランドを維持する
...
- **共有決済**。L1 上の相互信頼ブリッジ
- **共有 RPGF**。Retroactive Public Goods Funding と OP 治理 token 配布各 Superchain 子チェーンは独立運営 + 独立ブランドを維持しつつ、基盤 stack を共有する。
...
**Base は規制による sequencer 制約を受けるか**: Coinbase は SEC / CFTC の規制対象である。SEC が Base sequencer に OFAC 制裁アドレスの検閲を強制すれば、Base の ETH-aligned L2 経済圏における status に影響する。現在 Coinbase は公開で検閲しないと約束している。ただし長期リスクは存在する。
...
**Account Abstraction 主流化タイムテーブル**: Coinbase Smart ウォレット / Argent / Braavos / MetaMask Smart Account がいずれも推進している。2026 ユーザー ~20-30% が AA を使用する可能性があり、2027-2028 に主流化する可能性がある。[[systems/erc-4337-embedded-wallet-adoption|ERC-4337 embedded ウォレット adoption]] を参照。これにより L2 ユーザー体験が大幅に改善する。
...
- [[systems/erc-4337-embedded-wallet-adoption|ERC-4337 embedded ウォレット adoption]]
