...
- **Blob の仕組み**:各 block target=3 / max=6 blob(Dencun オリジナル)· 各 blob 128 KiB(131072 bytes)· 合計 max ~768 KiB/block(Dencun)
...
- **Blob 手数料市場**:EIP-1559 スタイル · 独立 base fee · target を超えると fee が指数増加 · target を下回ると fee 減衰 · gas fee market と非結合
...
この項目は [[systems/INDEX|systems index]] 配下に位置する。EIP-4844,  を直接拡張する 2025-Q2  の blob 目標倍増は [[systems/pectra-eip-7691-blob-l2-impact|Pectra EIP-7691 · blob 倍増と L2 経済連鎖]] と照合し、Ethereum アップグレード全体の文脈は [[systems/pectra-upgrade-overview|Pectra upgrade overview]] を参照する。Blob 経済を構造的に重要にする L1/L2  の戦略的位置づけは [[systems/vitalik-l1-l2-strategy-anchor|Vitalik L1/L2 strategy anchor]] を参照。Blob 市場が再編する DA レイヤー競争は [[systems/data-availability-celestia-eigenda-blob-comparison|DA layer 全景対照]]、blob を消費する rollup エコシステムは [[systems/rollup-market-share-2026-arbitrum-optimism-base|rollup market share 2026]] と [[systems/zk-evm-rollup-maturity-comparison-matrix-2026|ZK-EVM rollup maturity matrix 2026]] を参照。Blob 処理の背後にあるバリデータ経済を支える機関向けステーキング経路は [[systems/pectra-eip-7251-institutional-staking|Pectra EIP-7251 · institutional staking]] と [[systems/bft-validator-economy-overview|BFT validator 経済学概観]]、restaking で保護される代替 DA は [[systems/eigenlayer-overview|EigenLayer overview]] と [[systems/restaking-avs-landscape-matrix-eigenlayer-vs-symbiotic|restaking AVS landscape matrix]] を参照。
...
### Blob 手数料市場(EIP-1559 スタイル · 独立)

**コアメカニズム**(EIP-4844 spec):
...
|---|---|---|---|---|---|
| **GB あたりコスト** | $500-2000+(GB あたり calldata 相当 ETH gas) | ~$100-300/GB-day | $60-150/GB-day | $20-50/GB-day | $10-30/GB-day |
...
| **スループット** | L1 gas 制限を受ける(~50-100 KB/block 実際) | 0.75 MB/block (Dencun) → 1.1 MB (Pectra) → ~11 MB (Fusaka PeerDAS) | 15 MB/s sustained | 2 MB/block / 6s = 28 GB/day | 理論 100 MB/s |
| **ファイナリティ** | L1 finality(~12-15 min) | L1 finality(~12-15 min) | ~10-30s + slashing | ~6s (Tendermint) | ~2-3s (NEAR) |
...
| **手数料市場設計** | user gas と共有(混雑伝染) | 独立 EIP-1559 base fee | EigenDA dispatcher が独自プライシング | Celestia 独自プライシング (TIA 計価) | NEAR storage fee モデル |
