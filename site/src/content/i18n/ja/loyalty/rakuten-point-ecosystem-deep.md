...
楽天ポイントは複数の会計バケットにまたがるため、単一の負債として読むべきではない。[[loyalty/point-liability-accounting-boundary|point liability accounting boundary]] のバケットを用いると、
...
- **現金チャージ価値**は別の規制に属する——楽天のプリペイド／電子マネーの系譜（楽天 Edy）は、ロイヤルティ会計よりも資金決済法のプリペイド層に近い。[[payments/edy-rakuten-prepaid|Edy / Rakuten prepaid]] と [[payments/funds-transfer-vs-prepaid-boundary|funds-transfer vs prepaid boundary]] を参照。
