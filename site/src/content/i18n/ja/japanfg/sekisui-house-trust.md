---
source: japanfg/sekisui-house-trust
source_hash: f6989566c4a0e7a1
lang: ja
status: machine
fidelity: ok
title: "積水ハウス信託"
translated_at: 2026-06-02T15:14:17.151Z
---

# 積水ハウス信託

## ウィキ上の位置づけ

積水ハウス信託は [[JapanFG/INDEX|JapanFG]] 配下の独立した信託会社ルートで、[[JapanFG/trust-companies-japan-index|the Japan trust companies registry index]] から昇格した項目。[[banking/japan-trust-bank-custody-map|Japan trust-bank custody map]]、[[policy-finance/japan-housing-finance-agency|Japan Housing Finance Agency]]、[[JapanFG/daiwa-living-trust|Daiwa Living Trust]] と併読する。

このページは、不動産管理信託、賃貸住宅の承継、住宅会社による信託サービスに関する論点で使う。

## 概要

積水ハウス信託は、Sekisui House group に連なる管理型信託会社である。同社ページは管理型信託業としての登録を説明し、賃貸住宅管理や承継をめぐる不動産所有者の課題解決を支援するために開業したと記載している。

このページが重要なのは、住宅会社系の信託ルートだからである。建設・住宅グループが、長期的な不動産管理と家族内承継を支援するために信託会社機能をどう使えるかを示す。

## 法人境界

| 項目 | 読み方 |
|---|---|
| 正式名 | 積水ハウス信託株式会社 |
| ライセンス種別 | 管理型信託会社 |
| 公開情報上の焦点 | 不動産管理・承継信託 |
| グループ文脈 | Sekisui House group |
| 登録上の経路 | 金融庁の信託会社一覧 |

このページは信託会社に焦点を絞る。住宅・建設の事業会社側は、必要に応じて [[manufacturing/INDEX|manufacturing]] または事業ページを使う。

## 事業上の役割マップ

| 機能 | FinWiki 上の関連性 |
|---|---|
| 不動産管理信託 | 不動産所有者の承継・管理に関する論点の入口になる。 |
| 賃貸住宅 | 公開ページでは賃貸住宅所有者が対象ユーザー層として示されている。 |
| グループの信託機能 | 住宅会社による信託サービスへの垂直統合を示す。 |
| 承継・管理 | [[JapanFG/daiwa-living-trust|Daiwa Living Trust]] および [[JapanFG/starts-trust|Starts Trust]] との比較対象として有用。 |

## 公開情報上の文脈

住宅資産を金融資産・承継資産として扱う論点では、積水ハウス信託が FinWiki 上で有用である。信託会社の役割は受託財産の管理・運営であり、一般的な銀行バランスシートを提供することではない。

商品別の開示がない限り、融資審査、税務上のメリット、賃貸利回りの結果を推定しない。

## 調査チェックリスト

1. 金融庁の信託会社一覧で現在の登録を確認する。
2. 登録、目的、信託手続きは会社ページを使う。
3. 設立背景は Sekisui House のリリースを使う。
4. 住宅建設、不動産管理、融資、信託管理を分ける。

## 関連項目

- [[JapanFG/INDEX]]
- [[JapanFG/trust-companies-japan-index]]
- [[banking/japan-trust-bank-custody-map]]
- [[policy-finance/japan-housing-finance-agency]]
- [[JapanFG/daiwa-living-trust]]
- [[JapanFG/starts-trust]]
- [[INDEX|FinWiki index]]

## 出典

- FSA: 信託会社一覧。
- Sekisui House Trust: 会社情報・信託手続きページ。
- Sekisui House: 設立に関する公開リリース。
