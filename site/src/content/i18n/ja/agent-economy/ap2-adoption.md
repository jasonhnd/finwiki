...
- Coinbase が AP2 の USDC 決済 adapter の役割を担う ^[extracted]
- Shopify / Square は初期の 加盟店統合者(2026-Q2 から pilot 開始) ^[extracted]
...
Google の4本柱の組み合わせ:**Google Pay(6 億 mobile ウォレットユーザー)** + **Gemini(数億の Android/Chrome/Workspace 接点)** + **W3C VC 標準への影響力(中核貢献者)** + **fraud detection AI(世界最大の ML 不正対策インフラ)** — どれか単独では標準を押し上げられないが、組み合わせると agent 決済の閉ループを形成する([[fintech/embedded-wallet-fintech-disintermediation-stripe-trojan-horse|Stripe 5層 Trojan horse]] の縦型 distribution 支配と対照)。
