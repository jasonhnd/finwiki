...
translated_at: 2026-06-02T11:47:37.306Z
---
...
| Coinbase Smart Wallet 実装 | Base メインネットで稼働中 | 2025-Q3 |
| Safe(Gnosis Safe)実装 | Safe v1.4+ でモジュールとして稼働中 | 2025-Q4 |
| Rabby / Phantom / Trust Wallet 実装 | 未出荷 | — |
...
エージェント決済に特化すると、ERC-4337 はエージェントの権限付きスコープを保持する基盤となる**スマートコントラクトウォレット**を提供する。本番アーキテクチャについては [[systems/erc-4337-overview|ERC-4337 総覧]] とより深い UserOp / バンドラーフローを参照。Coinbase Smart Wallet, Safe, ZeroDev, Biconomy、および2026 のほとんどの「パスキー + AA」モバイルウォレットは ERC-4337  SCW である。
...
| Coinbase Wallet EOA + 7702 | 稼働中 | 2025-Q4 |
...
| **Google Gemini Agent** | 未統合 | ファーストパーティ | ファーストパーティ(Android ウォレットパイロット) | ファーストパーティ(Android Wallet 4337 パス) | ファーストパーティ |
...
1. **ウォレット層**:ユーザーは MetaMask EOA + ERC-7702 委任、または Coinbase Smart Wallet(ERC-4337  SCW)を持つ。
...
| 本番 / メインネット稼働 | ERC-7702  EOA 委任 | Pectra 出荷 + MetaMask / Coinbase Wallet UX |
...
| 2025-Q3 | MetaMask + Coinbase Smart Wallet が ERC-7715 `wallet_grantPermissions` を出荷 |
...
本番エージェント決済スタックは5つの異なるセキュリティ境界を構成する。2026 のリファレンスリスト:
