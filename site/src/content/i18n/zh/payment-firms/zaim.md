---
source: japanfg/zaim
source_hash: 7f0983ee221a0109
lang: zh
status: machine
fidelity: ok
title: "Zaim"
translated_at: 2026-06-01T02:31:57.173Z
---
# Zaim

## Wiki route

本条目位于 [[JapanFG/INDEX|JapanFG index]] 之下，是与规模更大的 [[payment-firms/money-forward|Money Forward]] ME 并列的 **PFM（个人财务管理）**应用。可与 [[payment-firms/bnpl-landscape|Japan BNPL landscape]] 一起阅读，以理解相邻的消费金融语境；也可与 [[payment-firms/freee|freee]] / [[payment-firms/smartbank|SmartBank]] 这一组家庭金融同类页面，以及说明 KDDI 系分支故事的 [[megabanks/au-fh|au フィナンシャル HD]] 一起阅读。[[payments/japan-bnpl-credit-purchase-boundary|BNPL and credit-purchase boundary]] 和 [[payments/credit-purchase-card-operators-japan-index|credit-purchase registry]] 位于更下一层监管栈，[[payments/japan-payment-scheme-economics-matrix|Japan payment scheme economics matrix]] 则说明 PFM 应用为何依赖账户 API 经济性。

## TL;DR

Zaim 是 2011 年 公开、在日本国内资历最久的一类 **家计簿应用**。它与 [[payment-firms/money-forward|Money Forward]] ME 一起塑造了面向个人的 PFM 类别，并作为 fintech 版图的一部分，在 2022-04  **成为 KDDI 子公司**，被重新定位为 au 经济圈（[[megabanks/au-fh|au フィナンシャル HD]]）中的数据起点资产。其特点是收据 OCR、银行和银行卡 API 连接，以及地方政府补助金搜索等功能组合。

## 1. 法人和牌照边界

| Item | Reading |
|---|---|
| Legal entity | 株式会社 Zaim |
| Group | **KDDI 100% 子公司**（2022-04  完全子公司化），位于 [[megabanks/au-fh|au フィナンシャル HD]] 经济圈内 |
| Product | Zaim 家计簿应用（iOS / Android / Web） |

## 2. Business role

- 通过拍摄收据、OCR、自动分类，以及银行和银行卡明细自动连接，将家庭收支可视化的 PFM 应用。^[extracted]
- 其差异化在于，除家计功能外，还拥有地方政府补助金 / 公共支援服务搜索等 **生活信息层**，这一点与 [[payment-firms/money-forward|Money Forward]] ME 区分开来。^[extracted]

## 3. Why this standalone page matters

PFM 类别常被视为 [[payment-firms/money-forward|Money Forward]] ME 一强，但 Zaim 走过了 **独立创业 → KDDI 完全子公司化** 的少见退出路径，是通信运营商经济圈内 fintech 资产的典型案例，因此需要独立页面。

## Related

- [[payment-firms/money-forward]]
- [[payment-firms/freee]]
- [[payment-firms/smartbank]]
- [[megabanks/au-fh]]
- [[payment-firms/au-payment]]
- [[payment-firms/bnpl-landscape]]
- [[payment-firms/famima-digital-one]]
- [[payments/japan-bnpl-credit-purchase-boundary]]
- [[payments/credit-purchase-card-operators-japan-index]]
- [[payments/japan-payment-scheme-economics-matrix]]

## Sources

- Zaim corporate site: https://content.zaim.net/
- Zaim company profile: https://content.zaim.net/company/
- KDDI 2022-04-14 press release on Zaim becoming a wholly owned subsidiary: https://news.kddi.com/kddi/corporate/newsrelease/2022/04/14/6014.html
