...
本条目位于 [[agent-economy/INDEX|agent-economy index]]，从 Agent 执行视角解读 [[systems/erc-7702-overview|ERC-7702]]。SCW 侧路径见 [[agent-economy/erc-4337-account-abstraction-primer-for-agents|ERC-4337 Agent primer]]，通用协议比较见 [[systems/erc-7702-vs-erc-4337|systems-side comparison]]，上层权限层见 [[agent-economy/erc-7715-overview|ERC-7715]]。更广泛的协议图谱见 [[agent-economy/ai-agent-payment-protocols-overview|seven-protocol overview]] 和 [[agent-economy/ai-agent-payment-protocols-seven-layers|seven-layer table]]；系统 / 监管边界见 [[systems/INDEX|systems index]]。
...
这就是为何「7702 是 4337的竞争对手吗？」的 agent-economy 解读大体是「不，它们组合」。7702 解决 *地址迁移* 问题；4337 解决 *执行语义* 问题；合并后，它们在用户既有的地址上给同一个 agent 与一个全新 SCW 相同的受限定、可代付、可撤销的执行面。
...
对一个想 *帮助* 用户撤销一个恶意委托的 AI agent,该 agent 必须构造一个指向 `address(0)`、由该 EOA 签名的新 `SET_CODE_TX`。agent 无法仅凭一个 session key 做到这点 — 撤销需要 root EOA 签名。这种不对称是刻意的:用户必须始终保留撤销的能力,即便 agent 的 session-key 基础设施被入侵。误解此点、并试图让撤销可经由 session key 进行的钱包实现,正在制造一个安全漏洞。
...
**场景 C — 出于安全的 EOA「降级」。** 一个去年安装了一个 7702 委托并给了一个 agent 一个 session key 的用户,因怀疑 agent 平台被入侵而想撤销一切。一个指向 `address(0)` 的 `SET_CODE_TX` 完全移除该委托;全部已安装的 session key 在单个原子操作中变得不可达。这比一个撤销通常需要与 SCW 的 module-uninstall 逻辑交互的 4337 SCW 显著更简单。
