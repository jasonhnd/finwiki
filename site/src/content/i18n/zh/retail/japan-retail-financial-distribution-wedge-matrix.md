---
source: retail/japan-retail-financial-distribution-wedge-matrix
source_hash: 88e30bcfeed0b213
lang: zh
status: machine
fidelity: ok
title: "日本零售金融分销楔形矩阵"
translated_at: 2026-06-01T03:31:12.234Z
---
﻿
# 日本零售金融分销楔形矩阵

## 长话短说

日本三大零售金融堆栈 — **AEON**（以购物中心为主）、**Seven & i**（以便利店 + ATM 为主）以及 **Lawson + KDDI**（2024 KDDI / 三菱联合私有化后的便利店 + 电信混合）从消费者角度来看，— 表面上看起来很相似（商店、卡、积分、应用程序、银行或银行附近的产品）。他们在结构上有所不同，**他们利用哪种零售楔子，他们持有哪个许可证级别，他们拥有哪种点货币，他们持有哪种银行执照，以及他们如何通过客户 ID 图货币化**。该矩阵并排收集这些轴，因此可以在品牌问题之前回答楔形问题。位于 [[retail/INDEX|retail INDEX]] 内部，作为 [[retail/aeon-group|AEON Group]]、[[retail/seven-and-i-hd|Seven & i Holdings]] 和 [[retail/lawson-kddi-retail-finance|Lawson + KDDI retail finance]] 之间的横切比较表面。

## 维基路线

位于 [[retail/INDEX|retail INDEX]] 内部作为矩阵式阅读表面。每个实体的三个页面是 [[retail/aeon-group|AEON Group]]、[[retail/seven-and-i-hd|Seven & i Holdings]] 和 [[retail/lawson-kddi-retail-finance|Lawson + KDDI retail finance]]。忠诚度/积分上下文位于 [[loyalty/INDEX|loyalty INDEX]]，更广阔的前景位于 [[loyalty/japan-points-landscape|Japan points landscape]]，以及 [[loyalty/v-point-smbc-ccc-case|V Point (SMBC × CCC) case]]、[[loyalty/d-point-au-kddi-docomo-telco-point-consolidation|d Point / au / KDDI / docomo telco-point consolidation]] 和 [[loyalty/point-liability-accounting-boundary|point liability accounting boundary]] 的特定深入研究。银行牌照方面位于 [[banking/japan-net-bank-competition-map|Japan net bank competition map]] 和 [[banking/japan-baas-operating-models|Japan BaaS operating models]]，加上 [[banking/quick-deposit-four-methods|quick-deposit four methods]] 的融资轨道环境。支付通道上下文位于 [[payments/INDEX|payments INDEX]]，具有通往 [[payments/japan-code-payment-competitive-map|Japan code-payment competitive map]]、[[payments/japan-transit-prepaid-suica-pasmo-icoca-economics|Japan transit prepaid Suica / PASMO / ICOCA economics]]、[[payments/waon-nanaco-retail-prepaid-comparison|WAON vs nanaco retail prepaid comparison]] 和 [[payments/famipay-valucreate-strategy|FamiPay / Valucreate strategy]] 的特定路线。每个实体的财务页面：[[JapanFG/aeon-bank|Aeon Bank]]、[[JapanFG/aeon-financial-service|Aeon Financial Service]]、[[JapanFG/seven-bank|Seven Bank]]、[[JapanFG/seven-card-service|Seven Card Service]]、[[JapanFG/seven-payment-service|Seven Payment Service]]、[[JapanFG/lawson-bank|Lawson Bank]]、[[JapanFG/au-fh|au Financial Holdings]]、[[JapanFG/au-payment|au Payment]] 和[[JapanFG/recruit-mufg-business|Recruit / MUFG business]]。

## 为什么这个矩阵很重要

“零售集团经营银行”的框架过于粗糙。三个观察结果使楔形问题值得拥有自己的矩阵。

1。 **楔子不同。** AEON 的楔子是**购物中心和超市集水区**，具有多小时的停留时间。 Seven & i 的楔子是**便利店频率加上自有 ATM 密度**。在 KDDI 和三菱公司联合私有化 2024  后，Lawson 的楔子是**便利店密度加上电信经济区**（au PAY、au PAY 卡、au じぶん銀行），它由它托管而不是拥有。
2。 **牌照级别不同。** AEON 拥有 [[JapanFG/aeon-bank|Aeon Bank]] 下的完整普通银行牌照，可提供抵押贷款和投资信托分配。七号拥有 [[JapanFG/seven-bank|Seven Bank]] 下的 ATM 专业普通银行牌照，并拥有 B2B 现金和认证产品。 Lawson 在 [[JapanFG/lawson-bank|Lawson Bank]] 下持有规模小得多的 ATM 普通银行牌照，并通过合资母公司间接接触 [[JapanFG/au-fh|au FH]]。
3。 **积分货币不同。** AEON 的 WAON POINT 是一种 **封闭的集团内货币**。 Seven 的 nanaco 点是**封闭的集团内货币**。 Lawson 的忠诚度是由 Loyalty Marketing（CCC 剥离后的三菱集团）— 控制的**多品牌 Pontaポイント 联盟**，这是一个完全不同的网络效应形状，包括 JAL、KDDI、加油站和许多非 Lawson 连锁店。

这三个差异导致交叉销售的 CAC 存在显着差异、结算数据货币化存在显着差异，以及银行、BaaS、预付费和 銀行代理業（银行机构）注册之间存在显着不同的监管边界。

## 每个实体部分

### 永旺集团 — 商城锚定全栈零售金融

- **锚楔**：百货商店、超市和购物中心。 **多小时停留时间**意味着柜台主导销售（卡申请、保险主导、抵押贷款咨询）是经济的 — 与便利店吞吐量经济学根本不同。
- **银行章程**：[[JapanFG/aeon-bank|Aeon Bank]]（成立2007-10-29，根据银行法获得普通银行牌照，母公司[[JapanFG/aeon-financial-service|Aeon Financial Service]] 100%子公司）。全零售银行—存款、抵押贷款、投资信托分销、保险分销。永旺集团设施和车站内 6,500+ 的近似 ATM 网络。
- **发卡机构**：[[JapanFG/aeon-financial-service|Aeon Financial Service]] 旗下的 AEON 卡（AFS、TSE Prime 8570）— 是国内流通卡数量最大的信用卡发卡机构之一。卡+WAON双轨发行，Visa/Mastercard/JCB品牌变种。
- **预付费电子货币**：**WAON**（《支付服务法》预付费支付工具登记册下基于 FeliCa 的储值电子货币）。封闭式方案；商户足迹最深的AEON门店；许多公交附近和便利店商家都接受。
- **积分币**：**WAON POINT** — 封闭团内奖励。与其他共通ポイント方案的跨程序转换受到限制；忠诚度价值是群体内部的粘性，而不是网络效应。
- **应用程序层**：**iAEON** + **AEON Pay**（与 WAON、AEON 卡、优惠券、客户 ID 集成的二维码/代码支付应用程序）。位于 [[payments/japan-code-payment-competitive-map|Japan code-payment competitive map]] 内部，作为 PayPay / d払い / au PAY / 楽天ペイ 后面的小规模钱包，但具有**自有商店活动控制**。
- **客户ID图表**：AEON客户ID由iAEON注册+AEON卡+WAON会员+AEON银行账户锚定。集团内部营销的交叉参考；亚洲消费金融子公司（泰国、马来西亚、香港等）将ID图谱跨境延伸。
- **ATM 网络**：~6,500 ATM 遍布 AEON 设施和车站 — 规模较大，但不是主要的便利店 ATM 网络。战略地位是**商场走廊存款和余额检查基础设施**。
- **银行产品的 CAC**：**店内交叉销售较低**（在商场柜台进行抵押贷款咨询；在金融分行进行投资信托推介）。 **纯在线存款更高**，AEON 银行在应用程序用户体验和利率方面与 [[banking/japan-net-bank-competition-map|net banks]] 竞争。
- **结算数据货币化**：AEON 拥有三者中**丰富的店内 SKU 级购物篮数据**，因为杂货购物篮是主要的 SKU 类别。这是对美国“第一方零售媒体”模式最有力的类比。数据产品化路线贯穿永旺的零售媒体和品类管理职能。
- **监管边界**：拥有直接银行牌照+预付费票据发行人注册+保险招揽/代理牌照+亚洲消费金融子公司。无主要 BaaS 赞助商身份 — AEON 使用自己的许可证而不是批发它们。 銀行代理業（银行代理）通道位于 AFS / Aeon 银行内，用于店内分发。

### Seven & i Holdings — 便利店锚定ATM平台

- **锚楔**：7-11 便利店密度 — **日本按商店数量计算最大的 CVS 网络**（公开报道围绕 21,000+ 国内商店；国内 7-11 日本运营部门是特许经营核心）。高频、短停留时间的访问推动了 ATM、预付费和账单支付行为，而不是柜台主导的产品销售。
- **银行章程**：[[JapanFG/seven-bank|Seven Bank]]（TSE Prime 8410, 成立2001-04-10为爱华银行，开设2001-05-21，更名为2005-10）。 **ATM 专业普通银行**。大约 **27,000+ 拥有的 ATM 放置在 7-11 和其他合作伙伴站点内，~600  合作伙伴金融机构支付 ATM 使用费。主要收入是**合作银行支付的ATM使用费**，而不是净息差。
- **发卡臂**：[[JapanFG/seven-card-service|Seven Card Service]] — 七卡（Visa/JCB 品牌）和nanaco 联卡层。发行量小于 AEON 卡，但与 nanaco 充值和 7-11 零售优惠紧密结合。
- **预付费电子货币**：**nanaco**（基于FeliCa的储值电子货币）。封闭式方案； Seven & i 集团商店内最深入的商户足迹；上限和充值机制受《支付服务法》预付费工具发行人登记册管辖。与 [[payments/waon-nanaco-retail-prepaid-comparison|WAON vs nanaco retail prepaid comparison]] 中的 WAON 并列。
- **积分币**：**nanaco积分** — 封闭团内奖励。无共同点联盟会员资格；跨程序转换有限。群体内部粘性而非网络效应。
- **应用层**：**7iD**（Seven & i 统一客户 ID 程序）+ nanaco 应用程序 + 7NOW 应用程序界面。Seven 的二维码支付努力（最著名的是 7pay、已停产的 2019）历来弱于 AEON Pay 或 PayPay — Seven 的战略面已退回到 nanaco + ATM + 7iD，而不是新钱包。
- **客户ID图**：**7iD**是统一ID，锚定nanaco、七卡、全能7 EC、7NOW和七银行账户联动。按访问次数计算，7iD 的覆盖范围是日本零售业中最频繁的 ID-touch（因为 7-11 英尺的客流量频率），但覆盖的每次访问 SKU 范围比 AEON 篮子窄。
- **ATM 网络**：~27,000+ ATM — **日本最大的便利店 ATM 网络**。战略定位是针对更广泛的银行系统的**最后一英里现金访问和身份验证基础设施**；合作伙伴银行实际上将分行 ATM 业务外包给七家银行。
- **银行产品的 CAC**：**ATM 费用收入非常低**（每个使用 7-11 ATM 的合作银行持卡人都是一个计费事件，七方获取成本为零）。 **自营七家银行存款账户较高**，因为在大多数情况下该银行不是消费者的主要银行 — 七家银行与大型银行和网络银行竞争存款余额。 **在 POS 柜台发行 nanaco 的价格较低**。
- **结算数据变现**：Seven 的数据**高频、小篮子规模、低 SKU 广度** — 日需定价和缺货检测信号较强，跨品类篮子亲和力信号较弱。零售媒体的潜力是真实的，但每次访问的数据形状限制了深度。
- **监管边界**：拥有直接银行牌照+预付费票据发行人注册+电子支付机构相邻注册。 B2B 层（[[JapanFG/seven-payment-service|Seven Payment Service]] 和七家银行的企业支付/ATM 收据产品）比面向消费者的零售金融更接近**资金转移和银行服务实用程序**边界。不是 AEON/大型银行意义上的 BaaS 赞助商。

### Lawson + KDDI — 便利店 + 电信混合（2024 后）

- **锚楔**：Lawson便利店密度（**按商店数量计算，是继7-Eleven和FamilyMart之后的第三大CVS连锁店**；公开报道围绕14,000+国内商店）。 2024-02 联合私有化（¥4,952 每股，KDDI+三菱商事50/50, 完成中期2024）后，楔子成为 **CVS 密度 × KDDI au 用户群 × Pontaポイント 联盟**。添加的战略资产不是新的银行许可—，而是通过Lawson POS和Pontaポイント网络路由的au PAY经济区。
- **银行牌照**：[[JapanFG/lawson-bank|Lawson Bank]]（成立2017-11-15，开业2018-10-15，普通银行牌照）。无论是从 ATM 数量（大约是劳森商店中的 13,000 ATM）还是从合作银行覆盖范围来看，都远远小于 [[JapanFG/seven-bank|Seven Bank]]。通过 KDDI 母公司 — 间接接触 **au じぶん銀行**，请参阅 [[JapanFG/au-fh|au Financial Holdings]]。
- **发卡机构**：Lawson 不拥有 AEON 卡/七卡规模的主要信用卡发卡机构。 **au PAY Card**（通过KDDI方au Financial Holdings发行）是2024之后通过Lawson渠道推出的信用卡。 Lawson Ponta 信用卡变种存在，但发卡机构的重心位于 KDDI 经济区内，而不是 Lawson 内部。
- **预付费电子货币**：Lawson **不运行与 WAON 或 nanaco 类似的基于 FeliCa 的专有储值方案**。 POS 接受交通 IC（Suica / PASMO / ICOCA 系列）和主要代码支付钱包。这是结构性差异—无封闭式预付费锁定。
- **积分货币**：**Pontaポイント** — **多品牌联盟**，由Loyalty Marketing, Inc.（CCC业务剥离后的三菱商事集团）运营。会员包括JAL、KDDI、加油站（Shell/ENEOS）、Hot Pepper/餐馆、Lawson以及许多非Lawson商户。网络效应与 WAON POINT/nanaco point 根本不同。
- **应用层**：**Lawson应用程序+au PAY应用程序+Pontaポイント应用程序**三合一。 Lawson POS 接受 au PAY（KDDI 侧，参见 [[JapanFG/au-payment|au Payment]]）以及 PayPay、d払い、楽天ペイ、JCB / Visa / Mastercard、交通 IC 和 Pontaポイント。没有任何一个应用内钱包在劳森领域占据主导地位。
- **客户 ID 图表**：**分散在三个 ID 中** — Lawson ID、au ID (KDDI)、Pontaポイント ID。 2024 以后的战略问题是这些是否可以合并或保持联合。 au + Lawson 跨 ID 联动正在进行中，但 iAEON / 7iD 规模的**尚不存在单一统一的零售金融 ID**。
- **ATM 网络**：~13,000 Lawson 银行 ATM。相对于七家银行来说，战略地位相对较小，但仍然是一个有意义的店内现金和代码支付充值节点。
- **银行产品的CAC**：**混合。** Au PAY 卡和au じぶん銀行 收购可以通过Lawson POS 和Lawson 应用程序推送，但客户通常通过KDDI 移动订阅而不是Lawson — 进入，因此CAC 结构是**电信锚定，而不是零售锚定**。劳森银行自己的存款账户收购规模较小。
- **结算数据货币化**：结算数据**由三方共享** — Lawson POS 数据、KDDI au 电信/支付数据、三菱公司分销数据 (Ponta)。没有任何一方可以控制整个图表。这在结构上弱于 AEON 的第一方数据或 Seven 的 7iD 锚定数据，但当电信/移动/餐厅/电子商务合作伙伴聚合时，**跨类别更强**。
- **监管边界**：Lawson 银行许可证 + Loyalty Marketing 下的 Pontaポイント 忠诚度运营 + 通过 Lawson POS 受理的 KDDI 侧预付费和资金转账许可证 (au PAY)。 **没有任何单一法人实体拥有集成堆栈**，由于 50/50 JV 治理 —，KDDI 和三菱都不能在未经对方同意的情况下单方面在 Lawson 内部整合许可证。

## 大比较矩阵表

下面的矩阵将楔形轴并排折叠。数字是公开报告的近似值，并随披露周期 — 的变化而变化，请查阅每个实体的页面以获取最新的文件。

| 轴 | 永旺 | 七与我 | 罗森+KDDI |
|---|---|---|---|
| 锚定零售楔子 | 商场+超市（长驻） | 7-11 CVS（高频） | Lawson CVS + KDDI au 电信区 |
| 国内门店足迹 | ~6,500+ AEON集团设施、超市、商场、药妆店 | ~21,000+ 7-11家日本店 | ~14,000+罗森店 |
| 每日/每月大约客流量 | 每次访问停留时间长；集团横幅的每月客流量达数亿 | 每家商店的最高访问频率； 7-11 集团每月客流量低至数十亿 | CVS 客流量排名第三；每月客流量低至中亿 |
| POS 的主要支付方式 | FeliCa(WAON)+AEON卡+AEON Pay(QR)+交通IC+PayPay / d払い / au PAY / 楽天ペイ受理 | FeliCa(nanaco)+7卡+交通IC+PayPay/d払い/au PAY/楽天ペイ受理 | 无专有的 FeliCa 方案；交通IC + au PAY + PayPay / d払い / 楽天ペイ + Pontaポイント |
| 拥有预付电子货币 | **WAON**（已关闭） | **nanaco**（已关闭） | **非专有** |
| 自有二维码支付钱包 | **AEON Pay / iAEON** | 无规模（7pay 已停产 2019） | 非专有； **au PAY**（KDDI 侧）通过 |
| 银行牌照等级 | 普通银行（全零售银行） — [[JapanFG/aeon-bank\|Aeon Bank]] | 普通银行（ATM专用） — [[JapanFG/seven-bank\|Seven Bank]] | 普通银行（仅限ATM、子规模）— [[JapanFG/lawson-bank\|Lawson Bank]] + 间接[[JapanFG/au-fh\|au FH]] / au じぶん銀行 |
| 发卡机构臂 | **AEON 卡** [[JapanFG/aeon-financial-service\|AFS]] (TSE Prime 8570) | **七张卡** [[JapanFG/seven-card-service\|Seven Card Service]] 下 | Lawson Ponta 卡 + **au PAY 卡**（KDDI 方） — 没有 AEON 卡规模的大型集团内发行人 |
| 积分货币 | **WAON POINT**（团体内封闭） | **nanaco 点**（团体内封闭） | **Pontaポイント**（多品牌联盟：JAL、KDDI、燃气、餐厅、罗森等） |
| 客户 ID 锚 | **iAEON** + AEON 卡 + WAON | **7iD** + nanaco + 七卡 | 联合：Lawson ID + au ID + Ponta ID |
| 拥有ATM网络 | ~6,500 AEON-横幅ATM机 | **~27,000+ 七台银行 ATM**（最大的 CVS 银行 ATM 网络） | ~13,000 Lawson 银行 ATM 机 |
| BaaS 发起人身份 | 有限公司 — AEON 消耗自己的许可证 | 有限 — 七家银行向合作伙伴银行批发 **ATM 手续费收入**（与 BaaS 不同） | 有限 — KDDI 侧 au FH 可以通过 au じぶん銀行 批发银行业务，但罗森银行本身规模较小 |
| 亚洲/跨境足迹 | **重要** — AFS旗下亚洲消费金融子公司（泰国、马来西亚、香港、柬埔寨、缅甸） | **选择性** — 七家银行东盟ATM操作（印尼、菲律宾）+西联汇款 | **直接可忽略不计** — KDDI 移动国际足迹存在，但不存在零售金融 |
| 保险分销 | 是 — 通过 AFS / Aeon Allianz Life 柜台主导的保险推销 | 有限公司 — 主要产品捆绑 | 通过电信公司分配的有限 — KDDI au 保险线路 |
| 抵押贷款发放 | **是的 — 永旺银行**是一家有意义的零售抵押贷款发起人（商场金融分行的固定费用+浮动利率产品） | 有限公司—不是主要产品线 | 没有规模化 |
| 银行产品CAC | **低**，适用于店内交叉销售；网上存款更高 | **ATM 费用合作伙伴收入**非常低**；直接存款更高 | 通过 KDDI 固定的混合 — 电信固定，非零售固定 |
| 结算数据形状 | 高SKU广度、多小时购物篮 | 高频、窄篮 | 分布在三方（Lawson、KDDI、三菱公司） |
| 监管边界（许可证+銀行代理業+BaaS） | 银行+预付+保险代理+亚洲消费金融 | 银行+预付+电子支付机构相邻 | 银行 + Ponta 忠诚度运营 + 通过合资企业获得 KDDI 许可 |
| 治理形态 | 单一上市母公司 (AEON Co. 8267) + AFS (8570) 上市子公司 | 单一上市母公司 (Seven & i 3382) + 七银行 (8410) 上市子公司 | **50/50 JV** (KDDI 9433 + Mitsubishi Corp 8058) — Lawson Inc. 退市 2024-Q3 |

## 边界情况/战略分歧

在特定场景下进行压力测试时，这三个模型的聚类情况有所不同。楔形差异在边界处变得最明显。

### 无现金比例大幅上升（2030+场景）

METI 无现金比率目标为 2030  的 65%（2025 — 中的当前基线 58%，请参阅披露包中的 [[payments/INDEX|payments INDEX]]）意味着每家商店的 ATM 取款量较少。

- **AEON**：无现金增长*有帮助* — AEON Pay / AEON 卡 / WAON 都受益于更高的无现金支出。抵押贷款和保险柜台业务在很大程度上与无现金比率无关。
- **七号**：无现金的崛起*首先损害了 ATM 费用线*，但七号银行一直在转向 **B2B 现金支付、身份验证和数字程序**基础设施。问题是这些新线路能否抵消 ATM 取款量下降的影响。
- **Lawson + KDDI**：无现金崛起*有助于au PAY在Lawson POS*，但不能解决Lawson银行的规模劣势问题。 KDDI 的动机是推动 au PAY 的份额，而不管 Lawson 银行如何。

### 主流二维码钱包（PayPay）要求独家 POS 受理

假设：占主导地位的钱包推动独家或首选 POS 放置。

- **AEON**：可能会抵制 — AEON Pay 是内部替代方案，而向 PayPay 的让步会削弱 iAEON ID 图的发挥。
- **Seven**：对多钱包接受更加开放 — Seven 没有大规模的竞争 QR 钱包，因此接受所有主要钱包是现有的默认设置。
- **Lawson + KDDI**：**不能单方面同意**，因为 50/50 JV — KDDI 将抵制 PayPay 独占权（它与 au PAY 竞争），而三菱公司将抵制 Ponta 网络侵蚀。这里的治理僵局是特征，而不是缺陷。

### 对预付发行人的监管压力（未使用余额保障）

如果 FSA 将预付支付工具保障比率收紧至超出当前的 50%。

- **AEON**：在给定 AFS 资产负债表的情况下，通过 WAON 余额 — 进行的直接敞口在操作上是可管理的。
- **七**：鉴于七银行的资产负债表，通过 nanaco 余额 — 的直接风险在操作上是可控的。
- **Lawson + KDDI**：**无直接暴露**，因为 Lawson 不运行专有的 FeliCa 方案。 au PAY 余额风险敞口位于 KDDI / au FH 内部，而不是 Lawson。

### 一家主要银行尝试插入 BaaS 赞助

如果一家大型银行将 BaaS 批发给三个零售集团之一。

- **AEON**：低配 — AEON 银行已涵盖大多数零售银行功能。
- **七**：可能的 — 七银行是合作伙伴银行友好的（它已经批发 ATM 访问 ~600  机构）。存款账户层的 BaaS 合作伙伴关系在结构上是兼容的。
- **Lawson + KDDI**：可以通过 au じぶん銀行（已位于 KDDI 经济区内）或通过 Lawson POS 路由的 [[JapanFG/recruit-mufg-business|Recruit / MUFG business]] 式外部合作伙伴关系实现。

### 创始人/治理继任者

- **永旺**：创始人冈田家族的存在仍然存在，但该集团作为上市企业集团 — 的继承是由公司治理介导的。
- **七**：维权投资者压力（公开报道的 2024-2025）和 2024-2025  分离/私有化讨论使治理形态更加流畅 — 七号和 i 是否仍然是单一上市集团的问题是否会影响七号银行的战略使命留在便利店特许经营范围内。
- **Lawson + KDDI**：**治理是最新颖的变量**，因为50/50 JV对于日本主要零售金融—来说没有先例，KDDI和三菱都没有单方面退出，战略僵局可能引发重组。

### 跨境消费金融扩张

- **AEON**：**已经通过 AFS 的亚洲消费金融子公司 — 泰国、马来西亚、香港、柬埔寨、缅甸实现实质性规模的跨境业务**。盈利组合对东盟监管周期敏感。
- **七**：**跨境ATM业务**（印度尼西亚、菲律宾），但不是大规模的消费金融发起。
- **Lawson + KDDI**：**直接**零售金融跨境足迹可忽略不计； KDDI 移动是国际性的，但零售金融堆栈仅限于国内。

### 转向嵌入式金融/BaaS 赞助

假设：一家非银行金融科技公司希望通过三个零售集团之一批发存款账户产品。

- **AEON**：可能会在存款账户层传递 BaaS 赞助，因为 AEON 银行已经涵盖了内部用例；嵌入式金融机会更有可能通过 [[JapanFG/aeon-financial-service|AFS]]（为合作伙伴发行自有品牌卡）进入**信用卡即服务**层。
- **七号**： **最适合 BaaS 赞助的结构** — 七号银行的合作伙伴银行模式（~600  支付 ATM 使用费的机构）已经批发了基础设施即服务产品；扩展到存款账户 API 层是一个自然的步骤。阻碍因素是产品策略选择，而不是许可。
- **Lawson + KDDI**：BaaS 赞助更有可能通过 KDDI 的 **au じぶん銀行** 途径获得，而不是通过 Lawson 银行本身获得。合资企业结构使独家合作安排变得复杂。

### 对利率正常化的敏感性

日本央行政策利率上升的零利率政策后环境。

- **AEON**：抵押贷款发放利差扩大 — 直接增加 [[JapanFG/aeon-bank|Aeon Bank]] 的收益。亚洲消费金融子公司对日本央行利率不太敏感，但会关注当地市场动态。
- **七**：由于存款/贷款账簿规模较小，净息差风险有限。 ATM 手续费收入对费率不敏感。只有在有意义的利率水平上，nanaco 未用余额的浮动才能成为更物质的收入贡献者。
- **Lawson + KDDI**：Lawson 银行的小账本限制了直接敏感性。 au じぶん銀行 方面（[[JapanFG/au-fh|au FH]] 内）从利率正常化中受益更多。

### 忠诚点负债波动性

如果 FRS / IFRS 15 式的点负债披露进一步收紧 —，请参阅 [[loyalty/point-liability-accounting-boundary|point liability accounting boundary]] 的会计框架。

- **AEON / Seven**：两者都运行封闭的集团内积分（WAON POINT、nanaco 积分），并在分部报告中披露破损假设。更严格的确认将压缩损益确认时间，但不会直接影响运营现金。
- **Lawson + KDDI**：Pontaポイント 责任属于 Loyalty Marketing, Inc.（三菱商事集团）内部。劳森的风险敞口是通过其合作伙伴会员成本**间接**的，而不是作为负债持有人直接承担的。会计边界不同。

## 横截面：应用程序与商店的集成深度

这三个楔子的不同之处在于店内、应用内和银行内表面如何拼接在一起。这是一个结构性产品设计问题，而不仅仅是一个营销问题。

| 图层 | 永旺 | 七与我 | 罗森+KDDI |
|---|---|---|---|
| 统一应用程序 | iAEON + AEON Pay | 7iD + nanaco 应用程序 + 7NOW （没有单个超级应用程序） | Lawson APP + au PAY + Ponta APP（联合） |
| 店内识别 | iAEON 扫码、WAON 点按、AEON 卡刷卡 | 7iD 代码扫描、nanaco 水龙头、七卡刷卡 | Pontaポイント扫描、au PAY扫描、Lawson应用程序代码 |
| 自动柜员机识别 | 永旺银行现金卡、合作银行卡 | 七家银行现金卡、合作银行卡（~600 partners）、海外卡 | 劳森银行现金卡、合作银行卡 |
| 跨产品交叉销售提示 | 高— AEON Pay可提示永旺银行、永旺卡、保险柜台 | 中号 — 7iD 可提示nanaco、七卡、七行 | 低 — 联合 ID 减少了应用内交叉销售摩擦，但表面分为三个应用程序 |
| 收据即参与 | iAEON收据/优惠券返回循环 | 七收据/7iD 优惠券循环 | Lawson收据/Ponta积分+au PAY优惠券 |
| 抵押/保险主管 | 商场金融分行柜台主导 | 有限 — 不是战略面 | 有限 — 电信渠道保险代替 |

应用程序集成深度是为什么“这三个都是以零售为主的金融集团”低估了结构性差异的最明显例子。 AEON 的 **单一应用程序 + 柜台主导** 模式和 Seven 的 **单一 ID + 交易频率主导** 模式与 Lawson 的 **联合应用程序 + 合作伙伴网络主导** 模式有着根本的不同。

## 横截面：商店经济到金融产品映射

每种商店经济形态都会创造一组不同的金融产品机会。该映射不是任意的 —，它是根据停留时间、购物篮大小和访问频率得出的。

| 商店经济维度 | 永旺购物中心 / SM | 7-十一CVS | 劳森CVS |
|---|---|---|---|
| 每次访问停留时间 | 30 分钟 – 3 小时 | 2 – 8 分钟 | 2 – 8 分钟 |
| 平均购物篮尺寸 | ¥3,000 – ¥10,000+ | ¥500 – ¥1,500 | ¥500 – ¥1,500 |
| 每个客户的访问频率 | 1 – 4 次/月 | 8 – 30 次/月 | 8 – 30 次/月 |
| 最适合的金融产品 | 柜台主导抵押贷款、保险、投资信托 | 缴费、ATM取款、预付费充值 | 缴费、ATM取款、扫码充值 |
| 最高杠杆数据 | 跨品类购物篮（食品+服饰+家居） | 日常需求 SKU 速度 | 通过 KDDI 的日常需求 SKU 速度 + 电信/移动交叉数据 |
| 客户 ID 激活时刻 | 商场入口处的应用内优惠券；财务部门静坐 | POS 应用内推送；自动柜员机交易 | POS 应用内推送； au PAY充值；蓬塔扫描 |

商店经济学决定了哪些金融产品与每个楔子“操作上兼容”。柜台主导的抵押贷款发放在结构上与购物中心规模的停留时间兼容，而不是 CVS 频率。大规模 ATM 手续费收入在结构上与 CVS 频率兼容，而不是与商场驻留频率兼容。每个楔子都**擅长不同的金融动作**。

## 三向楔形相互作用矩阵

这三个组并不严格相互排斥，存在重叠区域，其中客户属于多个生态系统。下面的交互矩阵收集了三个楔子如何处理共享/有争议的客户界面。

| 有争议的表面 | 永旺治疗 | 七与一治疗 | 劳森+KDDI治疗 |
|---|---|---|---|
| 柜台接受二维码支付 | 接受 PayPay / d払い / au PAY / 楽天ペイ 以及 AEON Pay | 接受所有主要的二维码钱包 | 接受所有主要 QR 钱包 + au PAY（固定电信） |
| 交通IC验收 | 通用受理（Suica/PASMO/10-IC联盟） | 普遍接受 | 普遍接受 |
| 跨卡授信 | AEON 卡为自有发行机构；接受其他发卡机构的卡 | 七卡是自有发行人；接受其他发卡机构的卡 | au PAY卡被推；接受其他发卡机构的卡 |
| ATM合作银行承兑 | 是 — AEON 银行提供合作银行卡服务 | 是 — 七家银行服务 ~600  合作银行 | 是 — 罗森银行为合作银行提供服务 |
| 通过店内支付 | 在支持 AEON Pay 的自助服务终端和 AEON 银行 ATM 机上 | 在 nanaco / 7iD 层和七家银行 ATM 机 | Loppi + Lawson银行ATM + au PAY应用程序 |
| 保险柜台 | 有 — AEON 商场保险服务柜台 | 否 — 不是战略面 | 否 — 不是战略面 |
| 按揭柜台 | 是 — 永旺银行商城金融分行 | 否 — 不是战略面 | 否 — 不是战略面 |
| 入境旅游卡受理 | 是 — AEON 银行 ATM 接受海外卡 | 是 — 七家银行 ATM 接受海外卡（战略重点） | 是 — Lawson Bank ATM 接受海外卡 |
| B2B 现金支付/退款 | 有限 — 永旺银行服务专线 | 是 — [[JapanFG/seven-payment-service\|Seven Payment Service]] 是战略线 | 有限公司 |
| 认证/数字程序基础设施 | 有限公司 | 战略重点 — 七行支点方向 | 有限公司 |

有争议的表面矩阵表明，**收敛是真实的，但只是部分的**：每个组都接受所有支付方式，但只有 AEON 运行完整的零售银行柜台堆栈，只有七号运行完整的 B2B 现金实用程序堆栈。

## 楔子战略阅读

每个楔子**独特拥有**与**租金**与**没有**的简短散文总结。

- **AEON 拥有**商场和超市集水区、可大规模发放抵押贷款的完整零售银行牌照、封闭式预付电子货币 (WAON)、顶级发行卡品牌 (AEON 卡)、亚洲消费金融足迹。 AEON **租金** 二维码支付占据主导地位（即使 AEON Pay 是店内首选，PayPay 仍然在代码支付总价值中占据主导地位）。 AEON **没有** CVS 密度楔子或七银行规模的 ATM 平台公用事业业务。
- **Seven & i 拥有**最大的 CVS 网络、最大的 CVS 银行 ATM 平台、封闭式预付费电子货币 (nanaco)、7iD 客户 ID 锚、B2B 现金实用业务。七种**租金**机构银行存款关系（它不与大型银行或网络银行正面竞争主要存款）。七**没有**以商场走廊柜台为主导的金融分销面或永旺规模的亚洲消费金融子公司集团。
- **Lawson + KDDI 拥有**第三大 CVS 网络，间接接触 KDDI 用户群（~30M 移动用户），通过 Mitsubishi Corp/Loyalty Marketing 建立 Pontaポイント 多品牌联盟。 Lawson + KDDI **租赁**其专有金融堆栈 — au PAY、au PAY 卡、au じぶん銀行 归 KDDI 所有，并通过 Lawson POS 进行路由，不归 Lawson 所有。 Lawson + KDDI **没有**基于 FeliCa 的专有储值方案、AEON 规模的发卡机构、Aeon 银行规模的全零售银行或七家银行规模的 ATM 平台。

本文表明，这三者最好不要被理解为**同一客户钱包的竞争对手**，而是**不同金融分销楔子的运营商，这些楔子偶尔会在二维码支付接受层和入境游客ATM层进行竞争**。

## 相关

- [[retail/INDEX]]
- [[retail/aeon-group]]
- [[retail/seven-and-i-hd]]
- [[retail/lawson-kddi-retail-finance]]
- [[loyalty/INDEX]]
- [[loyalty/japan-points-landscape]]
- [[loyalty/v-point-smbc-ccc-case]]
- [[loyalty/d-point-au-kddi-docomo-telco-point-consolidation]]
- [[loyalty/point-liability-accounting-boundary]]
- [[banking/japan-net-bank-competition-map]]
- [[banking/japan-baas-operating-models]]
- [[banking/quick-deposit-four-methods]]
- [[payments/INDEX]]
- [[payments/japan-code-payment-competitive-map]]
- [[payments/japan-transit-prepaid-suica-pasmo-icoca-economics]]
- [[payments/waon-nanaco-retail-prepaid-comparison]]
- [[payments/famipay-valucreate-strategy]]
- [[JapanFG/aeon-bank]]
- [[JapanFG/aeon-financial-service]]
- [[JapanFG/seven-bank]]
- [[JapanFG/seven-card-service]]
- [[JapanFG/seven-payment-service]]
- [[JapanFG/lawson-bank]]
- [[JapanFG/au-fh]]
- [[JapanFG/au-payment]]
- [[JapanFG/recruit-mufg-business]]
- [[INDEX|FinWiki index]]

## 来源

- 永旺株式会社企业信息（https://www.aeon.info/en/company/）。
- 永旺金融服务公司简介和综合报告（https://www.aeonfinancial.co.jp/en/corp/about/）。
- 永旺银行官方公司概要（https://www.aeonbank.co.jp/company/about/outline/）。
- WAON官方服务页面（https://www.waon.net/about/）。
- 七安控股官方企业简介（https://www.7andi.com/en/company/profile）。
- Seven & i Holdings IR 金融服务数据 (https://www.7andi.com/en/ir/library/co_financial/2025/finance/)。
- 七家银行官方企业和集团服务页面 (https://www.sevenbank.co.jp/english/corp/, https://www.sevenbank.co.jp/group/)。
- nanaco官方介绍（https://www.nanaco-net.jp/introduction/）。
- Lawson, Inc. 官方公司信息 (https://www.lawson.co.jp/company/)。
- 罗森银行官方IR (https://www.lawsonbank.jp/)。
- KDDI 企业新闻中心 — 2024  与三菱公司 (https://www.kddi.com/corporate/newsrelease/) 联合在 Lawson 上进行 TOB。
- au PAY官方网站（https://aupay.auone.jp/）。
- Pontaポイント 官方网站 (https://www.ponta.jp/)。
- 三菱公司关于 Lawson 投资和 Ponta 业务的新闻稿 (https://www.mitsubishicorp.com/jp/ja/pr/)。
- FSA 银行牌照登记处 (https://www.fsa.go.jp/menkyo/menkyoj/ginkou.xlsx)。
- METI 无现金支付政策门户 (https://www.meti.go.jp/policy/mono_info_service/cashless/)。
