---
source: japanfg/capital-trust
source_hash: aeb430e8a253bfd4
lang: zh
status: machine
fidelity: ok
title: "Capital Trust / キャピタル信託"
translated_at: 2026-05-31T11:13:44.955Z
---

# Capital Trust / キャピタル信託

Wiki route: [[JapanFG/INDEX]] -> [[trust-banks/trust-companies-japan-index]] -> [[trust-banks/capital-trust]]

## 概览

Capital Trust 是一家运营型信托公司。公开资料强调另类资产信托、重组 / 再包装、证券化 / 流动化交易，以及项目融资中的信托和代理功能。在 FinWiki 中，它是企业结构化金融信托路线，区别于 [[trust-banks/asahi-trust]] 等个人信托运营方。

## 实体边界

本页覆盖 キャピタル信託株式会社 的当前法定名称路线。同一登记行具有 DB / Deutsche、Sanne、Apex Group 等阶段的历史名称变更背景；页面应使用当前路线，同时保留旧名称作为别名以避免重复建页。

## 业务角色图

| 路线 | 解读 |
|---|---|
| 另类资产 | 为 feeder fund / 再包装功能提供信托结构。 |
| 证券化 | 与 [[securities/japan-market-infrastructure-map|capital-market infrastructure]] 相邻的企业信托服务。 |
| 项目融资 | 结构化金融中的信托 / 代理功能。 |
| 登记控制 | 当前行记录在 [[trust-banks/trust-companies-japan-index]]。 |

## 公开语境

Capital Trust 可用于区分企业结构化金融信托、信托银行托管以及零售继承类页面。该页面也帮助记录名称变更漂移，因为登记行可能连续存在，即使品牌和所有权已经变化。

## 研究检查清单

1. 检查 2025 年 Capital Services Group 所有权转换后是否出现新的官方披露。
2. 保留旧名称别名，避免生成重复页面。
3. 在添加证券业务表述前，交叉检查 FIEA 或房地产许可邻接关系。

## 相关

- [[trust-banks/trust-companies-japan-index]]
- [[securities/japan-market-infrastructure-map]]
- [[finance/japan-acquisition-finance]]
- [[trust-banks/lombard-odier-trust]]
- [[INDEX|FinWiki index]]

## 来源

- FSA trust company list: https://www.fsa.go.jp/menkyo/menkyoj/sintaku01.xlsx
- Capital Trust official site: https://www.captrustjp.com/
- Capital Trust about page: https://www.captrustjp.com/about
