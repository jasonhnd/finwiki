---
source: payment-firms/kuroneko-atobarai
source_hash: 38cac22717755d2c
lang: en
status: machine
fidelity: ok
title: "Kuroneko Atobarai Payment Service"
translated_at: 2026-05-31T23:59:49.398Z
---
# Kuroneko Atobarai Payment Service

## Wiki route

This entry sits under the [[payment-firms/INDEX|payment-firms INDEX]] as a **logistics-anchored BNPL** product. Read it against the [[payment-firms/bnpl-landscape|Japan BNPL landscape]] system note, the [[payment-firms/net-protections-hd|Net Protections HD]] / [[payment-firms/paidy|Paidy]] BNPL peer set, and the [[payments/japan-bnpl-credit-purchase-boundary|Japan BNPL and credit-purchase boundary]] for the regulatory question. The [[payments/credit-purchase-card-operators-japan-index|METI credit-purchase registry]] is the registration anchor and the [[payments/japan-payment-scheme-economics-matrix|Japan payment scheme economics matrix]] shows where atobarai differs from card / code rails on merchant economics.

## TL;DR

Kuroneko Atobarai Payment Service is an e-commerce deferred-payment service provided by Yamato Credit Finance Co., Ltd., a Yamato HD group company. Because product delivery and invoice enclosure are operated together on the Kuroneko Yamato TA-Q-BIN network, it is treated as a representative logistics-linked BNPL example alongside Net Protections' NP Atobarai. It is an essential comparator for showing the BNPL category of "credit embedded in a logistics network."

## 1. Legal entity / license boundary

| Item | Reading |
|---|---|
| Operating entity | Yamato Credit Finance Co., Ltd. (Yamato HD Group) |
| Product type | E-commerce deferred payment (invoice-enclosed type + convenience-store / bank-transfer / e-money collection) |
| Logistics integration | Delivery + invoice enclosure executed on the Kuroneko Yamato TA-Q-BIN / DM network |

## 2. Business role

- By integrating **logistics + credit + collection agency**, the service gives e-commerce merchants a simplified UX: outsourcing payment functions to the delivery company as well. ^[extracted]

## 3. Why this standalone page matters

An independent page is needed as the **logistics BNPL anchor** directly referenced from [[payment-firms/bnpl-landscape|Japan BNPL landscape]] and [[payment-firms/paidy|Paidy]] / [[payment-firms/atone|atone]] / [[payment-firms/net-protections-hd|Net Protections HD]]. Yamato HD itself is a logistics conglomerate, so it is cleaner to carve out the financial sub-segment separately.

## 4. Regulation / policy

- **Installment Sales Act**: Registered with METI as a credit purchase intermediary. Credit assessment, complaint handling, and links to designated credit-information agencies are key issues.

## Related

- [[payment-firms/bnpl-landscape]]
- [[payment-firms/net-protections-hd]]
- [[payment-firms/atone]]
- [[payment-firms/paidy]]
- [[megabanks/nippon-post]]
- [[card-issuers/jcb]]
- [[payments/japan-bnpl-credit-purchase-boundary]]
- [[payments/credit-purchase-card-operators-japan-index]]
- [[payments/japan-payment-scheme-economics-matrix]]

## Sources

- Yamato Credit Finance "クロネコ代金後払いサービス" product page: https://www.kuronekoyamato.co.jp/ytc/business/payment/atobarai/
- Yamato Credit Finance payment services overview: https://www.kuronekoyamato.co.jp/ytc/business/payment/
- METI registered operators under the Installment Sales Act: https://www.meti.go.jp/policy/economy/consumer/credit/115tourokujigyousyaitiran.html
