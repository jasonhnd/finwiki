...
**Lido stake overlap**: ~100% (Mellow vaults are basically all based on stETH / wstETH / LST sets) · the largest single route from Lido → restaking.
...
| **Mellow** | **LRT vault aggregator** (on EigenLayer+Symbiotic) · socialize slashing | ~$1.8B | Vault-level socialize | Multi-AVS pool basket rewards | Highest (in-vault cascade) | Borrows underlying · no new concentration | Mellow DAO+MLW 2025-Q2 · vault curator sub-gov | Sigma Prime · Spearbit · C4 · joint with Symbiotic | ~100% (basically all LST) | Close to managed fund · avoids US |
