...
status: machine
fidelity: ok
title: "Coinbase AgentKit & Agentic Wallets · MPC agent wallets on Base"
...
This entry sits under [[agent-economy/INDEX|agent-economy index]] as the Coinbase-specific note on agent wallets and agent payments. Read it with [[agent-economy/coinbase-cdp-developer-platform|Coinbase CDP]] for the broader developer-platform substrate, [[agent-economy/x402-http-payment-overview|x402]] for the HTTP payment protocol, [[agent-economy/erc-4337-account-abstraction-primer-for-agents|ERC-4337 account abstraction for agents]] for adjacent wallet-permission design, and [[agent-economy/privy-embedded-wallet-overview|Privy embedded wallets]] for the main embedded-wallet peer.
...
- Coinbase's Agentic Wallet pages position x402 as the core payment standard, state that x402 had processed more than 50M transactions, and emphasize machine-to-machine payments, API paywalls, and programmatic resource access. ^[extracted]
...
- "MPC agent wallets" is therefore best treated as a legacy / category shorthand or Coinbase cryptography background, while the verified Agentic Wallet launch claim is CDP Wallets + TEE isolation + session and transaction guardrails. ^[inferred]
