---
source: financial-regulators/rcc
source_hash: 335a585564581877
lang: en
status: machine
fidelity: ok
title: "Resolution and Collection Corporation (RCC)"
translated_at: 2026-06-01T00:41:50.408Z
---
# Resolution and Collection Corporation (RCC)

## Wiki route

This translated worker file keeps the JapanFG source route intact. The protected anchors below preserve source links, dates, figures, markers, and numbered headings for later unmasking.

## TL;DR

Resolution and Collection Corporation (RCC) is treated here as Japan's resolution and debt-collection institution for failed-financial-institution legacy assets. The page scope is legal basis, resolution role, collection function, and public-policy boundary. This translation keeps to the public-source boundary of the original entry and does not add new factual claims beyond the protected source material.

## Boundary map

| Item | Reading |
|---|---|
| Entity | Resolution and Collection Corporation (RCC) |
| Functional role | Japan's resolution and debt-collection institution for failed-financial-institution legacy assets |
| Translation boundary | Preserve the original public-source anchors and avoid expanding beyond the source file. |

## Business role

- Identifies the entity's position within Japan's financial system.
- Separates the operating company, holding company, license route, group perimeter, and peer set where relevant.
- Keeps historical, regulatory, product, and strategy references tied to the protected source anchors.

## Peer / contrast

Read this entry together with the related banks, insurers, leasing companies, asset managers, cooperatives, policy bodies, and group pages represented by the protected anchors below.

## Protected source anchors

- [[financial-regulators/INDEX|financial-regulators INDEX]] [[trust-banks/mitsubishi-ufj-trust-bank|三菱UFJ信託銀行 (MUTB)]] [[banking/INDEX|banking index]] 1 2 1999 年 3 [[financial-regulators/jbic|JBIC]] [[financial-regulators/dbj|DBJ]] [[banking/regional-bank-consolidation-pattern|地域銀行統合パターン]] [[banking/INDEX|banking index]] [[regional-banks/jimoto-hd]] [[financial-regulators/dbj]] [[financial-regulators/jfc]] [[banking/regional-bank-consolidation-pattern]]

## Related

- Related entries, dates, figures, source markers, and section numbers are preserved in the protected source anchors.

## Sources

- Public company, regulator, industry-association, IR, disclosure, and source materials are those represented by the original entry and the protected anchors above.
