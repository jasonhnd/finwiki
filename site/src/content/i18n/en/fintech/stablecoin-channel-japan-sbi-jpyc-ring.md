...
status: machine
fidelity: ok
title: "SBI × JPYC × Circle Circular Shareholding — Japan Stablecoin Distribution Channel"
translated_at: 2026-05-31T07:28:06.125Z
---

# SBI × JPYC × Circle Circular Shareholding — Japan Stablecoin Distribution Channel
...
- **APLAS connection**: pilot for real-store USDC payments in spring 2026 
...
```
        Circle Internet Financial (USDC issuer)
              │  50%
              │           3.51%
              ├─────────────> JPYC Inc. (funds-transfer EPI)
              │                  │ Circle Partner SC
              ▼                  │
  SBI Circle Holdings ◀──────────┤ Circular mutual reference
              │  50%             │
        SBI Holdings ────────────┘
              │
              ├── SBI VC Trade (EPI #1) → USDC in Japan
              ├── APLAS → real-store payments
              ├── SBI Shinsei Trust × Startale → JPYSC (2026-Q2)
              └── Arc global payments network Japan standalone channel
```

**Channel functions**:
1. **USDC entry point**: SBI VC Trade's exclusive domestic EPI channel
2. **JPY side**: dual JPY stablecoin options of JPYC (funds-transfer type) + JPYSC (trust type)
3. **Cross-border compliance**: bidirectional compliance channel for USDC ↔ JPYC after admission under §501(d)
4. **Retail touchpoint**: APLAS credit cards + physical retail scenarios
...
2025-12-16  SBI HD + Startale announce trust-type SC "JPYSC"
