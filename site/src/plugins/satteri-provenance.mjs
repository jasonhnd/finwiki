...
function visitNode(node, fn) {
  fn(node);
  for (const child of node?.children ?? []) visitNode(child, fn);
}
...
function collectSourcesFromTree(tree) {
...
export default function satteriProvenance() {
  let sources;
  const getSources = (ctx) => {
    sources ??= collectSourcesFromTree(markdownToMdast(ctx.source));
    return sources;
  };

  return {
    name: 'finwiki-provenance',
    link(node, ctx) {
      const label = textContent(node);
      const url = String(node.url ?? '');
      if (!hasMarker(label) && !hasMarker(url) && !/%5E%5B/i.test(url)) return;

      const localSources = getSources(ctx);
      const payloadMatch = label.match(/\^(?:\\)?\[([^\]\r\n]+?)\]/) || url.match(/\^(?:\\)?\[([^\]\r\n]+?)\]/) || url.match(/%5E%5B([^%\]\r\n]+?)(?:%5D|$)/i);
      const payload = payloadMatch?.[1] ? decodeURIComponent(payloadMatch[1]) : '';
      ctx.setProperty(node, 'url', cleanMarkerUrl(url));
      ctx.setProperty(node, 'children', [
        { type: 'text', value: label.replace(/\^(?:\\)?\[[^\]\r\n]+?(?:\\)?\].*$/, '').replace(/[)\uFF09\u3002]+$/, '') },
      ]);

      if (payload) {
        ctx.insertAfter(node, { type: 'html', value: renderMarker(payload, localSources) });
      }
    },
    text(node, ctx) {
      const value = String(node.value ?? '');
      if (!hasMarker(value)) return;
      const out = renderMarkerParts(value, getSources(ctx));
      if (!out) return;
    ...
    html(node, ctx) {
      const value = String(node.value ?? '');
      if (!hasMarker(value)) return;
      ctx.replaceNode(node, { type: 'html', value: renderMarkerHtml(value, getSources(ctx)) });
    },
    code(node, ctx) {
      const value = String(node.value ?? '');
      if (!hasMarker(value)) return;
      ...
      ctx.replaceNode(node, {
        type: 'html',
        value: `<pre class="astro-code prov-code" tabindex="0" data-language="${lang}"><code>${renderMarkerHtml(value, getSources(ctx), { escapeText: true })}</code></pre>`,
      });
    },
  };
}
