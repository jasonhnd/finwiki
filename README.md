...
| 指標 | 現在値 | 集計口径 |
| --- | ---: | --- |
| Markdown files | 1465 | `.git` を除外し、release notes / control docs / templates を含む repository-wide `.md` files |
...
| Text volume | 約993万字 | Markdown 全体の空白除外 UTF-8 文字数（約 9,933,059） |
...
| --- | ---: | --- |
| Markdown files | 1465 | Repository-wide `.md` files excluding `.git`, including release notes, control documents, and templates |
...
| Text volume | ~9.93M chars | ~9,933,059 non-space UTF-8 characters across Markdown |
...
| 指标 | 当前值 | 统计口径 |
| --- | ---: | --- |
| Markdown files | 1465 | 排除 `.git`，包含 release notes、控制文档、模板在内的全仓库 `.md` 文件 |
...
| Text volume | 约993万字 | 全库 Markdown 空白除外 UTF-8 字符数（约 9,933,059） |
