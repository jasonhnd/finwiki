...
| 指標 | 現在値 | 集計口径 |
| --- | ---: | --- |
| Markdown files | 1464 | `.git` を除外し、release notes / control docs / templates を含む repository-wide `.md` files |
...
| Vercel shadow 設定 | repository root の [vercel.json](vercel.json)。Install: `cd site && bun install --frozen-lockfile` / Build: `node tools/vercel_build.mjs` / Output: `_vercel_public` |
| Vercel 出力 | Astro HTML (`site/dist`) と raw Markdown、`llms.txt`、`llms-full.txt`、`ai-index.json`、`sitemap.xml`、`robots.txt`、`api/entries/*.json` を同じ静的出力に組み立て |
...
| --- | ---: | --- |
| Markdown files | 1464 | Repository-wide `.md` files excluding `.git`, including release notes, control documents, and templates |
...
| Vercel shadow config | Root [vercel.json](vercel.json). Install: `cd site && bun install --frozen-lockfile` / Build: `node tools/vercel_build.mjs` / Output: `_vercel_public` |
| Vercel output | Assembles Astro HTML (`site/dist`) together with raw Markdown, `llms.txt`, `llms-full.txt`, `ai-index.json`, `sitemap.xml`, `robots.txt`, and `api/entries/*.json` into one static output |
...
| 指标 | 当前值 | 统计口径 |
| --- | ---: | --- |
| Markdown files | 1464 | 排除 `.git`，包含 release notes、控制文档、模板在内的全仓库 `.md` 文件 |
...
| Vercel shadow 配置 | 仓库根目录 [vercel.json](vercel.json)。Install: `cd site && bun install --frozen-lockfile` / Build: `node tools/vercel_build.mjs` / Output: `_vercel_public` |
| Vercel 输出 | 把 Astro HTML (`site/dist`) 与 raw Markdown、`llms.txt`、`llms-full.txt`、`ai-index.json`、`sitemap.xml`、`robots.txt`、`api/entries/*.json` 组装成同一个静态输出 |
