...
| 指標 | 現在値 | 集計口径 |
| --- | ---: | --- |
| Markdown files | 1439 | `.git` を除外し、release notes / control docs / templates を含む repository-wide `.md` files |
| Topical domains | 23 | `INDEX.md` domain map の主要テーマ領域 |
| Link-audited entries | 1401 | `tools/wiki_link_audit.py` が確認する public wiki entries |
| Unresolved link issues | 0 | body route / peer / system link audit の未解決 issue |
| Text volume | 約1038万字 | Markdown 全体の空白除外 UTF-8 文字数（約 10,384,749） |
| Word-like tokens | 約250万 | English / CJK mixed corpus の近似 token count |
...
| --- | ---: | --- |
| Markdown files | 1439 | Repository-wide `.md` files excluding `.git`, including release notes, control documents, and templates |
| Topical domains | 23 | Major topic areas in the `INDEX.md` domain map |
| Link-audited entries | 1401 | Public wiki entries checked by `tools/wiki_link_audit.py` |
| Unresolved link issues | 0 | Open body route / peer / system-link audit issues |
| Text volume | ~10.38M chars | ~10,384,749 non-space UTF-8 characters across Markdown |
| Word-like tokens | ~2.50M | Approximate English / CJK mixed-corpus token count |
...
| 指标 | 当前值 | 统计口径 |
| --- | ---: | --- |
| Markdown files | 1439 | 排除 `.git`，包含 release notes、控制文档、模板在内的全仓库 `.md` 文件 |
| Topical domains | 23 | `INDEX.md` domain map 中的主要主题领域 |
| Link-audited entries | 1401 | `tools/wiki_link_audit.py` 覆盖的 public wiki entries |
| Unresolved link issues | 0 | body route / peer / system link audit 的未解决 issue |
| Text volume | 约1038万字 | 全库 Markdown 空白除外 UTF-8 字符数（约 10,384,749） |
| Word-like tokens | 约250万 | English / CJK mixed corpus 的近似 token count |
