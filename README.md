...
| 指標 | 現在値 | 集計口径 |
| --- | ---: | --- |
| Markdown files | 1484 | `.git` を除外し、release notes / control docs / templates を含む repository-wide `.md` files |
...
| Text volume | 約1008万字 | Markdown 全体の空白除外 UTF-8 文字数（約 10,079,420） |
...
| --- | ---: | --- |
| Markdown files | 1484 | Repository-wide `.md` files excluding `.git`, including release notes, control documents, and templates |
...
| Text volume | ~10.08M chars | ~10,079,420 non-space UTF-8 characters across Markdown |
...
| 指标 | 当前值 | 统计口径 |
| --- | ---: | --- |
| Markdown files | 1484 | 排除 `.git`，包含 release notes、控制文档、模板在内的全仓库 `.md` 文件 |
...
| Text volume | 约1008万字 | 全库 Markdown 空白除外 UTF-8 字符数（约 10,079,420） |
