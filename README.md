...
| 指標 | 現在値 | 集計口径 |
| --- | ---: | --- |
| Markdown files | 1467 | `.git` を除外し、release notes / control docs / templates を含む repository-wide `.md` files |
...
| Text volume | 約995万字 | Markdown 全体の空白除外 UTF-8 文字数（約 9,951,539） |
...
| --- | ---: | --- |
| Markdown files | 1467 | Repository-wide `.md` files excluding `.git`, including release notes, control documents, and templates |
...
| Text volume | ~9.95M chars | ~9,951,539 non-space UTF-8 characters across Markdown |
...
| 指标 | 当前值 | 统计口径 |
| --- | ---: | --- |
| Markdown files | 1467 | 排除 `.git`，包含 release notes、控制文档、模板在内的全仓库 `.md` 文件 |
...
| Text volume | 约995万字 | 全库 Markdown 空白除外 UTF-8 字符数（约 9,951,539） |
