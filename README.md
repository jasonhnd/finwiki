...
| 指標 | 現在値 | 集計口径 |
| --- | ---: | --- |
| Markdown files | 1455 | `.git` を除外し、release notes / control docs / templates を含む repository-wide `.md` files |
...
| Text volume | 約985万字 | Markdown 全体の空白除外 UTF-8 文字数（約 9,852,328） |
...
> i18n 進捗: 2026-06-01 JST の GPT 翻訳バッチ v2026.06.01-3 後、`site/src/content/i18n` は zh 1013 / en 1013 ファイルです。本輪の `commit-translate` は ok=260, needs_review=0, missing=0 で完了し、占位符検証を通過した訳文だけを保持しています。
...
| --- | ---: | --- |
| Markdown files | 1455 | Repository-wide `.md` files excluding `.git`, including release notes, control documents, and templates |
...
| Text volume | ~9.85M chars | ~9,852,328 non-space UTF-8 characters across Markdown |
...
> i18n progress: after the 2026-06-01 JST GPT translation batch v2026.06.01-3, `site/src/content/i18n` contains 1013 zh files and 1013 en files. This round's `commit-translate` finished with ok=260, needs_review=0, missing=0, and only placeholder-verified translations are retained.
...
| 指标 | 当前值 | 统计口径 |
| --- | ---: | --- |
| Markdown files | 1455 | 排除 `.git`，包含 release notes、控制文档、模板在内的全仓库 `.md` 文件 |
...
| Text volume | 约985万字 | 全库 Markdown 空白除外 UTF-8 字符数（约 9,852,328） |
...
> i18n 进度：2026-06-01 JST 的 GPT 翻译批次 v2026.06.01-3 后，`site/src/content/i18n` 为 zh 1013 / en 1013 文件。本轮 `commit-translate` 结果为 ok=260、needs_review=0、missing=0，仅保留通过占位符校验的译文。
