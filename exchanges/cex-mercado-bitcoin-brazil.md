---
title: Mercado Bitcoin — ブラジル最大 CEX / SoftBank 出資 / RWA tokenization push
aliases:
  - Mercado Bitcoin
  - MB
  - 2TM Group
  - Mercado Bitcoin Brasil
domain: exchanges
slug: cex-mercado-bitcoin-brazil
created: 2026-05-25
last_updated: 2026-05-25
last_tended: 2026-05-25
review_by: 2027-05-25
confidence: likely
tags:
  - exchanges
  - cex
  - brazil
  - brl-spot
  - rwa
  - tokenization
  - softbank
status: active
sources:
  - 公開情報整理 (Mercado Bitcoin / 2TM 公式 IR, mercadobitcoin.com.br / 2tm.io)
  - 公開情報整理 (Banco Central do Brasil / CVM 仮想資産規制声明・通達)
  - 公開情報整理 (Lei 14.478/2022 marco legal de criptoativos 官報資料)
  - 公開情報整理 (SoftBank Latin America Fund 2021-07 $200M 投資ラウンド発表)
  - 公開情報整理 (2X Capital / Tribe Capital / DST Global / Coinbase Ventures 投資ラウンド報道)
  - 公開情報整理 (BCB ATIVOS VIRTUAIS regulação 2024-2025 公開資料)
---

# Mercado Bitcoin — ブラジル最大 CEX / SoftBank 出資 / RWA tokenization push overview

## Wiki route

This entry sits under [[exchanges/INDEX|exchanges index]]. Read it against [[exchanges/cex-foxbit-brazil|Foxbit]] for the closest Brazil peer / contrast context, [[exchanges/global-cex-top10-comparison|global CEX top 10 comparison]] for the broader global benchmark, and [[exchanges/rwa-tokenization-cex-integration|RWA tokenization × CEX integration]] for the RWA-on-CEX vertical.

> 2013-07 設立 · ブラジル最大手 CEX · 2TM Group 持株 · 2021-07 SoftBank Latin America Fund リード $200M (Unicorn 化) · BCB / CVM 規制下 · RWA tokenization で南米先導

## 1. 法人・株主

- 商号（事業会社）：**Mercado Bitcoin Serviços Digitais Ltda.**
- 持株会社：**2TM Participações S.A.**（2TM Group）
- 設立：2013-07（São Paulo, Brazil）
- 創業者：Gustavo Chamati, Maurício Chamati, Mauro Andrade
- 主要株主・資金調達ラウンド：
  - **SoftBank Latin America Fund**：2021-07 リード Series B $200M、評価額 **$2.1B（南米暗号通貨業界初の Unicorn）**
  - **2X Capital**：南米暗号通貨専門 VC、複数ラウンド参加
  - **Tribe Capital** — Series B
  - **DST Global**（Yuri Milner 系）— Series B
  - **Coinbase Ventures** — 戦略出資
  - **Parallel Ventures** — 早期投資
  - **GP Investments**：ブラジル現地 PE 出資
- 子会社・グループ会社：
  - **Mercado Bitcoin Bank**（payment processor 関連）
  - **MB Tokens**（tokenization 子会社）
  - **Portal**（Web3 wallet / infrastructure）
- IPO 検討：2021–2022 期に米国・ブラジル両市場での上場検討報道、暗号市場下落で保留

## 2. 登録・牌照状況（ブラジル制度）

- **Banco Central do Brasil (BCB) 認可**：**Lei 14.478/2022（marco legal de criptoativos、暗号資産法、2022-12 公布、2023-06 施行）** 下で BCB が VASP（**Prestadores de Serviços de Ativos Virtuais, PSAV**）の認可・監督機関に指定
  - 2024–2025 年期：BCB の本格的な PSAV 認可制度運用開始、Mercado Bitcoin も登録プロセス対象
- **CVM（Comissão de Valores Mobiliários、ブラジル証券委員会）**：トークン化証券（tokenized securities）監督機関、**MB Tokens** の RWA tokenization は CVM 規制下
- **Receita Federal**：暗号通貨課税 / 取引報告義務（**月次取引 BRL 30,000 超は申告義務**）
- **AML/CFT**：COAF（Conselho de Controle de Atividades Financeiras、ブラジル版 FIU）への報告義務

## 3. 取扱通貨

- BRL spot 市場（ブラジル最大の流動性）
- 取扱銘柄数：250+（BTC / ETH / SOL / XRP 等主要銘柄 + ブラジル系プロジェクト）
- USDT / USDC market 併設
- **MB Tokens（RWA tokenization）**：**precatórios（裁判所判決債権）**、不動産持分、農業 receivable 等のトークン化商品を販売（ブラジル独自の RWA ユースケース）

## 4. 業務範囲

- 現物 spot 取引（BRL pair 中心、グローバル最大の BRL 流動性）
- 派生：先物・margin 取引は限定的、規制環境を慎重に観察
- staking：限定的（カスタディアル staking）
- **RWA tokenization**：MB Tokens 経由で precatórios / 不動産 / 農業 receivable / 中小企業債権 / 太陽光発電 receivable 等のトークン化商品提供（**ブラジルでは Lei 14.478/2022 + CVM ICVM 88 でユーティリティトークンと証券トークンの線引き運用**）
- 法定通貨入金：**PIX**（ブラジル中央銀行即時決済システム）統合、TED / DOC 経由
- 機関向け：MB Institutional、OTC desk

## 5. 市占・取引高

- **ブラジル spot 市場シェア**：圧倒的首位（推定 60–70%）
- 24h volume：$50–500M（BRL 流動性換算）
- ユーザー数：400 万人超（公式公表ベース）
- グローバル CEX ランキング：top 50 圏内（南米地域では断然 #1）

## 6. 沿革・重大事件

| 時期 | 事項 |
|------|------|
| 2013-07 | Mercado Bitcoin 設立（ブラジル初期の本格 CEX として営業開始） |
| 2017 | ブラジル仮想通貨ブーム期に最大手として地位確立 |
| 2020 | 2TM Group 持株構造完成 |
| 2021-01 | 2X Capital リード Series A |
| **2021-07** | **SoftBank Latin America Fund リード Series B $200M、評価額 $2.1B（南米暗号通貨業界初 Unicorn）** |
| 2021-09 | Mercado Bitcoin Bank（payment processor）買収 |
| 2022 | 暗号市場下落、IPO 計画保留 |
| 2022-12 | **Lei 14.478/2022（marco legal de criptoativos）公布**、ブラジル暗号資産法成立 |
| 2023-06 | Lei 14.478/2022 施行、BCB が PSAV 監督機関に指定 |
| 2023–2024 | MB Tokens 経由で **precatórios tokenization** 商品展開、南米 RWA 市場の先導者 |
| 2024 | BCB PSAV 認可制度本格運用開始、Mercado Bitcoin 登録対象 |
| 2024–2026 | RWA tokenization 商品ライン拡大、機関投資家向け展開 |

## 7. 戦略動向

- **RWA tokenization 戦略**：ブラジル法制度の柔軟性（特に precatórios / 農業 receivable 等の現地特有 RWA）を活用し、南米地域 RWA tokenization の先導者ポジション確立。グローバル RWA プレイヤー（[[exchanges/rwa-tokenization-cex-integration|RWA × CEX integration]] 参照）と比較しても **南米特有 RWA に特化** した独自ポジション
- **PIX 統合**：ブラジル中央銀行即時決済システム PIX のフル統合により、24/7 入出金 UX で他 CEX を圧倒
- **BCB / CVM 規制対応**：Lei 14.478/2022 施行後の PSAV 登録 + CVM トークン化証券規制への二重対応で、規制対応コスト負担増だが先行者利益
- **SoftBank との関係**：SoftBank Latin America Fund の継続的サポート、Latin America Fund のポートフォリオ企業との連携機会
- **南米地域展開**：アルゼンチン / コロンビア / メキシコ等への展開検討、ただしブラジル本体への集中継続
- **Stablecoin 戦略**：BRL ステーブルコイン構想 / USDT / USDC の現地流動性提供

## 8. 業界での位置付け

Mercado Bitcoin は **南米暗号通貨業界の旗艦企業** として、ブラジル現地規制（Lei 14.478/2022 + CVM tokenized securities）への対応 + RWA tokenization の先行展開 + SoftBank 等のグローバル機関投資家サポートの 3 要素で南米 CEX 業界をリードする。

[[exchanges/global-cex-top10-comparison|グローバル top 10]] には届かないが、**南米地域では事実上の唯一の準グローバル CEX**。比較対象として [[exchanges/cex-foxbit-brazil|Foxbit]]（ブラジル #2、real-pair 取引の長期歴史）はあるが、規模・ファンディング・規制対応で Mercado Bitcoin が大きくリードしている。

## Related

- [[exchanges/cex-foxbit-brazil]] — ブラジル #2（最重要 cross-link）
- [[exchanges/global-cex-top10-comparison]] — グローバル CEX top 10
- [[exchanges/rwa-tokenization-cex-integration]] — RWA tokenization × CEX 統合
- [[exchanges/global-vasp-regulatory-comparison-matrix]] — グローバル VASP 規制比較
- [[exchanges/INDEX]] — 取引所ドメインインデックス
- [[exchanges/cex-coindcx-india]] — 新興国 CEX 比較対照
- [[exchanges/cex-bithumb-korea]] — アジア地域 CEX 比較対照

## Sources

- 公開情報整理 (Mercado Bitcoin / 2TM 公式 IR, mercadobitcoin.com.br / 2tm.io)
- 公開情報整理 (Banco Central do Brasil / CVM 仮想資産規制声明・通達)
- 公開情報整理 (Lei 14.478/2022 marco legal de criptoativos 官報資料)
- 公開情報整理 (SoftBank Latin America Fund 2021-07 $200M 投資ラウンド発表)
- 公開情報整理 (2X Capital / Tribe Capital / DST Global / Coinbase Ventures 投資ラウンド報道)
- 公開情報整理 (BCB ATIVOS VIRTUAIS regulação 2024-2025 公開資料)
- 公開情報整理 (MB Tokens precatórios tokenization 商品展開報道, Valor Econômico / Folha de S.Paulo / CoinDesk Brasil)
- 公開情報整理 (PIX 統合関連 Banco Central do Brasil 公開資料)
