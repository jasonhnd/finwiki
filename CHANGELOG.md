...
- **範囲**: root `INDEX.md` の冒頭 snapshot と Domain Map 13 行、`docs/roadmap.md` / `docs/backlog.md` の進捗同期、三語 README/CHANGELOG/`releases/v2026.06.05-4.md`、release-generated discovery surface。wiki body facts、entity page、site config、領域数、entry 数は不変。
...
- **範囲**: `tools/wiki_link_audit.ts`、`tools/release.ts`、`tools/release.helpers.ts`、`SCHEMA.md`、`docs/entry-authoring.md` / `decisions.md`(ADR-007) / `roadmap.md` / `backlog.md`、再生成された discovery surface、counts sync された `README.md` / `index.html`、`CHANGELOG.md` / `releases/v2026.06.04-3.md`。
...
- **次の作業**: push 後に GitHub Actions / remote HEAD / GitHub Release を確認。開発の次手（`docs/roadmap.md`）: P1 残 = `canonical_anchor` を mirror page の hard requirement に昇格する是非評価（audit を gate 化）、P4 = v12 新 entry の i18n、P3 = `non-profit` / `trade` 続扩。
...
- **既知の注意点**: canonical_anchor Phase 2 の残: discovery 出力への entity edge（`ai-index.json` / `llms-full.txt` に mirror→anchor 関係を暴露）と hard requirement 化は後続（`docs/backlog.md` 参照）。
...
- **背景**: `docs/roadmap.md` の P2（JapanFG 深化）/ P3（薄領域の内容拡充）に基づき、ユーザー要請で 10 領域へ並列 subagent で公開情報の金融 entry を一括追加した。docs/ の進度・規劃文書（v11）が「次に何をするか」を定義したので、それを実際の開発で消化する初回。
...
- **背景**: v2026.06.03-10 で docs/ 開発文書システム（参考型 5 件）を新設した。本 release では (1) それを進度・規劃型文書で拡充し docs を開発進度の支柱として機能させ、(2) `docs/roadmap.md` で定めた P1 = canonical_anchor Phase 2 の最初の一手を実行する。
