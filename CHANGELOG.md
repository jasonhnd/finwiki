...
#### 日本語記録 / English Record / 中文记录
- **JST 時刻**: 2026-06-21 12:53 JST。
- **背景**: GitHub Issue #23 は human-site entry reading experience の `needs-spec` 状態を解消し、承認済み UI baseline を保ったまま metadata hierarchy、prose rhythm、table containment、TOC readability、mobile overflow を改善することを求めていた。Issue #32 / #33 / #34 はそれぞれ Japanese-finance coverage gap-map、AI surface consumability、provenance / source-completeness audit の planning artifact と downstream build task packet を求めていた。
- **範囲**: #23 では `site/src/layouts/EntryLayout.astro`, `site/src/layouts/Base.astro`, `site/src/styles/global.css` を変更し、entry page の reading measure、factbar chip wrapping、source / table readability、desktop rail / inline TOC behavior、390px / 320px mobile header containment を改善した。#32 / #33 / #34 は `docs/02-product/japanese-finance-coverage-gap-map.md`, `docs/05-functional-specs/ai-surface-consumability-design.md`, `docs/07-quality/provenance-source-completeness-audit.md` と docs index / roadmap / backlog を同期した。wiki corpus entry、i18n mirror body、route、API schema、AI discovery contract は #23 実装では変更しない。
- **主要ファイル**: `site/src/layouts/Base.astro`, `site/src/layouts/EntryLayout.astro`, `site/src/styles/global.css`, `docs/05-functional-specs/human-site-reading-experience-pass.md`, `docs/02-product/japanese-finance-coverage-gap-map.md`, `docs/05-functional-specs/ai-surface-consumability-design.md`, `docs/07-quality/provenance-source-completeness-audit.md`, `docs/01-strategy/backlog.md`, `docs/01-strategy/roadmap.md`, `README.md`, `CHANGELOG.md`, `releases/v2026.06.21-4.md`。
- **実行手順**: #23 の spec を確認し、EntryLayout の central column を `var(--w-reading)` に寄せ、long title / tag / TOC links を wrap 可能にした。`.prose` は paragraph / heading rhythm、blockquote spacing、long link wrapping、table block overflow、source section density を調整した。mobile header は narrow viewport で language switch と Pagefind trigger を隠し、brand / primary nav / theme button を viewport 内に保持するよう Base layout の flex behavior を明示した。#32 は FSA / BOJ / JSDA / JPX / LIAJ / IMAJ / leasing association / NTA など公開 registry surface を coverage comparison source として整理した。#33 は static-first API enrichment と MCP deferral、#34 は claim-signal / marker-proximity / report-shape / false-positive control を設計した。
- **検証結果**: `cd site && bun run build` は PASS、4435 pages built。Chrome DevTools Protocol による #23 mobile layout 計測は 390px で `innerWidth=390`, `docScrollWidth=375`, `bodyScrollWidth=375`, theme button `x=331..363`、320px で `innerWidth=320`, `docScrollWidth=305`, `bodyScrollWidth=305`, theme button `x=261..293`。`bun tools/release.ts --write` は `markdown_files=1570`, `public_pages=1569`, `sitemap_urls=1570`, `domains=40`, `link_audited_entries=1483`, `api_entries=1476` を生成した。`bun run docs:audit` PASS、`bun tools/release.ts --check --strict` PASS、`bun run surface:drift` PASS、`bun run ai:audit` PASS（`llms.txt` 52 links, `llms-full.txt` 1569 links, `llms-tasks.txt` 160 links, broken 0）、`bun run html:check` PASS（`checked=4435`, `duplicate_ids=0`）、`git diff --check` EXIT=0。
- **残タスク**: 現 branch の push / PR update / issue closeout evidence は publish 指示後に行う。#28 / #29 / #30 の follow-up implementation issue、#24 INDEX-count gate、#32 / #33 / #34 の build issue は別 issue として継続する。
- **EN**: Issue #23 is implemented as a conservative human-site reading pass. Entry pages keep the approved reference layout while improving article measure, title and metadata wrapping, prose rhythm, table containment, source-section density, TOC readability, and mobile header containment. Issues #32 / #33 / #34 add planning artifacts for Japanese-finance coverage gaps, AI-surface consumability, and provenance/source-completeness auditing, each with a downstream build packet. Corpus entries, i18n bodies, public routes, API schema, and AI discovery contracts are not changed by #23.
- **中文**: Issue #23 已按保守阅读体验方案实现：保留既有参考型布局，同时改善正文宽度、标题和 metadata 换行、正文节奏、表格 containment、source section 密度、TOC 可读性和移动端 header containment。#32 / #33 / #34 分别补齐 Japanese-finance coverage gap、AI surface consumability、provenance/source-completeness audit 的规划文档与后续 build packet。#23 不改 corpus 正文、i18n 正文、公开 route、API schema 或 AI discovery contract。

### Issue #26 - Wave D entity deepening / insurance, trust and manufacturer anchors / 保险、信托与制造业锚点实体页深化

#### 日本語記録 / English Record / 中文记录
- **JST 時刻**: 2026-06-21 04:10 JST。
- **背景**: GitHub Issue #26 は #22 の entity-deepening task packet を実行し、thin registry-like entity pages に business model、regulatory positioning、group / parent relationship を追加することを求めている。Wave C の securities / asset-management / leasing pass が完了したため、本作業では Wave D の insurance / trust / manufacturer anchors 4 ページを対象にした。
- **範囲**: `life-insurers/lifenet.md`, `life-insurers/orix-life.md`, `trust-banks/alterna-trust.md`, `manufacturer-finance/seiko-epson.md` を深化し、ja / en / zh の i18n mirror 12 件、`README.md`, `CHANGELOG.md`, `releases/v2026.06.21-3.md`, root `index.html`, AI discovery surface、API entry JSON を同期対象にした。
- **主要ファイル**: `life-insurers/`, `trust-banks/`, `manufacturer-finance/`, `site/src/content/i18n/{ja,en,zh}/life-insurers/`, `site/src/content/i18n/{ja,en,zh}/trust-banks/`, `site/src/content/i18n/{ja,en,zh}/manufacturer-finance/`, `README.md`, `CHANGELOG.md`, `releases/v2026.06.21-3.md`, `index.html`, `sitemap.xml`, `llms.txt`, `llms-full.txt`, `ai-index.json`, `api/entries/index.json`。
- **実行手順**: LifeNet official IR archive / shareholder information / mid-term plan、ORIX Group insurance page / integrated report、FSA trust-company list / Mitsui & Co. Digital Asset Management service page / SMTB release、Epson official company / business / segment / financial results pages を確認した。LifeNet には東証 Prime / auFH 18.32% / GCL / IFRS17 指標、ORIX Life には group insurance engine と hybrid channel、Alterna Trust には trust-company licence boundary と tokenized-asset trustee role、Seiko Epson には manufacturer-finance anchor と working-capital / capex / FX / supply-chain use cases を追加した。source body の SHA-256 short hash を再計算し、12 mirror の `source_hash` と翻訳本文を更新した。
- **検証結果**: `bun run i18n:status --json` は ja / zh / en すべて `current=1436`, `stale=0`, `missing=0`, `orphaned=0`。`bun tools/release.ts --write` は link audit `issues=0`, `canonical_drift=0`、`markdown_files=1569`, `public_pages=1568`, `sitemap_urls=1569`, `domains=40`, `link_audited_entries=1483`, `api_entries=1476` を生成し、README / root `index.html` / AI discovery surface を同期した。`bun tools/release.ts --check --strict` は counts in sync、JSON / LF / duplicate-id verify OK。`bun tools/wiki_link_audit.ts --fail-on-issues` は `entries_checked=1483`, `entries_with_issues=0`, `dead_wikilink_references=0`, `dead_wikilink_targets=0`, `canonical_anchor_drift=0`。`bun run surface:drift` は API aligned 1476 entries / docs leakage 0。`bun run ai:audit` は `llms.txt` 52 links, `llms-full.txt` 1568 links, `llms-tasks.txt` 160 links すべて broken 0。`git diff --check` は EXIT=0。`cd site && bun run build` は EXIT=0 で 4435 pages built。`bun run html:check` は `checked=4435`, `duplicate_id_pages=0`, `duplicate_ids=0`。
- **残タスク**: #26 は Wave A-D の実装完了後に issue closeout evidence をまとめる。#23 は design approval gate のため UI 実装前に方案確認が必要。origin/main への push、GitHub Release 公開、issue close は publish 指示がある場合のみ行う。
- **EN**: Issue #26 executes the downstream content work from #22. Wave D deepened four existing insurance / trust / manufacturer-anchor entity pages without changing URLs: LifeNet Insurance, ORIX Life Insurance, Alterna Trust, and Seiko Epson. The pass adds public-source context on listed direct-life metrics, ORIX group insurance economics, tokenized-asset trust-company boundaries, and manufacturer-finance balance-sheet use cases, then synchronizes the 12 ja / en / zh i18n mirrors plus release surfaces.
- **中文**: Issue #26 是 #22 的下游内容执行。本次 Wave D 在不改变 URL 的前提下深化 4 个保险 / 信托 / 制造业锚点实体页：LifeNet Insurance、ORIX Life Insurance、Alterna Trust、Seiko Epson。正文补充公开来源可核验的 listed direct-life metrics、ORIX group insurance economics、tokenized-asset trust-company boundary、manufacturer-finance balance-sheet use cases，并同步 12 个 ja / en / zh i18n mirror 及发布面。

### Issue #26 - Wave C entity deepening / securities, asset management and leasing / 证券、资管与租赁实体页深化

#### 日本語記録 / English Record / 中文记录
- **JST 時刻**: 2026-06-21 03:30 JST。
- **背景**: GitHub Issue #26 は #22 の entity-deepening task packet を実行し、thin registry-like entity pages に business model、regulatory positioning、group / parent relationship を追加することを求めている。Wave B の bank / foreign-branch pass が完了したため、本作業では Wave C の securities / asset-management / leasing 4 ページを対象にした。
- **範囲**: `securities-firms/matsui-sec.md`, `securities-firms/aizawa-sg.md`, `asset-managers/tokio-marine-asset-management.md`, `leasing-firms/mizuho-lease.md` を深化し、ja / en / zh の i18n mirror 12 件、`README.md`, `CHANGELOG.md`, `releases/v2026.06.21-2.md`, root `index.html`, AI discovery surface、API entry JSON を同期対象にした。
- **主要ファイル**: `securities-firms/`, `asset-managers/`, `leasing-firms/`, `site/src/content/i18n/{ja,en,zh}/securities-firms/`, `site/src/content/i18n/{ja,en,zh}/asset-managers/`, `site/src/content/i18n/{ja,en,zh}/leasing-firms/`, `README.md`, `CHANGELOG.md`, `releases/v2026.06.21-2.md`, `index.html`, `sitemap.xml`, `llms.txt`, `llms-full.txt`, `ai-index.json`, `api/entries/index.json`。
- **実行手順**: Matsui Securities official profile / Important Information Sheet / securities report / integrated report、Aizawa Securities Group official profile / history / medium-term plan / Aizawa Securities company profile、Tokio Marine Asset Management company profile / English site / Tokio Marine HD group-company page、Mizuho Leasing company / shareholder / group / international business / integrated report / Marubeni alliance release を確認した。松井証券には online brokerage economics と FIEA boundary、アイザワ証券グループには公式ページに基づく PRIME / founding / establishment correction と Asia-stock retail brokerage、TMAM には insurer-owned AM / AUM / mandate channel、Mizuho Leasing には Mizuho / Marubeni satellite と asset-finance lanes を追加した。source body の SHA-256 short hash を再計算し、12 mirror の `source_hash` と翻訳本文を更新した。
- **検証結果**: `bun run i18n:status` は ja / zh / en すべて `current=1436`, `stale=0`, `missing=0`, `orphaned=0`。`git diff --check` は EXIT=0。`bun tools/wiki_link_audit.ts --fail-on-issues` は `entries_checked=1483`, `entries_with_issues=0`, `dead_wikilink_references=0`, `dead_wikilink_targets=0`, `canonical_anchor_drift=0`。`bun tools/release.ts --write` は link audit `issues=0`, `canonical_drift=0`、`markdown_files=1568`, `public_pages=1567`, `sitemap_urls=1568`, `domains=40`, `link_audited_entries=1483`, `api_entries=1476` を生成し、README / root `index.html` / AI discovery surface を同期した。`bun tools/release.ts --check --strict` は counts in sync、JSON / LF / duplicate-id verify OK。`bun run surface:drift` は API aligned 1476 entries / docs leakage 0。`bun run ai:audit` は `llms.txt` 52 links, `llms-full.txt` 1567 links, `llms-tasks.txt` 160 links すべて broken 0。
- **残タスク**: #26 の Wave D（insurance / trust / manufacturer anchors）は未実施。#23 は design approval gate のため UI 実装前に方案確認が必要。origin/main への push、GitHub Release 公開、issue close は publish 指示がある場合のみ行う。
- **EN**: Issue #26 executes the downstream content work from #22. Wave C deepened four existing securities / asset-management / leasing entity pages without changing URLs: Matsui Securities, Aizawa Securities Group, Tokio Marine Asset Management, and Mizuho Leasing. The pass adds public-source context on online-brokerage economics, FIEA / Type-I boundaries, insurer-owned asset-management channels, and Mizuho / Marubeni asset-finance lanes, then synchronizes the 12 ja / en / zh i18n mirrors plus release surfaces.
- **中文**: Issue #26 是 #22 的下游内容执行。本次 Wave C 在不改变 URL 的前提下深化 4 个证券 / 资管 / 租赁实体页：Matsui Securities、Aizawa Securities Group、Tokio Marine Asset Management、Mizuho Leasing。正文补充公开来源可核验的 online-brokerage economics、FIEA / Type-I 边界、保险集团持有的资管渠道、Mizuho / Marubeni asset-finance lanes，并同步 12 个 ja / en / zh i18n mirror 及发布面。
...
#### 日本語記録 / English Record / 中文记录
- **JST 時刻**: 2026-06-20 15:52 JST。
- **背景**: GitHub Issue #27 は、human / agent contributor が clone 後に issue-driven workflow、公開情報ルール、validation gates、i18n mirror pipeline を逆引きせず把握できる root `CONTRIBUTING.md` の追加を求めていた。既存の `docs/` 体系は詳細だが、contributor-facing の単一入口がなかった。
- **範囲**: 新規 `CONTRIBUTING.md`、`README.md` の三語入口表へのリンク追加、`CHANGELOG.md` の本記録、root contributor guide を control doc として扱うための `lib/markdown_helpers.ts` / audit tool control-doc list 更新。新しい root Markdown により repository-wide Markdown count が 1564 → 1565 へ変わったため、`release.ts --write` で root `index.html` と AI discovery surface (`llms.txt`, `llms-full.txt`, `ai-index.json`, `sitemap.xml`, `api/entries/index.json`) も同期した。追加検証で `llms-tasks.txt` の退役 route 1 件を検出したため、現行 canonical route へ修正した。wiki corpus entry、domain entry、release note、GitHub Release は変更しない。
- **主要ファイル**: `CONTRIBUTING.md`, `README.md`, `CHANGELOG.md`, `lib/markdown_helpers.ts`, `tools/wiki_link_audit.ts`, `tools/docs_link_audit.ts`, `index.html`, `llms.txt`, `llms-full.txt`, `llms-tasks.txt`, `ai-index.json`, `sitemap.xml`, `api/entries/index.json`。
- **実行手順**: Issue #27 の scope / acceptance criteria を確認し、`AGENTS.md`、`docs/06-implementation/model-agent-workflow.md`、`docs/06-implementation/entry-authoring.md`、`docs/00-governance/documentation-system.md`、`.github/ISSUE_TEMPLATE/**`、public information / toolchain / deployment docs を読み、既存ルールを重複 policy としてではなくリンク付き contributor checklist に統合した。README の日本語・英語・中国語入口表へ `CONTRIBUTING.md` を追加した。初回 `release.ts --check --strict` で新 root Markdown が wiki entry と誤分類されることを確認したため、`CONTRIBUTING.md` を既存の root control docs と同じ分類・監査対象へ追加した。count drift を解消するため `bun tools/release.ts --write` を実行し、generated surface を現在 snapshot へ同期した。`ai:audit` で検出した `llms-tasks.txt` の `corporate-strategy/partial-spinoff-tax-deferral` route を、#8 後の canonical `corporate-strategy/japan-kabushiki-bunpai-spinoff-regime` へ修正した。
- **検証予定 / 結果**: `bun run docs:audit`、`git diff --check`、`bun tools/release.ts --check --strict`、`bun tools/wiki_link_audit.ts --report wiki-link-improvement-plan.md --fail-on-issues` を実行する。`release.ts --write` は `entries=1483`, `issues=0`, `canonical_drift=0`, `markdown_files=1565`, `public_pages=1564`, `api_entries=1476` で完了した。
- **残タスク**: 検証結果を確認し、commit に Issue #27 を紐付ける。`origin/main` への push と GitHub Release 更新は、明示的な publish 指示がある場合のみ行う。
- **EN**: Issue #27 asked for a root contributor guide so a human or agent can understand the issue-driven workflow, public-source boundary, validation gates, and i18n mirror expectations without reverse-engineering the internal docs tree. The guide links existing authoritative docs instead of creating new policy. The first strict release check showed the new root Markdown would otherwise be treated as a wiki entry, so `CONTRIBUTING.md` was added to the same control-doc classification used by README / AGENTS / SCHEMA and covered by docs-link audit. Because the repository-wide Markdown count changed from 1564 to 1565, `release.ts --write` refreshed root `index.html` and the AI discovery surface counts. Additional `ai:audit` validation found one stale retired route in `llms-tasks.txt`, now corrected to `corporate-strategy/japan-kabushiki-bunpai-spinoff-regime`. Wiki corpus entries, domain indexes, release notes, and GitHub Release are unchanged.
- **中文**: Issue #27 要求新增根目录贡献者指南，让 human / agent contributor 不必反向阅读全部内部文档，就能找到 issue 驱动流程、公开来源边界、验证 gate 与 i18n mirror 要求。该指南只综合并链接既有权威文档，不新增政策。第一次严格 release check 显示新根目录 Markdown 会被误当成 wiki entry，因此把 `CONTRIBUTING.md` 加入与 README / AGENTS / SCHEMA 相同的 control-doc 分类，并纳入 docs link audit。由于全仓 Markdown 数从 1564 变为 1565，已用 `release.ts --write` 刷新 root `index.html` 与 AI discovery surface 的 count。额外 `ai:audit` 验证发现 `llms-tasks.txt` 中有 1 个退役 route，已改为 `corporate-strategy/japan-kabushiki-bunpai-spinoff-regime`。wiki corpus entry、领域索引、release notes 和 GitHub Release 不变。

### i18n stale → ZERO — final benign-drift clearance + pre-rename pointer fix / Clear all stale mirrors to zero / 把所有过期镜像清到零

#### 日本語記録 / English / 中文

- **JST 時刻**: 2026-06-19 22:00 JST。
- **背景**: メンテナによる方針決定(全量 opus)に従い、`i18n:status` の残 stale を **完全にゼロ**化。Issue #21 と直近 2 波(benign-drift clearance wave 1/2)の後に残っていた **764 の link_only mirror**(ja 205 / zh 327 / en 232)は、すべて mask トークナイザの空白敏感性に起因する **良性の数値/日付整形ドリフト**(prose は不変、レンダリング正常、リンク健全)。加えて **9 mirror**(manufacturer-finance 6 + non-profit 3)が v2026.06.03-9 のドメイン改名(`manufacturing→manufacturer-finance` / `governance→non-profit`)前の `source:` ポインタを保持したままで orphaned 扱い・manifest 圏外だった。
- **作業**: (1) 764 link_only mirror を **対話内 opus subagent**(mask→opus→verify→unmask→commit)で 3 波(192+200+残り source)再翻訳。(2) 9 source-pointer-drift mirror の frontmatter `source:` を path 一致(現ドメイン)へ確定的に訂正→ link_only 再分類→ 27 mirror(9×3)再翻訳。合計 **791 mirror**(764+27)。
- **範囲**: `site/src/content/i18n/**` の 791 mirror、`CHANGELOG.md`。corpus 本文・生成サーフェスは不変(site/ は canonical counts 対象外)。
- **検証**: `commit-translate` ok 合計=791 / needs_review=0(途中 wave2 で 4 件の placeholder 不整合を検出→該当 mirror 削除→再翻訳で回収)、占位符 leak **0**、`i18n:status` **stale ja/zh/en = 0 / 0 / 0**(current=1436・fidelity ok=1436・orphaned=0・missing=0・source-pointer drift=0)、`wiki_link_audit` entries=1483 issues=0 dead=0 canonical_drift=0、`release --check --strict` PASS(counts in sync)。
- **EN**: Per the maintainer's decision (full-opus), drove `i18n:status` stale to **absolute zero**. The 764 link_only mirrors left after Issue #21 and the two benign-drift waves were all the benign numeric/date formatting drift (mask-tokenizer whitespace artifacts; prose unchanged, renders correctly, links resolve). A further 9 mirrors (6 manufacturer-finance + 3 non-profit) still carried pre-rename `source:` pointers (`manufacturing/`, `governance/`) from the v2026.06.03-9 domain renames, leaving them orphaned and outside the manifest. Work: (1) retranslated the 764 link_only mirrors via in-conversation opus subagents (mask→opus→verify→unmask→commit) in 3 waves; (2) deterministically corrected the 9 mirrors' `source:` frontmatter to match their path (current domain), reclassified as link_only, retranslated the 27 mirror-langs. Total 791 mirrors. Verified: `commit-translate` ok=791/needs_review=0 (4 mid-run placeholder mismatches caught → deleted → re-translated); zero placeholder leaks; `i18n:status` stale 0/0/0 (current=1436, fidelity ok, orphaned=0, missing=0, no source-pointer drift); `wiki_link_audit` entries=1483 issues=0 dead=0 canonical_drift=0; `release --check --strict` PASS. This supersedes the prior "residual stale is a benign by-design non-goal" stance — the residual is now fully cleared.
- **中文**: 按维护者决定(全量 opus),把 `i18n:status` 的残余 stale **彻底清零**。Issue #21 与最近两波(benign-drift wave 1/2)之后剩的 **764 个 link_only 镜像**(ja 205 / zh 327 / en 232)全是 mask 分词器空白敏感导致的**良性数值/日期格式漂移**(prose 未变、渲染正常、链接健全);另有 **9 个镜像**(manufacturer-finance 6 + non-profit 3)仍带 v2026.06.03-9 域改名(`manufacturing→manufacturer-finance` / `governance→non-profit`)前的旧 `source:` 指针,被判 orphaned、落在 manifest 之外。处理:(1)用**对话内 opus 子代理**(mask→opus→verify→unmask→commit)分 3 波重译 764 个 link_only 镜像;(2)确定性订正这 9 个的 frontmatter `source:` 使其与路径(现域名)一致→重分类为 link_only→重译 27 个镜像。合计 **791 个镜像**(764+27)。已验证:`commit-translate` ok 合计=791 / needs_review=0(wave2 中途自检出 4 个占位符不符→删除→重译回收);占位符泄漏 **0**;`i18n:status` **stale ja/zh/en = 0 / 0 / 0**(current=1436、fidelity ok、orphaned=0、missing=0、无 source-pointer drift);`wiki_link_audit` entries=1483 issues=0 dead=0 canonical_drift=0;`release --check --strict` PASS。此举取代此前「残余 stale 为良性、设计上不处理」的结论——残余已全部清除。
...
### i18n Batch C-2a — link-only mirror の確定的 wikilink リターゲット (#21 進行中) / Deterministic wikilink retarget for link-only mirrors / link-only 镜像的确定性 wikilink 重映射

#### 日本語記録 / English / 中文

- **JST 時刻**: 2026-06-15 JST。
- **背景**: C-1 が skip した「token が現行 source と異なる」link_only mirror に対し、`tools/i18n_remap_links.ts`(新規・LLM 不使用)で確定的 wikilink リターゲットを試行。mirror と現行 source の token 多重集合を差分し、残差が全て wikilink かつ件数一致なら (basename, label) でペアリング(JapanFG 分割等の移設シグネチャ)し target path のみ書換、`mask(newBody).masks` が現行 source と完全一致する場合のみ source_hash と共に確定(post-verify がセーフティネット)。
- **重要な発見**: 残 1307 link_only のうち、確定的にリターゲット可能な「純 wikilink 移設」は **7 件のみ**(`regional-banks/sony-bank`→`banking/sony-bank` 等、source 側でエンティティが実際に移動)。残り 1268 件は **wikilink ではなく数値/日付トークンの整形ドリフト**(翻訳が `2011年`→`2011 年`、`696兆円`→分割 等の空白/単位差を導入し、空白依存の mask tokenizer の境界がずれて token 数まで相違)。これらは**表示上は正しく壊れていない**ため retranslation 不要のコスメティック差。32 件は (basename,label) が曖昧で C-2b 送り。
- **範囲**: `tools/i18n_remap_links.ts` 新規、mirror 7 件(各 source_hash + wikilink target 行のみ、本文 prose 不変)。
- **検証**: 7 mirror の diff は全て source_hash + リンク行のみ、`git diff --check` clean、`wiki_link_audit` dead=0、`release.ts --check --strict` PASS(entries=1485 issues=0 drift=0)。
- **残タスク再評価**: #21 の残 stale ≈1886 の内訳 — 数値整形ドリフト 1268(良性・コスメティック)/ ambiguous wikilink 32 / prose_changed 547 / unknown 39。真に retranslation を要するのは prose_changed + unknown(+曖昧 32)≈618 で、1268 は LLM 不要。C-2b は 618 に絞るのが妥当。
- **EN**: For the link_only mirrors C-1 skipped (tokens differ from the current source), `tools/i18n_remap_links.ts` (new, no LLM) attempts a deterministic wikilink retarget: diff the mirror's token multiset against the current source's, and if the leftovers are all wikilinks of equal count, pair them by (basename, label) — the split-migration signature — rewriting only the target path, committing the source_hash bump ONLY when `mask(newBody).masks` exactly equals the current source's (post-verify safety net). **Key finding**: of the 1307 remaining link_only, only **7** are pure wikilink relocations (e.g. `regional-banks/sony-bank`→`banking/sony-bank`, where the entity genuinely moved). The other 1268 are **numeric/date formatting drift, not link changes** — translation introduced spacing/unit differences (`2011年`→`2011 年`, `696兆円` split) that shift the whitespace-sensitive mask tokenizer's boundaries (token counts even differ); these render correctly and are cosmetic, needing no retranslation. 32 are ambiguous and deferred. Scope: `tools/i18n_remap_links.ts` + 7 mirrors (each only the source_hash + wikilink-target line; prose unchanged). Verified: every diff is source_hash + link line; `git diff --check` clean; `wiki_link_audit` dead=0; `release.ts --check --strict` PASS. Re-scoped residual: of ~1886 stale, only prose_changed (547) + unknown (39) + ambiguous (32) ≈ 618 truly need C-2b retranslation; the 1268 drift do not.
- **中文**: 针对 C-1 跳过的「token 与当前源不同」的 link_only 镜像，`tools/i18n_remap_links.ts`（新增、不用 LLM）尝试确定性 wikilink 重映射：对镜像与当前源的 token 多重集合求差，若残差全是 wikilink 且数量相等，则按 (basename, label) 配对（JapanFG 拆分等迁移特征）仅改写 target path，且仅当 `mask(newBody).masks` 与当前源完全一致时才连同 source_hash 落盘（post-verify 为安全网）。**关键发现**：1307 个残余 link_only 中，可确定性重映射的「纯 wikilink 迁移」只有 **7 个**（如 `regional-banks/sony-bank`→`banking/sony-bank`，源端实体确实移动）。其余 1268 个是**数值/日期 token 的格式漂移，而非链接变化**——翻译引入了空格/单位差异（`2011年`→`2011 年`、`696兆円` 被拆分），使依赖空白的 mask 分词边界偏移（连 token 数都不同）；它们**显示正确、未损坏**，属化妆性差异，无需重译。32 个因配对歧义留待 C-2b。范围：`tools/i18n_remap_links.ts` + 7 个镜像（每个仅 source_hash + wikilink target 行，正文不变）。已验证：每个 diff 仅 source_hash + 链接行；`git diff --check` clean；`wiki_link_audit` dead=0；`release.ts --check --strict` PASS。残余重估：约 1886 条 stale 中，真正需要 C-2b 重译的仅 prose_changed(547) + unknown(39) + 歧义(32) ≈ 618；1268 条漂移不需要。

### i18n Batch C-2b — prose_changed / unknown / ambiguous の opus subagent 全量重訳 (#21 進行中) / Full opus-subagent retranslation of prose-drifted mirrors / opus subagent 全量重译正文漂移镜像

#### 日本語記録 / English / 中文

- **JST 時刻**: 2026-06-15 JST。
- **背景**: #21 の C-2b。C-1/C-2a で確定的に処理できなかった stale mirror のうち、本当に重訳が必要なもの = prose_changed(547)+ unknown(39)+ C-2a が曖昧で送った ambiguous link_only(32)= **618 (source,lang) pair** を、実証済みの mask→opus subagent→verify→unmask→commit 管線で全量重訳(外部 API 不使用・対話内 opus subagent)。
- **新規ツール**: `site/scripts/prep-c2b.mjs`(manifest 駆動の prep。`prep-parallel.mjs` の walkEntries では届かない JapanFG 分割後 domain にも到達)。`tools/i18n_remap_links.ts` に `--emit-deferred`(ambiguous mirror を manifest として書出し C-2b へ供給)。
- **実行**: 3 wave(wave1 130 source/356 pair、wave2 80 source/230 pair、wave3 28 source/32 pair)、計 **25 opus subagent**、ja/zh/en。各 subagent は worker dir 内の masked source を翻訳し `.{lang}.md` を出力、`commit-translate.mjs` が verify+unmask して mirror 確定。
- **多言語の知見**: source は「日本語本文 + 英語の構造スキャフォールド(`## Wiki route`、`> [!info] TL;DR`、"This entry sits under…" 等)」の混在。ja mirror も恒等コピーではなく、この英語スキャフォールドを自然な日本語へ localize する正当な翻訳対象(本文は faithful 保持)。
- **検証**: 全 wave で `commit-translate` ok 合計 619(356+231+32)/ **needs_review=0**、変更 mirror に占位符リーク **0**、`wiki_link_audit` dead=0、`release.ts --check --strict` PASS。`i18n:status` stale **ja 526→352 / zh 763→533 / en 634→413**。
- **残 stale の性質**: 重訳後の残 stale(≈1298)は C-2a で特定した**良性の数値/日付整形ドリフト**(`2011年`↔`2011 年` 等、表示は正しくリンクも解決、dead=0)。設計上 LLM 重訳の対象外。
- **範囲**: `site/scripts/prep-c2b.mjs` 新規、`tools/i18n_remap_links.ts`(--emit-deferred)、i18n mirror 618 件(3 commit: `f13b5152` wave1 / `88cc1387` wave2 / `a7d18de8` wave3)、`CHANGELOG.md`。
- **EN**: #21 Batch C-2b. The stale mirrors C-1/C-2a could not fix deterministically that genuinely need retranslation — prose_changed (547) + unknown (39) + the 32 ambiguous link_only C-2a deferred = **618 (source,lang) pairs** — were fully retranslated through the proven mask→opus-subagent→verify→unmask→commit pipeline (no external API; in-conversation opus subagents). New tooling: `site/scripts/prep-c2b.mjs` (manifest-driven prep that reaches post-split domains `prep-parallel.mjs`'s walkEntries cannot) and `tools/i18n_remap_links.ts --emit-deferred` (writes the ambiguous list as a manifest for C-2b). Executed in 3 waves (356 + 230 + 32 pairs), **25 opus subagents**, ja/zh/en; each subagent translated masked sources to `.{lang}.md`, `commit-translate.mjs` verified+unmasked into mirrors. Multilingual insight: sources mix Japanese prose with English structural scaffolding (`## Wiki route`, `> [!info] TL;DR`, "This entry sits under…"), so even `ja` is a real translation target (localize the English scaffolding into natural Japanese; keep Japanese prose faithful) — not an identity copy. Verified: `commit-translate` ok=619 total, **needs_review=0**, **zero placeholder leaks**, `wiki_link_audit` dead=0, `release --check --strict` PASS; `i18n:status` stale **ja 526→352 / zh 763→533 / en 634→413**. Residual stale (~1298) is the benign numeric/date formatting drift identified in C-2a (renders correctly, links resolve, dead=0) — out of scope for LLM retranslation by design. Scope: `prep-c2b.mjs`, `i18n_remap_links.ts` (--emit-deferred), 618 mirrors over 3 commits, CHANGELOG.
- **中文**: #21 的 C-2b。把 C-1/C-2a 无法确定性修复、且确实需要重译的 stale 镜像——prose_changed(547）+ unknown(39）+ C-2a 因歧义留下的 32 个 ambiguous link_only = **618 个 (source,lang) 对**——通过验证过的 mask→opus subagent→verify→unmask→commit 管线全量重译（不接外部 API、对话内 opus subagent）。新增工具：`site/scripts/prep-c2b.mjs`（manifest 驱动的 prep，能到达 `prep-parallel.mjs` 的 walkEntries 够不到的 JapanFG 拆分后 domain）；`tools/i18n_remap_links.ts --emit-deferred`（把 ambiguous 镜像写成 manifest 供 C-2b 使用）。分 3 波执行（356 + 230 + 32 对），共 **25 个 opus subagent**，ja/zh/en；每个 subagent 翻译 worker 目录里的 masked source 输出 `.{lang}.md`，由 `commit-translate.mjs` verify+unmask 落盘。多语言发现：源文是「日文正文 + 英文结构骨架（`## Wiki route`、`> [!info] TL;DR`、"This entry sits under…"）」混排，所以连 `ja` 也是正当翻译目标（把英文骨架本地化为自然日文、日文正文保持忠实），并非恒等复制。已验证：`commit-translate` 合计 ok=619、**needs_review=0**、变更镜像**零占位符泄漏**、`wiki_link_audit` dead=0、`release --check --strict` PASS；`i18n:status` stale **ja 526→352 / zh 763→533 / en 634→413**。重译后残余 stale（约 1298）是 C-2a 已识别的**良性数值/日期格式漂移**（显示正确、链接可解析、dead=0），按设计不做 LLM 重译。范围：`prep-c2b.mjs`、`i18n_remap_links.ts`（--emit-deferred）、618 个镜像分 3 次提交、CHANGELOG。
...
- **JST 時刻**: 2026-06-07 JST。
- **背景**: 既存 `docs/` は v2026.06.03-10 / v2026.06.04-1 で作られたが、BRD / PRD / ARD / FSD / NFR / RTM のような要求から検証までの主幹文書がなく、既存文書も平置きに近かった。今後の大規模な content、i18n、site、release 変更を agent が安全に引き継げるよう、開発文書体系を再構築した。
- **範囲**: `docs/` 全体、旧 site-level の architecture / development-plan 文書、`lib/markdown_helpers.ts`、root `README.md`、`AGENTS.md`、`CHANGELOG.md`、`releases/v2026.06.07.md`、および release-generated surface。wiki 本文、公開金融 entry、site UI、public API 仕様は不変。
- **主要変更**: 既存文書を `docs/01-strategy/`、`docs/02-product/`、`docs/04-architecture/`、`docs/06-implementation/`、`docs/07-quality/`、`docs/08-operations/`、`docs/99-archive/` へ移動。新規に `00-governance`、`03-requirements`、`05-functional-specs` を含む文書群を追加し、`brd.md`、`prd.md`、`nfr.md`、`rtm.md`、`ard.md`、`fsd.md` を主幹にした。追加レビュー（2026-06-08 JST）で [model-agent-workflow.md](docs/06-implementation/model-agent-workflow.md) を加え、高推論の仕様モデルが仕様・検収・task packet を作り、高速コード実装モデルが許可ファイルだけを実装する規則、active subagent 上限 10、完了 subagent closeout、allowed-file review を RTM / QA / acceptance / AGENTS に接続した。続くコード/文書整合性 audit で [code-doc-alignment-audit.md](docs/07-quality/code-doc-alignment-audit.md) と [next-development-plan.md](docs/01-strategy/next-development-plan.md) を追加し、40 domain 現状、`.txt` audit gap、docs link checker、i18n freshness report、deployment runbook 強化、v12 双批去重の順序を次期開発計画として固定した。あわせて `domains.md` と `astro-site-architecture.md` の旧 23-domain / 旧 site mirror / 旧 postbuild 前提を現行コード事実へ修正し、`lib/markdown_helpers.ts` の frontmatter 境界検出を LF / CRLF 両対応にして CRLF entry の API frontmatter 欠落を防いだ。
- **UI/UX 追補（2026-06-08 JST）**: UI/UX を FSD 配下の実行入口にしつつ、横断テーマとして [ui-ux-principles.md](docs/02-product/ui-ux-principles.md)、[theme-system.md](docs/04-architecture/theme-system.md)、[ui-ux.md](docs/05-functional-specs/ui-ux.md)、[visual-qa-checklist.md](docs/07-quality/visual-qa-checklist.md) を追加した。PRD / ARD / FSD / NFR / RTM / QA / test plan / site-rendering / astro-site-architecture / docs README を相互リンクし、日語読者優先、低ノイズ金融リファレンス、brand-purple 系への drift 回避、CSS token source-of-truth、light/dark、responsive、Japanese UI chrome、visual QA の验収経路を明文化した。これは site UI 実装変更ではなく、後続 UI/CSS 変更の仕様・検収文書整備。
- **文書 drift 監査追補（2026-06-08 JST）**: 全 active docs / root control docs / generated surfaces / source `canonical_anchor` を再監査し、[documentation-drift-audit.md](docs/07-quality/documentation-drift-audit.md) を追加した。`japanfg-split-design.md` を実際の 17 新領域 + 40 domains + `JapanFG/INDEX` umbrella 保持へ修正し、旧 `JapanFG/<entity>` alias を使っていた source `canonical_anchor` を現行 entity path へ更新した。`generate_ai_discovery.ts` は `api/entries/` を清掃してから現行 JSON を再生成するよう修正し、`lib/markdown_helpers.ts` は `docs/` への相対 Markdown links を AI surface の `markdown_links` から除外する。deployment / incident / maintenance runbook は local site build、duplicate-id check、Vercel shadow、GitHub Actions watch、public URL spot-check、stale API residue 対応まで拡張した。
- **実行手順**: clean worktree 確認 → `git mv` で既存文書を新階層へ移動 → `docs/README.md` を単一入口へ再構成 → governance / strategy / product / requirements / architecture / functional specs / quality / operations 文書を追加 → 旧パス参照を新パスへ機械更新 → root `README.md` と `AGENTS.md` を三語更新 → release note 作成 → `bun tools/release.ts --write` → mtime 汚染を HEAD の `ai-index.json` から復元 → CRLF frontmatter parser を修正 → UI/UX 追補 → full documentation drift audit → source canonical_anchor / generator cleanup 修正 → `--check --strict` → `wiki_link_audit --fail-on-issues` → `git diff --check`。
- **検証結果**: stale path scan は 0 件。docs markdown link check は `docs_markdown_links_ok`。generated docs-surface check は `docs/` が `ai-index.json` / `api/entries/index.json` の `source_path`、public URL、API URL、`markdown_links`、および `sitemap.xml` URL に出ていないことを確認。stale API residue check は `api/entries/JapanFG/` が retained umbrella の `INDEX.json` のみであることを確認。API manifest/file check は `api_manifest_files_ok files=1478`。`bun tools/release.ts --write` は EXIT=0（`md=1566`、`domains=40`、`entries=1485`、`issues=0`、`canonical_drift=0`）、`bun tools/release.ts --check --strict` は EXIT=0、`bun tools/wiki_link_audit.ts --fail-on-issues` は EXIT=0、`cd site && bun run build` は EXIT=0（4441 pages built）、`bun tools/check_duplicate_html_ids.ts site/dist` は EXIT=0（`checked=4441`、`duplicate_id_pages=0`、`duplicate_ids=0`）、`git diff --check` は EXIT=0。
- **残タスク**: 次期開発は [next-development-plan.md](docs/01-strategy/next-development-plan.md) の Phase A→E 順で進める。優先順は `.txt` route/link audit、docs Markdown link checker、active-doc stale scan、generated-surface drift scan、i18n freshness report、v12 dedup review。要求・仕様・実装・検証の関係が変わる作業では `docs/03-requirements/rtm.md` を同時更新する。
- **EN**: Rebuilt the developer documentation system because the previous `docs/` tree had useful operational notes but no explicit BRD / PRD / ARD / FSD / NFR / RTM backbone from requirements to validation. Scope: all `docs/`, the former site-level architecture / development-plan docs, `lib/markdown_helpers.ts`, root `README.md`, `AGENTS.md`, `CHANGELOG.md`, `releases/v2026.06.07.md`, source canonical-anchor metadata, and release-generated surfaces; no broad site UI redesign was made. Moved existing docs into the new hierarchy, added governance / requirements / functional-spec / quality / operations documents, rewrote `docs/README.md` as the single entry point, updated old path references, updated root docs/agent rules, and fixed frontmatter boundary parsing for LF/CRLF so CRLF entries do not lose API frontmatter. The 2026-06-08 JST follow-ups added `docs/06-implementation/model-agent-workflow.md`, the UI/UX cross-layer docs, `docs/07-quality/code-doc-alignment-audit.md`, `docs/07-quality/documentation-drift-audit.md`, and `docs/01-strategy/next-development-plan.md`. The drift audit fixed active-doc drift in `domains.md`, `astro-site-architecture.md`, and `japanfg-split-design.md`; updated source `canonical_anchor` fields from old `JapanFG/<entity>` aliases to current entity paths; made API generation clear stale `api/entries/` files before rewriting; and filtered relative `docs/` Markdown links out of AI-surface `markdown_links`. The next development order is now `.txt` route audit, docs link checker, active-doc stale scan, generated-surface drift scan, i18n freshness report, then content dedup/selected expansion. Validation passed with stale-path scan 0, docs markdown link check OK, generated docs-surface check OK, stale API residue check OK, API manifest/file check OK, `release.ts --check --strict` PASS, wiki-link audit PASS, site build PASS with 4441 pages, duplicate-id check PASS, and `git diff --check` PASS.
- **EN UI/UX follow-up (2026-06-08 JST)**: Added UI/UX as the FSD execution entry while keeping the theme cross-linked across product, architecture, functional specs, and quality: [ui-ux-principles.md](docs/02-product/ui-ux-principles.md), [theme-system.md](docs/04-architecture/theme-system.md), [ui-ux.md](docs/05-functional-specs/ui-ux.md), and [visual-qa-checklist.md](docs/07-quality/visual-qa-checklist.md). Connected PRD / ARD / FSD / NFR / RTM / QA / test plan / site-rendering / astro-site-architecture / docs README so future UI/CSS changes have a clear route through Japanese-reader-first UX, restrained financial-reference presentation, non-purple brand drift rules, CSS token source of truth, light/dark behavior, responsive behavior, Japanese UI chrome, and visual QA. This documents acceptance for future UI/CSS work; it does not change the site UI implementation.
- **中文**: 重新构建开发文档体系，因为原来的 `docs/` 虽然有操作说明，但缺少从需求到验证的 BRD / PRD / ARD / FSD / NFR / RTM 主干。范围包括整个 `docs/`、原 site-level 架构 / 开发计划文档、`lib/markdown_helpers.ts`、根 `README.md`、`AGENTS.md`、`CHANGELOG.md`、`releases/v2026.06.07.md`、source canonical-anchor 元数据以及 release 生成 surface；没有做大规模站点 UI redesign。已把既有文档迁入新层级，新增 governance / requirements / functional-spec / quality / operations 文档，重写 `docs/README.md` 为唯一入口，更新旧路径引用，同步根 README 与 AGENTS 规则，并修复 LF/CRLF frontmatter 边界解析，避免 CRLF entry 生成 API 时丢失 frontmatter。2026-06-08 JST 的后续补充包括 `docs/06-implementation/model-agent-workflow.md`、UI/UX 横切文档、`docs/07-quality/code-doc-alignment-audit.md`、`docs/07-quality/documentation-drift-audit.md` 和 `docs/01-strategy/next-development-plan.md`。漂移审计修正了 `domains.md`、`astro-site-architecture.md`、`japanfg-split-design.md` 的活文档漂移；把 source `canonical_anchor` 从旧 `JapanFG/<entity>` alias 更新为当前 entity path；让 API 生成在重写前清理旧 `api/entries/` 文件；并把 `docs/` 相对 Markdown links 从 AI surface 的 `markdown_links` 中过滤掉。下一阶段开发顺序调整为：`.txt` route audit、docs link checker、active-doc stale scan、generated-surface drift scan、i18n freshness report，然后再做内容去重/选择性扩充。验证已通过：旧路径扫描 0、docs markdown link check OK、generated docs-surface check OK、stale API residue check OK、API manifest/file check OK、`release.ts --check --strict` PASS、wiki-link audit PASS、site build 4441 页 PASS、duplicate-id check PASS、`git diff --check` PASS。
- **中文 UI/UX 补充（2026-06-08 JST）**：把 UI/UX 放在 FSD 作为执行入口，同时作为横切主题连接产品、架构、功能规格和质量文档：新增 [ui-ux-principles.md](docs/02-product/ui-ux-principles.md)、[theme-system.md](docs/04-architecture/theme-system.md)、[ui-ux.md](docs/05-functional-specs/ui-ux.md)、[visual-qa-checklist.md](docs/07-quality/visual-qa-checklist.md)。已把 PRD / ARD / FSD / NFR / RTM / QA / test plan / site-rendering / astro-site-architecture / docs README 互相连接，明确日语用户优先、低噪音金融参考站、避免品牌化紫色漂移、CSS token source of truth、light/dark、响应式、日语 UI chrome 与 visual QA 验收路径。本次是为后续 UI/CSS 修改补规格和验收文档，不改站点 UI 实现。
...
- **背景**: v2026.06.05-3 の CI 完了確認後、backlog の「各領域 INDEX count 常態校准」を先に消化。`release.ts --check --strict` は green だったが、root `INDEX.md` の reader-facing Domain Map で一部テーマ領域の Entries 値が disk 実数より +1 になっていた。
- **範囲**: root `INDEX.md` の冒頭 snapshot と Domain Map 13 行、`docs/01-strategy/roadmap.md` / `docs/01-strategy/backlog.md` の進捗同期、三語 README/CHANGELOG/`releases/v2026.06.05-4.md`、release-generated discovery surface。wiki body facts、entity page、site config、領域数、entry 数は不変。
- **主要変更**: `exchanges` 147→146、`fintech` 144→143、`systems` 62→61、`agent-economy` 41→40、`banking` 40→39、`policy-finance` 35→34、`insurance` 33→32、`finance` 27→26、`payments` 41→40、`securities` 33→32、`derivatives` 33→32、`real-estate-finance` 28→27、`structured-finance` 30→29。root snapshot は link-audited entries 1485 へ更新。
- **実行手順**: `git status` と HEAD/origin 一致確認 → `gh run list --limit 1` で v05-3 `Deploy FinWiki | success` 確認 → docs/ と `SCHEMA.md` / `AGENTS.md` 読了 → root Domain Map を Node script で non-INDEX `.md` disk count と照合 → `INDEX.md` を最小 patch → domain mismatch 再計測 → release gate。
- **検証結果**: `domain_count_mismatches=0`。`bun tools/release.ts --check --strict` **EXIT=0**、link audit `entries=1485 / issues=0 / canonical_drift=0`、domains=40、counts in sync、JSON/LF/dup-id OK。v05-3 CI は completed/success（run `26988241948`、HEAD `2a88e6c9`）。
- **既知の注意点 / 次の作業**: この release は reader-facing count calibration のみ。P2 の残り ~51 thin stubs と非 thin entity の stale `JapanFG/INDEX` route batch fix は未着手で backlog 継続。
- **EN**: After confirming the v2026.06.05-3 CI result, cleared the backlog item for routine INDEX count calibration. The release gate was already green, but the reader-facing root `INDEX.md` Domain Map had 13 thematic domains displayed at +1 versus the actual non-INDEX `.md` file count. Scope: root `INDEX.md` snapshot + 13 rows, docs progress sync (`docs/01-strategy/roadmap.md` / `docs/01-strategy/backlog.md`), trilingual README/CHANGELOG/`releases/v2026.06.05-4.md`, and regenerated discovery surfaces; no wiki-body facts, entity pages, site config, domain count, or entry count changed. Corrected `exchanges` 147→146, `fintech` 144→143, `systems` 62→61, `agent-economy` 41→40, `banking` 40→39, `policy-finance` 35→34, `insurance` 33→32, `finance` 27→26, `payments` 41→40, `securities` 33→32, `derivatives` 33→32, `real-estate-finance` 28→27, and `structured-finance` 30→29; updated the root snapshot to 1485 link-audited entries. Checks: `domain_count_mismatches=0`; `release.ts --check --strict` EXIT=0, link audit `entries=1485 / issues=0 / canonical_drift=0`, domains=40, counts in sync. v05-3 CI is completed/success (run `26988241948`, HEAD `2a88e6c9`). Next: P2 remaining ~51 thin stubs and the non-thin stale `JapanFG/INDEX` route batch fix remain open.
- **中文**: 在确认 v2026.06.05-3 CI 结果后，先清掉 backlog 里的常态 INDEX 数量校准项。release gate 原本已 green，但 root `INDEX.md` 的读者可见 Domain Map 有 13 个主题域比实际 non-INDEX `.md` 文件数各高 1。范围：root `INDEX.md` snapshot + 13 行、docs 进度同步（`docs/01-strategy/roadmap.md` / `docs/01-strategy/backlog.md`）、三语 README/CHANGELOG/`releases/v2026.06.05-4.md`、重新生成的 discovery surface；wiki 正文事实、实体页、site 配置、领域数和 entry 数都不变。修正 `exchanges` 147→146、`fintech` 144→143、`systems` 62→61、`agent-economy` 41→40、`banking` 40→39、`policy-finance` 35→34、`insurance` 33→32、`finance` 27→26、`payments` 41→40、`securities` 33→32、`derivatives` 33→32、`real-estate-finance` 28→27、`structured-finance` 30→29；root snapshot 更新为 1485 个 link-audited entries。验证：`domain_count_mismatches=0`；`release.ts --check --strict` EXIT=0，link audit `entries=1485 / issues=0 / canonical_drift=0`，domains=40，counts in sync。v05-3 CI 已 completed/success（run `26988241948`，HEAD `2a88e6c9`）。下一步：P2 剩余 ~51 个 thin stubs，以及非 thin 实体页 stale `JapanFG/INDEX` 路由批量修，继续留在 backlog。

### P2 深化（第 1 批）— JapanFG 拆分域の存根 58 件を深化 / P2 deepening batch 1 (58 split-domain stubs) / P2 深化第 1 批（58 个拆分域存根）
#### 日本語記録 / English / 中文
- **JST 時刻**: 2026-06-05 09:38 JST。
- **背景**: roadmap P2 = JapanFG 拆分（v04-4 完了）後の entity 内容深化。~110 件の <250 語「登記簿存根」が対象。ユーザー指示「P2 深化、10 並列」で着手し、本 batch（58 件）で一旦区切って commit（残りは backlog）。
- **範囲**: `regional-banks/` 48 + `cooperative-banks/` 10 = 58 entity ページ深化、roadmap/backlog 更新、discovery 再生成、三語 README/CHANGELOG/`releases/v2026.06.05-3.md`。INDEX 件数は不変（純深化、add/remove なし）。
- **主要変更**: 各存根を実体ページへ——License/group boundary（免許種別/FSA・財務局/地銀協・第二地銀協・信金中金）、Operating model、group/system 文脈、4+ core-body wikilink。並列 subagent 3（regional A/B/C、slug-range 別 file-scope 隔離）+ cooperative（前 batch）。split 取り残しの旧 `[[JapanFG/INDEX]]` route を 58 件すべて新域 INDEX へ修正（残存 0）。
- **無捏造の担保**: 具体数値（預金/設立年/コード/親会社/支店数）は web 照合（公式 IR・FSA 登記簿・各協会・公式プレス）できたものだけ記載。照合不能な記述は agent が削除（例: 一部設立年・業務改善命令断定の撤回）。照合例: Shonai+Hokuto→フィデア銀行 2027-01、shizuoka-bank 1943-03/コード 8355、daiwa-next-bank の Orix Bank 買収 2026-04。
- **実行手順**: thin（<250 語）存根を domain 別に列挙 → 並列 agent 深化（accuracy 厳守の recipe）→ audit oracle で dead=0/drift=0 確認 → spot-check（shizuoka-bank）→ `--write` → `--check --strict`。
- **検証結果**: `wiki_link_audit` `entries_with_issues=0` / `dead_wikilink_references=0` / `canonical_anchor_drift=0`（1485）。`release.ts --check --strict` **EXIT=0**、domains=40、counts in sync、JSON/LF/dup-id OK。
- **既知の注意点 / 次の作業**: 内容は subagent 生成のため**事実精度は人工抽查が必要**（ADR-006）。次: (1) 残り ~51 thin 存根（life-insurers/securities-firms/asset-managers/payment-firms/trust-banks/megabanks/card-issuers ほか）の深化。(2) **非 thin** 実体ページの `[[JapanFG/INDEX]]` 旧 route の batch 修復。
- **EN**: Deepened 58 of the ~110 sub-250-word registry stubs in the JapanFG-split domains (roadmap P2, after the v04-4 split): `regional-banks` 48 + `cooperative-banks` 10. Each stub → a proper entity page with a License/group boundary (license type / FSA + regional 財務局 / 地銀協・第二地銀協・信金中金), an institution-type operating model, group/system context, and 4+ core-body wikilinks. 3 parallel subagents (regional A/B/C, slug-range file-scope isolation) + cooperative from a prior batch. All 58 had their split-leftover stale `[[JapanFG/INDEX]]` route fixed to the new-domain INDEX (0 remain). No-fabrication enforced: specific numbers (deposits/founding year/ticker/parent/branches) only when web-verified (official IR, FSA register, associations, press); unverifiable claims were removed by the agents (e.g. some founding years, a business-improvement-order assertion). Verified examples: Shonai+Hokuto → フィデア銀行 2027-01, shizuoka-bank 1943-03 / code 8355, daiwa-next-bank's Orix Bank acquisition 2026-04. Verification: audit entries_with_issues=0 / dead=0 / drift=0 (1485); `release.ts --check --strict` EXIT=0, domains=40, counts in sync. Content is subagent-generated → factual precision needs human spot-check (ADR-006; spot-checked shizuoka-bank). User paused after this batch; the remaining ~51 stubs (11 domains) + a batch-fix of the stale `[[JapanFG/INDEX]]` routes on non-thin pages go to the backlog.
- **中文**: 深化了 JapanFG 拆分域里 ~110 个 <250 词登记簿存根中的 58 个（roadmap P2，v04-4 拆分之后）：`regional-banks` 48 + `cooperative-banks` 10。每个存根 → 完整实体页：License/group boundary（免许种别 / FSA + 地域财务局 / 地银协·第二地银协·信金中金）、机构类型 operating model、group/system 语境、4+ core-body wikilink。3 个并行 subagent（regional A/B/C，按 slug 范围 file-scope 隔离）+ 前批的 cooperative。58 个都把 split 残留的旧 `[[JapanFG/INDEX]]` 路由改到各自新域 INDEX（0 残留）。强制无捏造：具体数字（存款/设立年/代码/母公司/支店数）只在 web 核实后才写（官方 IR、FSA 登记簿、各协会、新闻）；核实不了的被 agent 删除（如部分设立年、业务改善命令断言）。核实例：Shonai+Hokuto→フィデア銀行 2027-01、shizuoka-bank 1943-03/代码 8355、daiwa-next-bank 的 Orix Bank 收购 2026-04。验证：audit entries_with_issues=0 / dead=0 / drift=0（1485）；`release.ts --check --strict` EXIT=0、domains=40、counts in sync。内容由 subagent 生成 → 事实精度需人工抽查（ADR-006；已 spot-check shizuoka-bank）。用户在本批后暂停；剩余 ~51 个存根（11 域）+ 非 thin 页的 `[[JapanFG/INDEX]]` 旧路由批量修，记入 backlog。

### P4 i18n — 55 entry に ja/zh/en mirror 165 file（鍵不要・subagent 直接翻訳）/ P4 i18n: 165 ja/zh/en mirrors, no API key / P4 i18n：55 entry × 3 语 = 165 镜像（无需密钥）
#### 日本語記録 / English / 中文
- **JST 時刻**: 2026-06-05 00:30 JST。
- **背景**: roadmap P4 = v12 並列追加 entry の i18n。v05-1 では「pipeline `translate.mjs` が `ANTHROPIC_API_KEY` 必要で未実行」とした——だがユーザー指摘の通り、これは誤った前提（`translate.mjs` は standalone Node script が別の Claude Haiku を API 呼びするから鍵が要るだけ）。subagent（Claude 自身）が直接翻訳すれば鍵不要。本 release でそれを実施。
- **範囲**: `site/src/content/i18n/{ja,zh,en}/` に 165 新 mirror（55 source × 3 言語：v12 の 43 + P3 の 12）。corpus（root .md）不変。三語 README/CHANGELOG/`releases/v2026.06.05-2.md`、backlog（P4 🟢）。
- **主要変更**: 8 並列 subagent（domain 別 file-scope 隔離）が各 entry を ja/zh/en へ翻訳。mirror frontmatter は pipeline 形式を再現: `source` / `source_hash`（= `sha256(body).slice(0,16)`、`translate.mjs` 同一算法）/ `lang` / `model: claude-opus-4-8` / `status: machine` / `fidelity: ok` / `title` / `translated_at`。i18n collection は `.passthrough()` で `model:` 等を許容（pipeline 自身も `model:` を書く）。17 新ドメイン INDEX は mirror 不要（domain landing は config 生成、banking も INDEX mirror 無し）。
- **実行手順**: i18n パイプライン調査（鍵不要と判明）→ mirror 欠如を per-lang で測定（55×3）→ source_hash manifest 生成（`splitFm` + `sha256(body).slice(0,16)` を厳密再現）→ 8 並列 subagent 翻訳 → 全体 link-integrity チェック → `--check --strict`。
- **検証結果（ガードレール）**: i18n は link audit の対象外なので、各 agent + 全体スクリプトで全 165 mirror の `[[link]]` target と数値が source と byte 一致を検証 → **`checked=165 link_mismatches=0`**。`[[…]]` は verbatim 保持（label 現地化は build の `localize-wikilinks`）。数 agent が翻訳中に label/数値を弄った数例を検出・差し戻し。`release.ts --check --strict` EXIT=0（corpus 不変、entries=1485 / issues=0 / dead=0 / canonical_drift=0 / domains=40 / counts in sync）。最終検証は CI Astro build。
- **既知の注意点 / 次の作業**: 強モデル（subagent）直接翻訳のため Haiku-pipeline mirror とは生成経路が異なるが形式・hash 互換。push 後 CI build 緑確認。
- **EN**: Did roadmap P4 (i18n for the v12 parallel-added entries) **without an API key**. v05-1 had claimed `translate.mjs` needs `ANTHROPIC_API_KEY` so P4 was "blocked" — but per the user's challenge, that was a wrong premise: the script needs the key only because it's a standalone Node program calling a *separate* Claude Haiku over the API. Subagents (Claude itself) can translate directly, no key. Generated 165 mirrors (55 sources × ja/zh/en: 43 v12 + 12 P3) under `site/src/content/i18n/`; corpus unchanged. 8 parallel subagents (per-domain, file-scope-isolated) translated each entry; mirror frontmatter replicates the pipeline format (`source_hash = sha256(body).slice(0,16)` exactly as `translate.mjs`, `model: claude-opus-4-8`, `status: machine`, `fidelity: ok`). The i18n collection is `.passthrough()` (accepts `model:`; the pipeline writes it too). The 17 new-domain INDEX pages need no mirror (landings are config-generated). Guardrail: since i18n is outside the link audit, each agent plus a global script verified every one of the 165 mirrors has `[[link]]` targets + numbers byte-identical to its source → `checked=165 link_mismatches=0` (a few agent label/number alterations were caught and reverted). `release.ts --check --strict` EXIT=0 (corpus unchanged, entries=1485 / issues=0 / dead=0 / drift=0 / domains=40, counts in sync). Final validation is the CI Astro build.
- **中文**: **没用密钥**完成了 roadmap P4（v12 并行新增 entry 的 i18n）。v05-1 曾说 `translate.mjs` 需要 `ANTHROPIC_API_KEY` 所以 P4「阻塞」——但正如用户所指，这是错误前提：该脚本需要密钥只因它是独立 Node 程序、通过 API 调另一个 Claude Haiku。subagent（Claude 自己）可以直接翻译，无需密钥。在 `site/src/content/i18n/` 下生成 165 个镜像（55 source × ja/zh/en：43 个 v12 + 12 个 P3）；corpus 不变。8 个并行 subagent（按域 file-scope 隔离）翻译每个 entry；镜像 frontmatter 复刻流水线格式（`source_hash = sha256(body).slice(0,16)` 与 `translate.mjs` 同算法、`model: claude-opus-4-8`、`status: machine`、`fidelity: ok`）。i18n collection 是 `.passthrough()`（接受 `model:`；流水线自身也写）。17 个新域 INDEX 不需要镜像（landing 由 config 生成）。护栏：因 i18n 不在 link audit 内，每个 agent 加全局脚本验证全部 165 个镜像的 `[[link]]` 目标 + 数字与 source byte 一致 → `checked=165 link_mismatches=0`（几处 agent 改动 label/数字被发现并还原）。`release.ts --check --strict` EXIT=0（corpus 不变，entries=1485 / issues=0 / dead=0 / drift=0 / domains=40、counts in sync）。最终验证是 CI Astro build。
...
- **背景**: ユーザー判断で、平坦で「太笼统」な `JapanFG/`（約 620 entity）を機関類型別に物理分割。[ADR-004](docs/04-architecture/japanfg-split-design.md) の「移動コスト極大ゆえ sub-INDEX で代用」方針の知情推翻。事前に `docs/04-architecture/japanfg-split-design.md`（設計）で方式・分類・機制を確定（物理分割 / 角色復数名 / 17 ドメイン / 単一原子 migration）。
...
- **背景**: v2026.06.04-2 で Phase 2 の entity edge を落地し、roadmap P1 は「`canonical_anchor` を hard requirement（mirror page 必填）+ audit を gate 化」の評価だけが残っていた。本 session で評価し、機械 gate できる部分を gate 化、できない部分を規範化して P1 を完了した。
- **範囲**: `tools/wiki_link_audit.ts`、`tools/release.ts`、`tools/release.helpers.ts`、`SCHEMA.md`、`docs/06-implementation/entry-authoring.md` / `decisions.md`(ADR-007) / `roadmap.md` / `backlog.md`、再生成された discovery surface、counts sync された `README.md` / `index.html`、`CHANGELOG.md` / `releases/v2026.06.04-3.md`。
- **評価結論（ADR-007）**: 「すべての mirror が `canonical_anchor` を必須宣言」は機械的に gate **できない**——mirror か否かは編集判断で列挙不能。実測で宣言済み 12 mirror のうち anchor と同 slug は 4 件のみ（`sony-bank` / `au-jibun-bank` / `daiwa-next-bank` / `ui-bank`）、残り 8 件は異 slug（`toyota-financial-services→toyota-financial` 等）、かつ同 slug 跨域対は 4/4 すべて宣言済み。slug 启发式は recall≈33% かつ violation 0。よって機械 gate は **宣言の有効性（drift）** に置き、「mirror 必須」は **規範 + review** で担保。
- **主要変更**: (1) `tools/wiki_link_audit.ts` に `--fail-on-canonical-drift`（`Args` field + parse + main 退出 gate `canonical_anchor_drift>0 → return 1`）。default は report-only を維持（flag 無しでは exit code 不変、ADR-002 の隔離保持）。report の見出しと注記も drift-gated へ更新。(2) `tools/release.ts` の `runWikiLinkAudit` が常に `--fail-on-canonical-drift` を渡す + `canonicalDrift` を parse、失敗時に broken-links と drift を区別して表示。(3) `release.helpers.ts` の `AuditResult` に `canonicalDrift`。(4) `SCHEMA.md` の `canonical_anchor` を「mirror page 必須・drift gate」へ、`docs/06-implementation/entry-authoring.md` に必須宣言と硬 gate を明記。(5) ADR-007 追加、roadmap/backlog で P1 完了。
- **実行手順**: roadmap P1 残を確認 → 同 slug 跨域対 + 宣言 recall を実測（mirror 検出が機械化不能と確認）→ audit に gate flag 追加 → 正例/負例/対照で検証 → SCHEMA/authoring/ADR/roadmap/backlog 更新 → 三語 README/CHANGELOG/release notes → `release.ts --write` → `--check --strict`。
- **検証結果**: 正例 `--fail-on-canonical-drift`（drift=0）→ EXIT=0；負例（壊れた `canonical_anchor` を持つ一時ページ）→ drift=1 → EXIT=1（gate 効く、確認後削除）；対照（flag 無）→ EXIT=0（report-only 既定保持）。`bun tools/release.ts --check --strict` EXIT=0、link audit entries=1466 / issues=0 / canonical_drift=0 → PASS、md=1538 / domains=23、counts in sync、JSON/LF/dup-id OK、`sitemap.xml` lastmod 汚染 0（wiki .md 未編集）、敏感情報 0。
- **既知の注意点**: ツール（gate）+ 規範が主体、公開 entry 新規追加無し（corpus entries 不変、md は release notes で +1）。「mirror 必須」は authoring/review-time ルールで機械 enforcement ではない（ADR-007）。
- **次の作業**: push 後に GitHub Actions / remote HEAD / GitHub Release を確認。P1 完了につき次は P4（v12 新 entry の i18n）/ P3（`non-profit` / `trade` 続扩）/ P2（JapanFG 深化）。
- **EN**: Completed roadmap P1 (canonical_anchor Phase 2) by landing the remaining "hard requirement + gate the audit" item. Evaluation (ADR-007): "every mirror must declare canonical_anchor" cannot be a mechanical gate — mirror-ness is editorial and not enumerable. Of the 12 declared mirrors only 4 are same-slug as their anchor (sony-bank / au-jibun-bank / daiwa-next-bank / ui-bank); the other 8 are editorial (e.g. toyota-financial-services→toyota-financial); and all same-slug cross-domain pairs are already declared (4/4). The slug heuristic has ~33% recall and 0 findable violations, so the mechanical gate enforces declaration validity (drift) while "mirror pages must declare" is enforced by standards + review. Changes: (1) `tools/wiki_link_audit.ts` gains `--fail-on-canonical-drift` (Args field + parse + exit gate `drift>0 → return 1`), keeping report-only as the default (no flag → exit code unchanged, preserving ADR-002 isolation); report heading/notes updated to drift-gated. (2) `tools/release.ts` `runWikiLinkAudit` always passes the flag and parses `canonicalDrift`, with a failure message that distinguishes broken links from drift. (3) `release.helpers.ts` `AuditResult` gains `canonicalDrift`. (4) `SCHEMA.md` marks `canonical_anchor` required-on-mirror/drift-gated; `docs/06-implementation/entry-authoring.md` documents the required declaration and the hard gate. (5) ADR-007 added; roadmap/backlog mark P1 complete. Verification: positive (drift=0)→EXIT=0; negative (planted broken anchor)→drift=1→EXIT=1 (removed after); control (no flag)→EXIT=0 (report-only preserved). `release.ts --check --strict` EXIT=0, audit entries=1466 / issues=0 / canonical_drift=0 → PASS, md=1538 / domains=23, counts in sync, JSON/LF/dup-id OK, 0 sitemap lastmod pollution (no wiki .md touched), 0 sensitive data. No public entries added (corpus entries unchanged; md +1 from this release note).
- **中文**: 完成 roadmap P1（canonical_anchor Phase 2），落地剩余的「hard requirement + audit 升 gate」。评估（ADR-007）：「每个 mirror 必须声明 canonical_anchor」无法机械 gate——mirror 与否是编辑判断、不可枚举。12 个已声明 mirror 里只有 4 个与 anchor 同 slug（sony-bank / au-jibun-bank / daiwa-next-bank / ui-bank），其余 8 个是异 slug（如 toyota-financial-services→toyota-financial），且同 slug 跨域对 4/4 全部已声明。slug 启发式 recall≈33% 且 0 可发现 violation，故机械 gate 落在声明有效性（drift），「mirror 必填」靠规范 + review 兜底。变更：(1) `tools/wiki_link_audit.ts` 加 `--fail-on-canonical-drift`（`Args` 字段 + parse + 退出 gate `drift>0 → return 1`），默认保持 report-only（不带 flag 时 exit code 不变，保持 ADR-002 隔离）；report 标题/注记改为 drift-gated。(2) `tools/release.ts` 的 `runWikiLinkAudit` 始终带此 flag 并 parse `canonicalDrift`，失败提示区分 broken links 与 drift。(3) `release.helpers.ts` 的 `AuditResult` 加 `canonicalDrift`。(4) `SCHEMA.md` 把 `canonical_anchor` 标为 mirror 必填 / drift-gated；`docs/06-implementation/entry-authoring.md` 写明必填声明与硬 gate。(5) 加 ADR-007；roadmap/backlog 标 P1 完成。验证：正例（drift=0）→EXIT=0；负例（放损坏 anchor）→drift=1→EXIT=1（验后删除）；对照（无 flag）→EXIT=0（保持 report-only）。`release.ts --check --strict` EXIT=0、audit entries=1466 / issues=0 / canonical_drift=0 → PASS、md=1538 / domains=23、counts in sync、JSON/LF/dup-id OK、0 sitemap lastmod 污染（未动 wiki .md）、0 敏感数据。未新增公开 entry（corpus entries 不变；md 因本 release note +1）。

### canonical_anchor Phase 2 — discovery entity edge（ai-index.json entities[] / llms-full.txt）/ canonical_anchor Phase 2 — discovery entity edges / canonical_anchor Phase 2 — discovery 实体边
#### 日本語記録 / English / 中文
- **JST 時刻**: 2026-06-04 11:37 JST。
- **背景**: roadmap P1 = canonical_anchor Phase 2。Phase 0（SCHEMA 字段 + pilot、v7）→ Phase 1（report-only audit、v8）→ back-fill（v8/v13、計 12 mirror、drift=0）まで完了済みで、残る唯一の P1 主体作業が「discovery 出力の entity edge」（proposal §4 Phase 2 / `frontmatter-canonical-anchor-field-proposal.md`）だった。本 session で落地し、frontmatter にしか無かった entity 同一性を機械可読な graph edge として discovery surface に露出した。
- **範囲**: `tools/generate_ai_discovery.ts`、`lib/markdown_helpers.ts`、再生成された discovery surface（`ai-index.json` / `llms.txt` / `llms-full.txt` / `api/entries/index.json`）、counts sync された `README.md` / `index.html`、`CHANGELOG.md` / `releases/v2026.06.04-2.md`。docs/ の roadmap / backlog / proposal も同期。
- **主要変更**: (1) `Entry` 型 + `buildEntry` に frontmatter `canonical_anchor` 抽出を追加。(2) `ai-index.json` に `entities[]`（12 edge: `anchor` / `anchor_url` / `anchor_resolves` / `member_count` / `mirror_count` / `members[relation: canonical | mirror]`）を新設、`entries[]` 各 entry に `canonical_anchor`、counts に `entity_anchors`=12 / `entity_mirror_pages`=12。(3) `llms-full.txt` の各 mirror page に `Canonical anchor: <anchor> -> <url>` 行（12 行）。(4) `llms.txt` snapshot に entity anchor 行 + AI 読解規則 1 条。(5) scope 判断: per-entry API（`api/entries/*.json`）は curated subset のため未変更（1459 file の機械的 diff 回避、entity edge は ai-index.json / llms-full.txt で完全露出、backlog に将来オプション記載）。
- **実行手順**: docs roadmap で P1 残 = entity edge を確認 → 12 件の canonical_anchor 宣言（全て JapanFG anchor、literal file 実在、drift=0）を盤点 → 型 + 生成器拡張 → temp out-dir で dry-run 検証（entities=12 / 全 `anchor_resolves=true` / llms-full 12 行 / `sitemap.xml` は wiki .md 無編集ゆえ commit と byte 一致）→ 三語 README/CHANGELOG/release notes 執筆 → `bun tools/release.ts --write` → `--check --strict`。
- **検証結果**: `bun tools/release.ts --check --strict` EXIT=0、link audit entries=1466 / issues=0 → PASS、md=1537 / domains=23、counts in sync、`entity_anchors=12` / `entity_mirror_pages=12` / drift=0、`sitemap.xml` lastmod 汚染 0（wiki 本文 .md 未編集）、JSON / LF / duplicate id verify OK、敏感情報 0。
- **既知の注意点**: discovery 生成器の拡張が主体で、公開 wiki 本文（entry）の新規追加は無し（corpus entries 不変、md は本 release notes で +1）。`entities[]` は mirror が 1 件以上ある実体のみ出力（自己参照のみの page は edge にしない）。
- **次の作業**: push 後に GitHub Actions / remote HEAD / GitHub Release を確認。開発の次手（`docs/01-strategy/roadmap.md`）: P1 残 = `canonical_anchor` を mirror page の hard requirement に昇格する是非評価（audit を gate 化）、P4 = v12 新 entry の i18n、P3 = `non-profit` / `trade` 続扩。
- **EN**: Landed the only remaining main P1 work for canonical_anchor Phase 2 — entity edges in the discovery output (proposal §4 Phase 2). Phase 0 (SCHEMA field + pilot, v7) → Phase 1 (report-only audit, v8) → back-fill (v8/v13, 12 mirrors, drift=0) were already done; this session exposed the entity identity that previously lived only in frontmatter as machine-readable graph edges on the discovery surfaces. (1) Extended the `Entry` type and `buildEntry` to extract frontmatter `canonical_anchor`. (2) Added an `entities[]` array to `ai-index.json` (12 edges: `anchor` / `anchor_url` / `anchor_resolves` / `member_count` / `mirror_count` / `members[relation: canonical | mirror]`), a `canonical_anchor` field on every entry in `entries[]`, and counts `entity_anchors`=12 / `entity_mirror_pages`=12. (3) Added a `Canonical anchor: <anchor> -> <url>` line to each mirror page in `llms-full.txt` (12 lines). (4) Added an entity-anchor snapshot line and one AI reader rule to `llms.txt`. (5) Scope: the per-entry API (`api/entries/*.json`) is left unchanged — its frontmatter block is already a curated subset, the edges are fully exposed via ai-index.json / llms-full.txt, and this avoids a mechanical 1459-file diff (recorded as a future option in the backlog). Verification: `release.ts --check --strict` EXIT=0, audit entries=1466 / issues=0 → PASS, md=1537 / domains=23, counts in sync, entity_anchors=12 / drift=0, 0 sitemap lastmod pollution (no wiki .md touched), JSON / LF / duplicate-id OK, 0 sensitive data. No public entries added (corpus entries unchanged; md +1 from this release note).
- **中文**: 落地了 canonical_anchor Phase 2 唯一剩余的主体工作——discovery 输出里的 entity edge（proposal §4 Phase 2）。Phase 0（SCHEMA 字段 + pilot，v7）→ Phase 1（report-only audit，v8）→ back-fill（v8/v13，12 个 mirror，drift=0）此前已完成；本 session 把原本只存在于 frontmatter 的 entity 同一性，作为机器可读的 graph edge 暴露到 discovery surface。(1) 扩展 `Entry` 类型与 `buildEntry`，从 frontmatter 抽取 `canonical_anchor`。(2) 在 `ai-index.json` 新增 `entities[]`（12 条 edge：`anchor` / `anchor_url` / `anchor_resolves` / `member_count` / `mirror_count` / `members[relation: canonical | mirror]`），`entries[]` 每个 entry 加 `canonical_anchor` 字段，counts 加 `entity_anchors`=12 / `entity_mirror_pages`=12。(3) 在 `llms-full.txt` 每个 mirror page 加 `Canonical anchor: <anchor> -> <url>` 行（12 行）。(4) 在 `llms.txt` snapshot 加 entity anchor 行 + 1 条面向 AI 的阅读规则。(5) scope：per-entry API（`api/entries/*.json`）不动——其 frontmatter block 本就是 curated subset，edge 已在 ai-index.json / llms-full.txt 完整暴露，避免 1459 个文件的机械 diff（作为未来选项记入 backlog）。验证：`release.ts --check --strict` EXIT=0、audit entries=1466 / issues=0 → PASS、md=1537 / domains=23、counts in sync、entity_anchors=12 / drift=0、sitemap lastmod 0 污染（未动 wiki .md）、JSON / LF / duplicate-id OK、0 敏感数据。未新增公开 entry（corpus entries 不变；md 因本 release note +1）。
...
#### 日本語記録 / English / 中文
- **JST 時刻**: 2026-06-03 20:40 JST。
- **背景**: roadmap P1 = canonical_anchor Phase 2。v2026.06.03-12 で Saison drift（第一手）を解消済み。本 release では subagent で残る mirror pair を盤点し back-fill した。
- **範囲**: mirror page 4 件（`banking/au-jibun-bank`、`insurance/kampo-japan-post-insurance`、`insurance/sony-life-group-life-operating-model`、`insurance/ja-kyosai-zenkyoren-overview`）、再生成された発見面、README / CHANGELOG / `releases/v2026.06.03-13.md`。anchor（JapanFG）ページ・root INDEX・tools は無変更。
- **主要変更**: 4 件の mirror page に `canonical_anchor` を SCHEMA canonical key order で追加: au-jibun-bank→JapanFG/au-jibun-bank、kampo-japan-post-insurance→JapanFG/kampo-life、sony-life-group-life-operating-model→JapanFG/sony-life、ja-kyosai-zenkyoren-overview→JapanFG/ja-kyosairen。au-jibun-bank は core body に anchor link が無かったため Wiki-route 段に cross-link を追加（drift 回避）、他 3 件は既に core body link 済みで frontmatter のみ追加。subagent は保守的に判定し、rakuten-bank / paypay-bank / mercari-bank（同一実体 JapanFG entity 無し）、TSE / OSE 等（JPX の子会社で親子≠同一実体）、hitachi-industrial-finance（旧日立キャピタルは別 legacy 実体）、新規 manufacturer-finance の Nissan / Honda / Komatsu（JapanFG entity 無し）を skip。
- **実行手順**: subagent が既存 canonical_anchor 8 件の pattern を確認 → net bank / insurer / securities / captive finance カテゴリで mirror pair を盤点 → 確信できる 4 件に anchor 設定 + core body link 確保 → `bun tools/wiki_link_audit.ts --fail-on-issues` で drift=0 を検証 → 主会話で `release.ts --write` → `--check --strict`。
- **検証結果**: `wiki_link_audit --fail-on-issues` EXIT=0、entries_checked=1466 / entries_with_issues=0 / dead_wikilink=0 / **canonical_anchor_checked=8→12 / drift=0**。`release.ts --check --strict` EXIT=0、counts in sync、JSON / LF / duplicate id verify OK。md / entries は不変（frontmatter + 1 body link のみ）、敏感情報 0。
- **既知の注意点**: canonical_anchor Phase 2 の残: discovery 出力への entity edge（`ai-index.json` / `llms-full.txt` に mirror→anchor 関係を暴露）と hard requirement 化は後続（`docs/01-strategy/backlog.md` 参照）。
- **次の作業**: push 後に GitHub Actions、remote HEAD、GitHub Release を確認。次は Phase 2 の entity edge 出力。
- **EN**: roadmap P1 canonical_anchor Phase 2 continuation. After v12 fixed the Saison drift (first step), a subagent inventoried remaining mirror pairs and back-filled 4: set `canonical_anchor` on `banking/au-jibun-bank`→JapanFG/au-jibun-bank, `insurance/kampo-japan-post-insurance`→JapanFG/kampo-life, `insurance/sony-life-group-life-operating-model`→JapanFG/sony-life, `insurance/ja-kyosai-zenkyoren-overview`→JapanFG/ja-kyosairen. au-jibun-bank lacked a core-body anchor link so one was added to its Wiki-route section (drift avoidance); the other 3 already cross-linked their anchor (frontmatter-only). The agent was conservative, skipping rakuten-bank / paypay-bank / mercari-bank (no same-entity JapanFG page), TSE/OSE (JPX subsidiaries — parent ≠ same entity), hitachi-industrial-finance (旧日立キャピタル is a separate legacy entity), and the new manufacturer-finance Nissan/Honda/Komatsu (no JapanFG entity). Verification: `wiki_link_audit --fail-on-issues` EXIT=0, entries 1466 / issues 0 / dead 0 / **canonical_anchor_checked 8→12 / drift=0**; `release.ts --check --strict` EXIT=0, counts in sync. md/entries unchanged (frontmatter + 1 body link). Remaining Phase 2: entity edges in discovery output + hard-requirement evaluation.
- **中文**: roadmap P1 canonical_anchor Phase 2 续。v12 修了 Saison drift（第一步）后，subagent 盘点剩余 mirror pair 并 back-fill 了 4 个：给 `banking/au-jibun-bank`→JapanFG/au-jibun-bank、`insurance/kampo-japan-post-insurance`→JapanFG/kampo-life、`insurance/sony-life-group-life-operating-model`→JapanFG/sony-life、`insurance/ja-kyosai-zenkyoren-overview`→JapanFG/ja-kyosairen 设 `canonical_anchor`。au-jibun-bank 的 core body 没有 anchor link，故在 Wiki-route 段补了一个（避免 drift）；另 3 个已 cross-link 其 anchor（仅加 frontmatter）。agent 保守，跳过 rakuten-bank / paypay-bank / mercari-bank（无同实体 JapanFG 页）、TSE/OSE（JPX 子公司，父子≠同实体）、hitachi-industrial-finance（旧日立 Capital 是另一 legacy 实体）、新 manufacturer-finance 的 Nissan/Honda/Komatsu（无 JapanFG entity）。验证：`wiki_link_audit --fail-on-issues` EXIT=0、entries 1466 / issues 0 / dead 0 / **canonical_anchor_checked 8→12 / drift=0**；`release.ts --check --strict` EXIT=0、counts in sync。md/entries 不变（frontmatter + 1 body link）。Phase 2 剩余：discovery 输出 entity edge + 评估 hard requirement。

### 並列 10 領域開発（46 entry 追加）/ Parallel 10-domain development (46 entries) / 并行 10 领域开发（新增 46 entry）
#### 日本語記録 / English / 中文
- **JST 時刻**: 2026-06-03 20:05 JST。
- **背景**: `docs/01-strategy/roadmap.md` の P2（JapanFG 深化）/ P3（薄領域の内容拡充）に基づき、ユーザー要請で 10 領域へ並列 subagent で公開情報の金融 entry を一括追加した。docs/ の進度・規劃文書（v11）が「次に何をするか」を定義したので、それを実際の開発で消化する初回。
- **範囲**: 10 領域（security / non-profit / trade / retail / corporate-strategy / manufacturer-finance / money-market / loyalty / business / JapanFG）に 46 件の新 entry + 各領域 INDEX、`INDEX.md`（root の domain count 10 行 + 先頭スナップショット）、再生成された発見面、`README.md` / `CHANGELOG.md` / `releases/v2026.06.03-12.md`。wiki 本文以外（tools/lib/site/docs）は無変更。
- **主要変更**: 各領域へ公開情報 only の entry を追加（security +4、business +6、corporate-strategy +6、loyalty +6、manufacturer-finance +6、money-market +6、JapanFG +3、non-profit +3、trade +3、retail +3 = 46）。各 entry は SCHEMA frontmatter + `## Wiki route` 開頭 + ≥3 body wikilink（全 target を ls/grep で実在検証）+ 公開 source 付き。root `INDEX.md` の 10 領域 count を disk 実数へ同期（例: business 19→24、money-market 12→17、corporate-strategy 6→11、JapanFG 631→634）、先頭スナップショットを 1411→1466 entries へ更新。
- **実行手順**: 初回は Workflow で 10 並列を試みたが schema(StructuredOutput) 層で全失敗（0 token）→ 直接 Agent 方式へ切替。探路 1 件(security)で成功確認 → 残 9 件を並列投入 → うち 5 件が API rate-limit（但し失敗前に file は書込済み、tool_uses=22-29）→ 該当 5 領域を低並列で再試行 → 各領域が「初回 3 + 再試行 3 = 6」の補完的 entry を獲得 → root INDEX count を disk 実数へ同期 → `bun tools/release.ts --write` → `--check --strict`。
- **検証結果**: `release.ts --check --strict` は EXIT=0、link audit **entries=1420→1466 / issues=0 → PASS**、**dead_wikilink_references=0 / dead_wikilink_targets=0**、canonical_anchor_checked=8 / drift=0、md=1487→1533 / domains=23、counts in sync、JSON/LF/duplicate id verify OK。lastmod 汚染 0、敏感情報 0。全 agent は file-scope を厳守（tools/lib/site/docs/root への越境 0）。
- **既知の注意点**: 5 領域（business / corporate-strategy / loyalty / manufacturer-finance / money-market）は rate-limit + 再試行により倍量(6)の entry を得たが、主題は補完的（公司案例 / 法律機制 / 各 benchmark 等で重複せず、retry agent が初回分を認識し別主題を選択）。内容は並列 agent 生成のため、構造・リンクは audit 済みだが事実精度は人手の抽出確認を推奨。i18n（ja/zh/en 機械翻訳）は未生成で後続。
- **次の作業**: push 後に GitHub Actions、remote HEAD、GitHub Release を確認。後続で i18n 翻訳生成、canonical_anchor Phase 2 残（mirror pair back-fill / entity edge）、各領域の継続拡充。
- **EN**: Per `docs/01-strategy/roadmap.md` P2 (JapanFG depth) / P3 (thin-domain content expansion) and a user request, bulk-added public-information financial entries across 10 domains via parallel subagents — the first time the v11 progress/planning docs drove actual development. Added 46 entries (security +4, business +6, corporate-strategy +6, loyalty +6, manufacturer-finance +6, money-market +6, JapanFG +3, non-profit +3, trade +3, retail +3); each has SCHEMA frontmatter, a "## Wiki route" opener, >=3 body wikilinks (every target verified to exist via ls/grep), and public sources. Synced the root INDEX's 10 domain counts to disk reality (e.g. business 19→24, money-market 12→17, JapanFG 631→634) and the header snapshot to 1466 entries. Process: an initial Workflow attempt failed entirely at the schema/StructuredOutput layer (0 tokens), so I switched to direct Agents — 1 scout (security) confirmed the path, then 9 in parallel of which 5 hit API rate-limit (but had already written their files, tool_uses 22-29); retrying those 5 at lower concurrency gave each "initial 3 + retry 3 = 6" complementary entries. Verification: `release.ts --check --strict` EXIT=0, audit **entries 1420→1466 / issues=0 PASS, 0 dead links**, canonical drift=0, md 1487→1533, counts in sync; 0 pollution, 0 sensitive data, every agent stayed strictly in its domain scope. Caveats: the 5 double-batch domains gained 6 complementary (non-duplicating) entries each; content is parallel-agent-generated — structure/links are audited but factual accuracy warrants a human spot-check; i18n machine translations are not yet generated (follow-up).
- **中文**: 依 `docs/01-strategy/roadmap.md` 的 P2（JapanFG 深化）/ P3（薄领域内容扩充）+ 用户要求，用并行 subagent 给 10 个领域批量加公开信息金融 entry —— 这是 v11 进度/规划文档首次驱动实际开发。新增 46 个 entry（security +4、business +6、corporate-strategy +6、loyalty +6、manufacturer-finance +6、money-market +6、JapanFG +3、non-profit +3、trade +3、retail +3）；每个有 SCHEMA frontmatter、`## Wiki route` 开头、≥3 个 body wikilink（每个 target 用 ls/grep 验证实在）、公开 source。把 root INDEX 的 10 个领域 count 同步到 disk 实数（如 business 19→24、money-market 12→17、JapanFG 631→634），先头快照改为 1466 entries。过程：初次用 Workflow 尝试 10 并行，在 schema(StructuredOutput) 层全失败（0 token），改用直接 Agent —— 1 个探路（security）确认可行后，9 个并行投入，其中 5 个撞 API rate-limit（但失败前已写好文件，tool_uses 22-29）；把这 5 个低并发重试，各领域得到「初次 3 + 重试 3 = 6」个互补 entry。验证：`release.ts --check --strict` EXIT=0、audit **entries 1420→1466 / issues=0 PASS、零死链**、canonical drift=0、md 1487→1533、counts in sync；0 污染、0 敏感、所有 agent 严守领域边界。注意：5 个双批领域各得 6 个互补（不重复）entry；内容由并行 agent 生成，结构/链接已 audit 但事实精度建议人工抽查；i18n 机器翻译未生成（后续）。

### 開発進度ドキュメント拡充と canonical_anchor Phase 2 起動 / Dev-progress docs + canonical_anchor Phase 2 kickoff / 开发进度文档扩充与 canonical_anchor Phase 2 起步
#### 日本語記録 / English / 中文
- **JST 時刻**: 2026-06-03 19:10 JST。
- **背景**: v2026.06.03-10 で docs/ 開発文書システム（参考型 5 件）を新設した。本 release では (1) それを進度・規劃型文書で拡充し docs を開発進度の支柱として機能させ、(2) `docs/01-strategy/roadmap.md` で定めた P1 = canonical_anchor Phase 2 の最初の一手を実行する。
- **範囲**: 新規 docs/（roadmap / backlog / decisions / domains の 4 件）、`docs/README.md`（索引再編）、`insurance/saison-automobile-fire.md`（drift 修正）、`README.md`、`CHANGELOG.md`、`releases/v2026.06.03-11.md`、再生成された発見面。
- **主要変更**: (1) docs/ に進度・規劃型 4 文書を追加: roadmap（v5→v10 里程碑 + P1-P3 近期重点 + 不做清单）、backlog（歴史 CHANGELOG の open-items を P1/P2/P3 分類 + 状態）、decisions（5 件の ADR: 物理改名 / canonical_anchor 分期 / docs 双排除 / JapanFG sub-INDEX / Astro allowlist）、domains（23 領域現状表 + 最薄 6 領域の扩充優先 + 領域境界備忘）。`docs/README.md` の索引を「参考型 / 進度・規劃型」に再編。(2) canonical_anchor Phase 2 の起点として Saison drift を解消: `insurance/saison-automobile-fire` の core body（Wiki route 段）に canonical anchor `JapanFG/saison-automobile-fire-insurance` への cross-link を追加。従来は anchor を本文で参照しておらず、report-only audit が drift=1 を出していた。
- **実行手順**: docs/ 4 文書を作成・README 索引再編 → `wiki_link_audit.ts` の `coreBody()` / `auditCanonicalAnchors()` を read-only で確認（core body = `## Related` / `## Sources` 前、drift = anchor は resolve するが core body に anchor への link が無い）→ saison の Wiki route 段に cross-link 追加 → `wiki_link_audit --fail-on-issues` で drift 解消を検証 → `release.ts --write` → `--check --strict`。
- **検証結果**: `wiki_link_audit --fail-on-issues` は EXIT=0、entries_checked=1420 / entries_with_issues=0 / dead_wikilink=0 / **canonical_anchor_checked=8 / canonical_anchor_drift=1→0**。`release.ts --check --strict` は EXIT=0、link audit issues=0、counts in sync、JSON / LF / duplicate id verify OK。docs/ 4 件は排除済みのため md 計数・sitemap / llms / ai-index には不計入（md は release notes 分のみ +1）。
- **既知の注意点**: docs の進度文書（roadmap の優先級排序等）は主観判断を含むため後続で随時更新する。canonical_anchor Phase 2 の残: 剩余 mirror pair back-fill、discovery への entity edge 出力、hard requirement 化（backlog 参照）。
- **次の作業**: push 後に GitHub Actions、remote HEAD、GitHub Release を確認。次は P1 の剩余 mirror pair back-fill、または P3 の security planned 4 件。
- **EN**: Expanded the docs/ system (created in v10) with 4 progress/planning docs — roadmap (milestones v5→v10, P1-P3 priorities, out-of-scope), backlog (historical open-items by P1/P2/P3 + status), decisions (5 ADRs: physical rename / canonical_anchor phasing / docs dual-exclusion / JapanFG sub-INDEX / Astro allowlist), domains (23-domain status table + thinnest-6 expansion priority + boundary notes) — and reorganized the docs index into "reference / progress-planning". As the first step of canonical_anchor Phase 2 (P1 in the roadmap), resolved the Saison drift: added a core-body cross-link from `insurance/saison-automobile-fire`'s Wiki-route section to its canonical anchor `JapanFG/saison-automobile-fire-insurance` (previously the page never referenced its anchor in the body, so the report-only audit showed drift=1). Verification: `wiki_link_audit --fail-on-issues` EXIT=0, entries_checked=1420 / issues=0 / dead=0 / canonical_anchor_checked=8 / **drift 1→0**; `release.ts --check --strict` EXIT=0, counts in sync. The 4 docs files are excluded, so they don't count toward md / sitemap / llms / ai-index (md only +1 from this release note).
- **中文**: 用 4 个进度/规划文档扩充 v10 建立的 docs/ 系统——roadmap（里程碑 v5→v10、P1-P3 优先级、不做清单）、backlog（历史 open-items 按 P1/P2/P3 + 状态）、decisions（5 条 ADR：物理改名 / canonical_anchor 分期 / docs 双排除 / JapanFG sub-INDEX / Astro allowlist）、domains（23 领域现状表 + 最薄 6 领域扩充优先 + 边界备忘）——并把 docs 索引重整为「参考型 / 进度·规划型」。作为 canonical_anchor Phase 2（roadmap 的 P1）第一步，修复 Saison drift：在 `insurance/saison-automobile-fire` 的 Wiki route 段（core body）加指向 canonical anchor `JapanFG/saison-automobile-fire-insurance` 的 cross-link（此前该页从不在本文引用其 anchor，故 report-only audit 显示 drift=1）。验证：`wiki_link_audit --fail-on-issues` EXIT=0、entries_checked=1420 / issues=0 / dead=0 / canonical_anchor_checked=8 / **drift 1→0**；`release.ts --check --strict` EXIT=0、counts in sync。4 个 docs 文件已排除，不计入 md / sitemap / llms / ai-index（md 仅因本 release note +1）。

### 開発文書システム docs/ の新設 / Internal developer-docs system under docs/ / 新建内部开发文档系统 docs/
#### 日本語記録 / English / 中文
- **JST 時刻**: 2026-06-03 18:25 JST。
- **背景**: repo の運用知識（アーキテクチャ、ツールチェーン、リリース SOP、落とし穴）が README / CHANGELOG / AGENTS に散在し、新規メンテナや agent が全体像を把握しにくかった。公開 wiki 本文とは分離した、内部開発者向けの文書ツリー `docs/` を新設して集約する。
- **範囲**: 新規 `docs/`（README 索引 / architecture / toolchain / release-process / gotchas の 5 件）、`lib/markdown_helpers.ts`、`tools/wiki_link_audit.ts`、`AGENTS.md`（三語規則追記）、`README.md`、`CHANGELOG.md`、`releases/v2026.06.03-10.md`。site 設定・wiki 本文・i18n は無変更。
- **主要変更**: (1) `docs/` に 5 件の開発文書を新設（仓库结构・corpus 模型・双排除机制・新增 entry 手順 / 各 tool・lib の職責と命令 / リリース SOP と自検チェックリスト / clone-mtime 汚染・self-ref count・site 盲推・假死链 等の落とし穴集）。(2) `docs/` を 2 つの独立した排除リストに登録: `lib/markdown_helpers.ts` の `EXCLUDED_WALK_DIRS`（`iterMarkdownFiles` 経由で `release.ts` 計数と `generate_ai_discovery.ts` 発見面を制御）と `tools/wiki_link_audit.ts` の `IGNORED_DIRS`（死リンク監査）。これにより `docs/` は corpus（md / entries 計数）・`sitemap.xml` / `llms.txt` / `ai-index.json` / `api/`・Astro サイト・死リンク監査のいずれにも含まれない内部専用となる。`site/` の `ENTRY_DOMAIN_DIRS` は allowlist のため `docs/` は自動的に非対象（site 設定は無変更）。(3) `AGENTS.md` に「`docs/` は内部開発文書・非公開・排除済み・解除禁止」の規則を三語で追記。
- **実行手順**: 2 つの排除機構を read-only で確認（`release.ts:135` と `generate_ai_discovery.ts:96` が共に `iterMarkdownFiles` を使用 → `EXCLUDED_WALK_DIRS` 一箇所で count / sitemap / discovery を網羅できると確定）→ `EXCLUDED_WALK_DIRS` と `IGNORED_DIRS` に `docs` を追加 → `docs/` 5 件を作成 → `AGENTS.md` / `README.md` を三語追記 → `bun tools/release.ts --write` → `--check --strict` で検証。
- **検証結果**: `bun tools/release.ts --check --strict` は EXIT=0、link audit entries=1420 / issues=0 → PASS、**md=1485 据え置き（docs 5 件は corpus に不計入）**、domains=23 / entries=1420、counts in sync、JSON / LF / duplicate id verify OK。`sitemap.xml` / `llms.txt` / `ai-index.json` の `docs/` URL は各 0、敏感情報 0。
- **既知の注意点**: `docs/` は公共 GitHub repo 上にあるため、内部文書でも密钥 / 個人情報は置かない（本地パス・内部フロー例は可）。排除は 2 系統（`EXCLUDED_WALK_DIRS` と `IGNORED_DIRS`）あり、片方だけでは漏れるため両方を維持すること。
- **次の作業**: push 後に GitHub Actions（Astro build に docs/ が影響しないことの確認を含む）、remote HEAD、GitHub Release を確認。`docs/` の内容拡充（CI / Vercel 詳細、i18n 翻訳フロー、Obsidian 運用）は後続。
- **EN**: Added an internal developer-documentation tree under `docs/` (5 files: README index / architecture / toolchain / release-process / gotchas) to consolidate operating knowledge (architecture, toolchain, release SOP, gotchas) that was scattered across README / CHANGELOG / AGENTS, kept separate from the public wiki body. Registered `docs/` in two independent exclusion lists: `EXCLUDED_WALK_DIRS` (`lib/markdown_helpers.ts`, which `iterMarkdownFiles` drives for `release.ts` counts and `generate_ai_discovery.ts` discovery surfaces) and `IGNORED_DIRS` (`tools/wiki_link_audit.ts`, dead-link audit), so `docs/` is excluded from the corpus (md / entries counts), `sitemap.xml` / `llms.txt` / `ai-index.json` / `api/`, the Astro site, and the audit. `site/`'s `ENTRY_DOMAIN_DIRS` is an allowlist so `docs/` is auto-excluded (no site config change). Added a trilingual AGENTS.md rule that `docs/` is internal, unpublished, excluded, and must not be re-included. Verification: `--check --strict` EXIT=0, audit entries=1420 / issues=0 PASS, **md=1485 unchanged (the 5 docs files are not counted)**, domains=23, counts in sync; 0 `docs/` URLs in sitemap / llms / ai-index; 0 sensitive data.
- **中文**: 在 `docs/` 新建内部开发者文档树（5 个文件：README 索引 / architecture / toolchain / release-process / gotchas），把散落在 README / CHANGELOG / AGENTS 的运营知识（架构、工具链、发布 SOP、陷阱）汇集起来，与公开 wiki 正文分离。把 `docs/` 注册进两个独立排除列表：`EXCLUDED_WALK_DIRS`（`lib/markdown_helpers.ts`，`iterMarkdownFiles` 据此驱动 `release.ts` 计数与 `generate_ai_discovery.ts` 发现面）和 `IGNORED_DIRS`（`tools/wiki_link_audit.ts`，死链审计），使 `docs/` 不进 corpus（md / entries 计数）、`sitemap.xml` / `llms.txt` / `ai-index.json` / `api/`、Astro 站点和审计。`site/` 的 `ENTRY_DOMAIN_DIRS` 是 allowlist 故 `docs/` 自动排除（site 配置未改）。给 AGENTS.md 三语加了一条规则：`docs/` 内部、不发布、已排除、禁止重新纳入。验证：`--check --strict` EXIT=0、audit entries=1420 / issues=0 PASS、**md=1485 不变（docs 5 文件不计入）**、domains=23、counts in sync；sitemap / llms / ai-index 的 `docs/` URL 均为 0；0 敏感数据。

### 領域物理改名 governance→non-profit・manufacturing→manufacturer-finance / Domain physical rename governance→non-profit, manufacturing→manufacturer-finance / 领域物理改名 governance→non-profit、manufacturing→manufacturer-finance
#### 日本語記録 / English / 中文
- **JST 時刻**: 2026-06-03 17:47 JST。
- **背景**: v2026.06.03-6 の盤点で 2 領域の mislabel（governance の実体は非営利・公益法人、manufacturing の実体は manufacturer-finance）を識別し、v2026.06.03-7 で INDEX に disambiguation を追加（file 移動なしの Option A）して暫定対処していた。本 release では恒久対処として C-A（物理改名）を選択し、ディレクトリ名自体を実体に合わせて改名した。公開 URL が不可逆に変わる点を承知の上での確定的な改名。
- **範囲**: root 領域ディレクトリ 2 件（`governance/`→`non-profit/` 4 entry、`manufacturing/`→`manufacturer-finance/` 8 entry）、対応する i18n（ja/zh/en）ディレクトリ、`.opinions/` 配下、約 460 件の path wikilink を含む 60+ ファイル、site build 設定 3 件（`site/src/content.config.ts`・`site/src/lib/siteIndex.mjs`・`site/src/i18n/groups.ts`）、`tools/wiki_link_audit.ts` の domain 配置、`INDEX.md` の domain 表、再生成された discovery surface。
- **主要変更**: (1) `git mv` で root + i18n + `.opinions` のディレクトリを改名。(2) bun スクリプトで `governance/…`→`non-profit/…`、`](governance/...)`→`](non-profit/...)`、frontmatter `domain: governance`→`non-profit`（引用符付き `domain: "manufacturing"` を含む）等の path 参照を精確置換。正文中の "governance"/"manufacturing" の語、および historical な `CHANGELOG.md` / `releases/*.md` の史実記述は意図的に不変とした（rename スクリプトが誤改変した分は `git checkout HEAD --` で復元）。(3) site build 設定の `ENTRY_DOMAIN_DIRS`（content.config.ts と siteIndex.mjs）、`DOMAIN_TITLES`（ja/en/zh の 3 言語マップ、表示名を「非営利 / Non-Profit / 非营利」「製造業金融 / Manufacturer Finance / 厂商金融」へ）、`i18n/groups.ts` の super-group（corporate に manufacturer-finance、governance group に non-profit）を更新。(4) `wiki_link_audit.ts` の per-domain suggested-link map を新 domain key へ更新。(5) `INDEX.md` の domain 表第一列を改名。
- **実行手順**: governance→non-profit を先行実施し root link audit PASS を確認 → manufacturing→manufacturer-finance を同手順で実施 → 引用符付き `domain: "manufacturing"` の取りこぼしを grep で発見し修正 → rename スクリプトが誤改変した historical record を `git checkout HEAD --` で復元 → site 設定 3 件・INDEX・audit 配置を sed で精確置換 → `bun tools/release.ts --write`（mtime は既に当日生成済みのため汚染なし）→ `--check --strict` で検証。
- **検証結果**: `bun tools/release.ts --check --strict` は EXIT=0、link audit entries=1420 / issues=0 → PASS（死リンク 0、residual `[[manufacturing/`・`[[governance/` 0）、md=1484 / domains=23 / entries=1420、counts in sync、JSON/LF/duplicate id verify OK。sitemap は non-profit 4 / manufacturer-finance 8 URL、旧 `governance/`・`manufacturing/` URL は 0、lastmod 汚染 0、敏感情報 0。
- **既知の注意点**: **公開 URL が不可逆に変わる**（`/{lang}/governance/x`→`/{lang}/non-profit/x`、`/{lang}/manufacturing/x`→`/{lang}/manufacturer-finance/x`）。旧 URL は 301 ではなく消滅するため外部 backlink は失効する。`site/` に node_modules が無いため Astro build を本地検証できず、build 健全性は push 後の GitHub Actions に依存する（build 失敗時は本 commit を revert）。canonical_anchor の mirror path（README の v8 note 等）は新 path へ追従済み。
- **次の作業**: push 後に GitHub Actions（Astro build）、remote HEAD、GitHub Release、公開 `/non-profit/INDEX`・`/manufacturer-finance/INDEX` を確認。build 失敗時は本 commit を revert する。
- **EN**: Physically renamed the two mislabeled domains identified during the v2026.06.03-6 audit and stop-gapped with INDEX disambiguation in v2026.06.03-7 (Option A, no file moves): `governance/`→`non-profit/` (4 entries; real scope is non-profit / public-interest foundations) and `manufacturing/`→`manufacturer-finance/` (8 entries; real scope is manufacturer / captive finance). Steps: `git mv` for root + i18n (ja/zh/en) + `.opinions`; a bun script precisely rewrote ~460 path references (`governance/…`→`non-profit/…`, `](governance/...)`, frontmatter `domain` including the quoted `domain: "manufacturing"` form), leaving prose words and the historical CHANGELOG/releases records intact (accidental edits restored via `git checkout HEAD --`); updated the site build config (`ENTRY_DOMAIN_DIRS` in content.config.ts + siteIndex.mjs, the ja/en/zh `DOMAIN_TITLES` to "非営利 / Non-Profit / 非营利" and "製造業金融 / Manufacturer Finance / 厂商金融", the super-groups in i18n/groups.ts), the `wiki_link_audit.ts` domain map, and the root INDEX domain table. Verification: `release.ts --check --strict` EXIT=0, audit PASS (entries=1420 / issues=0, 0 dead links, 0 residual `[[governance/`/`[[manufacturing/`), md=1484 / domains=23, counts in sync; sitemap 4 non-profit / 8 manufacturer-finance URLs, 0 old URLs, 0 lastmod pollution. Caveat: **public URLs change irreversibly** (`/{lang}/governance/x`→`/{lang}/non-profit/x` etc.; old URLs disappear, not 301). site/ has no node_modules, so the Astro build is verified only by GitHub Actions post-push (revert if it fails).
- **中文**: 物理改名 v2026.06.03-6 盘点识别、v2026.06.03-7 用 INDEX disambiguation 暂处（Option A 不移文件）的两个 mislabel 领域：`governance/`→`non-profit/`（4 entry；实体是非营利 / 公益法人）、`manufacturing/`→`manufacturer-finance/`（8 entry；实体是厂商金融 / captive finance）。步骤：root + i18n（ja/zh/en）+ `.opinions` 用 `git mv`；bun 脚本精确置换约 460 处 path 引用（`governance/…`→`non-profit/…`、`](governance/...)`、frontmatter `domain` 含带引号的 `domain: "manufacturing"`），正文词语与 historical CHANGELOG/releases 史实记述保持不变（误改部分用 `git checkout HEAD --` 复原）；更新 site build 配置（content.config.ts + siteIndex.mjs 的 `ENTRY_DOMAIN_DIRS`、ja/en/zh `DOMAIN_TITLES` 改为「非営利 / Non-Profit / 非营利」「製造業金融 / Manufacturer Finance / 厂商金融」、i18n/groups.ts 的 super-group）、`wiki_link_audit.ts` 的 domain map、root INDEX domain 表。验证：`release.ts --check --strict` EXIT=0、audit PASS（entries=1420 / issues=0、零死链、residual `[[governance/`/`[[manufacturing/` 为 0）、md=1484 / domains=23、counts in sync；sitemap 含 4 个 non-profit / 8 个 manufacturer-finance URL、0 个旧 URL、0 lastmod 污染。注意：**公开 URL 不可逆变更**（`/{lang}/governance/x`→`/{lang}/non-profit/x` 等；旧 URL 消失而非 301）。site/ 无 node_modules，Astro build 仅由 push 后 GitHub Actions 验证（失败则 revert）。

### canonical_anchor 全量 back-fill と Phase 1 report-only audit / canonical_anchor back-fill and Phase 1 report-only audit / canonical_anchor 全量 back-fill 与 Phase 1 report-only audit
#### 日本語記録 / English / 中文
- **JST 時刻**: 2026-06-03 17:22 JST。
- **背景**: v2026.06.03-7 で canonical_anchor の Phase 0（SCHEMA field + 2 pilot）を落地済み。本 release では background subagent により全 mirror pair への back-fill と Phase 1 の audit 拡張を落地した。
- **範囲**: mirror page 6 件（`manufacturing/sony-group-finance-arm`、`banking/ui-bank`、`banking/daiwa-next-bank`、`banking/sony-bank`、`banking/gmo-aozora-net-bank`、`JapanFG/japan-post-bank`）、`tools/wiki_link_audit.ts`、再生成された discovery surface。
- **主要変更**: (1) 「operating-profile companion to the entity anchor」等の自己記述を機械可読シグナルとして確認した 6 件の mirror page に `canonical_anchor` を設定（`sony-group-finance-arm`→`JapanFG/sony-fg`、net bank 4 件→各 JapanFG entity anchor、`japan-post-bank`→`JapanFG/yucho`）。case page（`business/sony-fg-partial-spinoff-case` 等）・anchor を body link しない peer（`au-jibun-bank`）・captive 不在の platform page は保守的に未設定。(2) `tools/wiki_link_audit.ts` に report-only の `auditCanonicalAnchors()` を追加: 各 `canonical_anchor` 宣言に対し anchor の実在性と core body cross-link を検証し、`canonical_anchor_checked` / `canonical_anchor_drift` counter と `--report` 専用セクションを出力。`AuditRow.issues`・`entries_with_issues`・dead counter・exit code には一切触れない厳格な report-only。
- **実行手順**: background subagent が mirror pair を発見・設定 → audit を拡張 → `bun tools/wiki_link_audit.ts --fail-on-issues`（EXIT=0）と、工具改動のみを適用し content edit を HEAD に戻した状態での `--check --strict` EXIT=0（隔離 regression test）で検証。
- **検証結果**: 直接 audit は entries_checked=1420 / entries_with_issues=0 / dead_wikilink_references=0 / dead_wikilink_targets=0 / **canonical_anchor_checked=8 / canonical_anchor_drift=1** / EXIT=0。`bun tools/release.ts --check --strict` は count sync 後 EXIT=0、link audit entries=1420 / issues=0 → PASS。drift=1 は Saison pilot（anchor を Related footer のみ参照）で、report-only のため gate を阻害しない。
- **既知の注意点**: drift=1 の Saison pilot は将来 body link を追加するか warning として残すか後続判断。Phase 2+（discovery 出力への entity edge、残る mirror pair の back-fill、hard requirement 化）は後続。
- **次の作業**: push 後に remote HEAD、GitHub Actions、GitHub Release を確認。後続で drift 解消・残 mirror pair back-fill・Phase 2 を検討。
- **EN**: Landed the canonical_anchor back-fill and Phase 1 audit extension via a background subagent. (1) Set `canonical_anchor` on 6 mirror pages confirmed via the machine-readable self-description "operating-profile companion to the entity anchor" (sony-group-finance-arm→JapanFG/sony-fg, 4 net banks→their JapanFG anchors, japan-post-bank→JapanFG/yucho); conservatively left unset on case pages (business/sony-fg-partial-spinoff-case), peers without a body link to the anchor (au-jibun-bank), and platform pages with no captive entity. (2) Added a report-only `auditCanonicalAnchors()` to tools/wiki_link_audit.ts that checks each declared anchor resolves and is core-body cross-linked, emitting `canonical_anchor_checked` / `canonical_anchor_drift` counters and a --report section, strictly without touching AuditRow.issues, entries_with_issues, dead counters, or any exit code. Verification: direct audit entries_checked=1420 / issues=0 / dead=0 / canonical_anchor_checked=8 / drift=1 / EXIT=0; release.ts --check --strict EXIT=0 after count sync, link audit PASS. The single drift is the Saison pilot (anchor referenced only in its Related footer); report-only, so it never blocks the gate.
- **中文**: 通过 background subagent 落地 canonical_anchor 全量 back-fill 与 Phase 1 audit 扩展。(1) 在 6 个经机器可读自述「operating-profile companion to the entity anchor」确认的 mirror page 上设 `canonical_anchor`（sony-group-finance-arm→JapanFG/sony-fg、4 个网络银行→各自 JapanFG anchor、japan-post-bank→JapanFG/yucho）；保守地未设 case page（business/sony-fg-partial-spinoff-case）、无 body link 指向 anchor 的 peer（au-jibun-bank）、以及无 captive 实体的 platform page。(2) 给 tools/wiki_link_audit.ts 加 report-only 的 `auditCanonicalAnchors()`，校验每个声明的 anchor 是否 resolve 且 core body cross-link，输出 `canonical_anchor_checked` / `canonical_anchor_drift` counter 与 --report 段，严格不触碰 AuditRow.issues、entries_with_issues、dead counter 或任何 exit code。验证：直接 audit entries_checked=1420 / issues=0 / dead=0 / canonical_anchor_checked=8 / drift=1 / EXIT=0；release.ts --check --strict 在 count sync 后 EXIT=0、link audit PASS。唯一的 drift 是 Saison pilot（anchor 只在 Related footer 引用）；report-only，不阻塞 gate。

### 領域再定位・JapanFG sub-INDEX・canonical_anchor Phase 0 / Domain rescope, JapanFG sub-indexes, canonical_anchor Phase 0 / 领域再定位、JapanFG sub-INDEX、canonical_anchor Phase 0
#### 日本語記録 / English / 中文
- **JST 時刻**: 2026-06-03 16:53 JST。
- **背景**: 内容整理（v2026.06.03-6）に続き、盤点で識別された構造系の改善 B/C/D を並列 write subagent 3 件で落地した。いずれも wiki 本文への新規事実追加はなく、構造・命名・索引の整理に限定。
- **範囲**: (B) `governance/INDEX.md`、`manufacturing/INDEX.md`。(C) JapanFG 新規 sub-INDEX 9 件 + `JapanFG/INDEX.md`。(D) `SCHEMA.md`、`frontmatter-canonical-anchor-field-proposal.md`、mirror pair 4 件（insurance/saison-automobile-fire・JapanFG/saison-automobile-fire-insurance・manufacturing/toyota-financial-services・JapanFG/toyota-financial）。再生成された discovery surface。
- **主要変更**: (B) mislabel だった 2 領域に scope/disambiguation を追加。governance は非営利・公益法人（一般財団 / 公益財団 / 一般社団 / NPO / 研究助成）が実体である旨を明示し corporate governance を finance/securities/exchanges へ案内。manufacturing は manufacturer-finance（captive / vendor / export finance）が実体である旨を明示し entity ページは JapanFG、ABS は structured-finance へ案内（file 移動なし）。(C) 631 件が flat だった JapanFG に frontmatter `tags` 駆動で機構類型別 sub-INDEX 9 件を新設（megabanks-and-fg / regional-banks / cooperative-finance / trust / insurance / securities-and-asset-management / payments-cards-leasing-finance / foreign-institutions / regulators-sro-policy、計 609 entry を bare wikilink で分類）。`legal-financial-licenses/INDEX.md` の subdomain パターンに倣い各 sub-INDEX に Placement Rule（entity ページは JapanFG/ に留まる）を明記、`JapanFG/INDEX.md` 冒頭に類型別導航ブロックを追加。**file は一切移動せず**（Option A）53,615 件超の inbound link・URL・sitemap を保全。(D) `canonical_anchor`（string、entity の単一 source-of-truth anchor への vault-root path）を `SCHEMA.md` の Optional / Legacy Fields と Canonical Key Order（`related` の直後）に追加。Saison・Toyota Financial Services の 2 mirror pair に pilot 設定（mirror ページが JapanFG anchor を指す）、proposal を status: active / Phase 0 adopted へ更新。Phase 1（audit tooling）は後続、`tools/*.ts` は未変更。
- **実行手順**: 並列 write subagent 3 件で B/C/D を各自独立ファイル域で実装（各 agent は他 agent の並行変更を検知し越境せず）→ `bun tools/release.ts --check` で link audit を検証（C の新 sub-INDEX 2,064 wikilink を含む）。
- **検証結果**: `bun tools/release.ts --check --strict` は EXIT=0、link audit entries=1420 / issues=0 → PASS（9 件の sub-INDEX wikilink 全 resolve、wiki 本文死リンク 0）、counts in sync、JSON/LF/duplicate id verify OK。corpus は md=1473→1482（+9 sub-INDEX）、entries=1411→1420、domains=23 維持。
- **既知の注意点**: sogo-shosha 親会社 7 件等 9 entity は非金融機関のため意図的に未分類（金融子会社は分類済み）。canonical_anchor の全量 back-fill と audit 工具対応（Phase 1+）、各領域の内容拡充は後続タスク。
- **次の作業**: push 後に remote HEAD、GitHub Actions、GitHub Release、公開 `JapanFG/INDEX` と新 sub-INDEX を確認。後続で内容拡充・canonical_anchor 全量化・audit tooling を検討。
- **EN**: Landed the structural improvements B/C/D with 3 parallel write subagents (independent file scopes). (B) Added scope/disambiguation to the mislabeled governance (non-profit/public-interest) and manufacturing (manufacturer-finance) INDEXes, routing corporate governance to finance/securities/exchanges, entity pages to JapanFG, and ABS to structured-finance; no files moved. (C) Subdivided the flat 631-entry JapanFG domain with 9 institution-type sub-indexes driven by frontmatter tags (megabanks-and-fg / regional-banks / cooperative-finance / trust / insurance / securities-and-asset-management / payments-cards-leasing-finance / foreign-institutions / regulators-sro-policy; 609 entries grouped), mirroring the legal-financial-licenses subdomain pattern with a Placement Rule, plus a type-navigation block in JapanFG/INDEX — zero file moves (Option A), preserving 53,615+ inbound links / URLs / sitemap. (D) Landed canonical_anchor Phase 0: added the optional `canonical_anchor` field to SCHEMA (Optional/Legacy + key order after `related`), set it as a pilot on the Saison and Toyota Financial Services mirror pairs, and marked the proposal active / Phase-0-adopted; Phase 1 audit tooling is future work, tools/*.ts untouched. Verification: `release.ts --check --strict` EXIT=0, audit PASS (entries=1420 / issues=0, every sub-index wikilink resolves, zero dead links), counts in sync; corpus md 1473→1482, entries 1411→1420, domains 23.
- **中文**: 用 3 个并行 write subagent（独立文件域）落地结构改善 B/C/D。(B) 给 mislabel 的 governance（非营利/公益法人）和 manufacturing（厂商金融）INDEX 加 scope/disambiguation，把 corporate governance 路由到 finance/securities/exchanges、entity 页到 JapanFG、ABS 到 structured-finance；不移文件。(C) 用 frontmatter tags 给扁平的 631-entry JapanFG 新建 9 个机构类型 sub-index（megabanks-and-fg / regional-banks / cooperative-finance / trust / insurance / securities-and-asset-management / payments-cards-leasing-finance / foreign-institutions / regulators-sro-policy，分组 609 个 entry），仿 legal-financial-licenses subdomain 模式并加 Placement Rule，JapanFG/INDEX 加类型导航块 —— 零文件移动（Option A），保全 53,615+ inbound link / URL / sitemap。(D) 落地 canonical_anchor Phase 0：把 optional `canonical_anchor` 字段加入 SCHEMA（Optional/Legacy + key order 在 related 后），在 Saison 与 Toyota Financial Services mirror pair 设 pilot，proposal 标为 active / Phase-0-adopted；Phase 1 audit tooling 留作后续，tools/*.ts 未动。验证：`release.ts --check --strict` EXIT=0、audit PASS（entries=1420 / issues=0、每个 sub-index wikilink 都 resolve、零死链）、counts in sync；corpus md 1473→1482、entries 1411→1420、domains 23。

### 全領域 INDEX 同期と meta 文書整理 / Domain-INDEX sync and meta-doc cleanup / 全领域 INDEX 同步与 meta 文档整理
#### 日本語記録 / English / 中文
- **JST 時刻**: 2026-06-03 16:25 JST。
- **背景**: 内容品質（INDEX 全領域カバー / frontmatter 0 null / link audit 0 issue / review 0 overdue）は満点だったが、2026-05-25 の内容拡充以降、多くの領域 INDEX が新 entry を反映せず stale だった。並列 subagent（read-only 10 件 + write 8 件）で全領域を棚卸しし、客観的に修正可能な整理項目を落地した。
- **範囲**: 8 領域 INDEX（corporate-strategy / loyalty / money-market / manufacturing / retail / governance / trade / security）、`INDEX.md`、`SCHEMA.md`、`HOW-TO-NAVIGATE.md`、`cross-domain-anchor-convention.md`、`frontmatter-canonical-anchor-field-proposal.md`、retail entry 3 件、再生成された discovery surface。wiki 本文への新規事実追加は行わない（整理のみ）。
- **主要変更**: (1) 8 領域 INDEX を実際の entry へ同期（漏れていた entry を追加。例: manufacturing 2→7、loyalty 4→11、money-market routing 6→11。`single entry`/`two entries` 等の古い記述を実数へ修正、stale backlog を整理）。(2) Bun 移行後に残っていた `tools/*.py` 参照を `SCHEMA.md`・`INDEX.md`（3 箇所）・`cross-domain-anchor-convention.md`・`frontmatter-canonical-anchor-field-proposal.md` で `tools/*.ts`（`bun` 実行）へ修正（CHANGELOG/releases の歴史記録は史実として保持）。(3) `HOW-TO-NAVIGATE.md` の count drift（1465+/24+ → 1,400+/23）を是正。(4) 孤立していた 5 件の convention/proposal 文書を `INDEX.md` Control Documents に収録。(5) security INDEX「Status」列が confidence 値を表示していた問題を是正、未作成 backlog 4 件を planned と明記。(6) Lawson TOB 価格の不整合（3 ページ ¥4,952 / 1 ページ ¥10,360）を公開開示（KDDI/Lawson、web 照合）に基づく ¥10,360 へ統一。
- **実行手順**: read-only subagent 10 件で方向 2/3/4 を並列棚卸し → 客観可修項を抽出 → write subagent 8 件で各領域 INDEX を並列同期（各自 1 領域のみ編集）→ 全局 meta 文書・count・Lawson 価格を手動修正 → 死リンク疑義は alias 機構を確認し誤検出と判定（修正不要）。
- **検証結果**: `bun tools/release.ts --check --strict` は EXIT=0、link audit entries=1411 / issues=0 → PASS（8 領域 INDEX が追加した wikilink は全て resolve、wiki 本文死リンク 0）、counts in sync、JSON/LF/duplicate id verify OK。corpus は md=1472 / domains=23 / entries=1411 を維持。
- **既知の注意点**: 領域名 mislabel（governance=非営利、manufacturing=manufacturer-finance）、各領域の内容拡充、JapanFG 631 件の sub-INDEX 細分（subagent は file を移さない Option A を推奨）、canonical_anchor proposal の落地は本整理の範囲外で後続開発タスクとする。retail/manufacturing の「死リンク」は explicit alias で resolve 済みのため未修正。
- **次の作業**: push 後に remote HEAD、GitHub Actions、GitHub Release、公開 INDEX / 各領域 INDEX を確認。後続で内容拡充・領域再定位・JapanFG sub-INDEX を順次検討。
- **EN**: Audited all domains with parallel subagents (10 read-only for discovery, 8 write for INDEX sync) and landed the objectively-fixable cleanup. Synced 8 stale domain INDEXes to their real entries (e.g. manufacturing 2→7, loyalty 4→11, money-market routing 6→11), fixing stale "single/two entries" wording and stale backlogs. Corrected lingering post-Bun-migration `tools/*.py` references to `.ts` in SCHEMA / INDEX / two convention-proposal docs (CHANGELOG and releases kept as historical record). Fixed the HOW-TO-NAVIGATE count drift (1465+/24+ → 1,400+/23). Added the 5 orphaned convention/proposal docs to the INDEX Control Documents. Relabeled the security INDEX "Status" column (it showed confidence) and marked its 4 unbuilt backlog pages as planned. Unified the Lawson TOB price to the publicly-disclosed ¥10,360 (web-verified) across 3 pages, removing the erroneous ¥4,952. The reported "dead links" were verified to resolve via explicit aliases (false positives, not changed). Verification: `release.ts --check --strict` EXIT=0, audit PASS (entries=1411 / issues=0, zero dead links), counts in sync.
- **中文**: 用并行 subagent（10 个 read-only 盘点 + 8 个 write 同步）盘点全部领域并落地客观可修项。把 8 个 stale 领域 INDEX 同步到实际 entry（如 manufacturing 2→7、loyalty 4→11、money-market routing 6→11），修正 "single/two entries" 过时表述与 stale backlog。把 SCHEMA / INDEX / 两个 convention-proposal 文档中遗留的 `tools/*.py` 改为 `.ts`（CHANGELOG 与 releases 作为史实保留）。纠正 HOW-TO-NAVIGATE 的 count drift（1465+/24+ → 1,400+/23）。把 5 个孤立 convention/proposal 文档收录进 INDEX Control Documents。修正 security INDEX 的 "Status" 列（实为 confidence），并把 4 个未建 backlog 标为 planned。依据公开披露（web 核实）把 Lawson TOB 价格在 3 个页面统一为 ¥10,360，清除错误的 ¥4,952。报告的"死链"经核实是 explicit alias 解析的误报（未改）。验证：`release.ts --check --strict` EXIT=0、audit PASS（entries=1411/issues=0、零死链）、counts in sync。

### AI タスク入口の死リンク修正とスナップショット同期 / AI task-entry dead-link repair and snapshot sync / AI 任务入口死链修复与快照同步
#### 日本語記録 / English / 中文
- **JST 時刻**: 2026-06-03 14:41 JST。
- **背景**: 手動管理の `llms-tasks.txt` は AI / browse tool 向けの task 指向入口だが、last refresh が 2026-05-25 JST のまま取り残されていた。自動生成される他の AI discovery surface（`llms.txt` / `llms-full.txt` / `ai-index.json` / `sitemap.xml` / `index.html`）は 2026-06-03 の現行スナップショットへ更新済みなのに対し、`llms-tasks.txt` だけが唯一 stale だった。`.txt` は `tools/wiki_link_audit.ts`（wiki 本文 `.md` のみ対象）のスコープ外のため、entry の改名・再編で生じた死リンクが audit に検出されず潜在していた。
- **範囲**: `llms-tasks.txt`、`README.md`、`CHANGELOG.md`、`index.html`、`releases/v2026.06.03-5.md` と、再生成された discovery surface（`sitemap.xml`、`ai-index.json`、`llms.txt`、`llms-full.txt`、`api/entries/index.json`、`api/entries/systems/cross-chain-five-pole-comparison-matrix.json`）、`wiki-link-improvement-plan.md`。計 12 ファイル。wiki 本文、i18n entry、site UI 実装は変更しない。
- **主要変更**: `llms-tasks.txt` の死リンク 10 件を現存 entry へ再マッピングした: `money-market/boj-open-market-operations-overview`→`boj-open-market-operations`、`money-market/japan-money-market-structure`→`japan-money-market`、`money-market/japan-call-money-market`→`call-market-structure`、`money-market/japan-jgb-repo-securities-financing`→`jgb-repo-market-japan`、`policy-finance/hokkaido-credit-guarantee-corporation`→`hokkaido-credit-guarantee-corp`、`securities/japan-pts-liquidity-data`→`japan-pts-liquidity-data-guide`、`payments/japan-cashless-payment-market-overview`→`cashless-jp-landscape`、`derivatives/japan-fx-options-otc-market`→`fx-options-japan-corporate-treasury`、`derivatives/jscc-otc-clearing-infrastructure`→`otc-clearing-jp-trade-repository`、`systems/hyperlane-permissionless-interop-overview`→`hyperlane-overview`。Snapshot 行を `1,449 files / ~10.43M chars / 2026-05-25 JST` から `1,472 files / 1,411 link-audited entries / 23 domains / ~9.99M non-space chars / 2026-06-03 JST` へ更新した。`releases/v2026.06.03-5.md` の追加で corpus が 1,471 から 1,472 files になったため discovery surface を再生成し、`README.md` / `index.html` / `ai-index.json` / `llms.txt` / `llms-full.txt` の counts を同期した。再生成では `systems/cross-chain-five-pole-comparison-matrix` の壊れていた frontmatter（全 null）も正しく再解析された。`README.md` には三語で AI 入口 note を追加した。
- **実行手順**: `llms-tasks.txt` の全 link target 160 件を抽出し、現存 `.md` / index と照合して死リンク 10 件を特定、対象 domain の現存ファイルと frontmatter title で再マッピング先を確定した。fresh clone は fs mtime を reset し、`generate_ai_discovery.ts` は mtime から lastmod を生成するため、commit 済み `sitemap.xml` の lastmod を真値として各ページの mtime を復元し、当日編集した `README.md` / `CHANGELOG.md` と新規 `releases/v2026.06.03-5.md` のみ 2026-06-03 とした。その後 `bun tools/release.ts --write`（wiki_link_audit + generate_ai_discovery + footer timestamp + README/index.html counts 同期）を実行した。
- **検証結果**: link target 再照合は unique=160 / missing=0。`bun tools/release.ts --check --strict` は EXIT=0、link audit entries=1411 / issues=0 → PASS、canonical counts md=1472 / domains=23 / entries=1411 / issues=0、counts in sync、JSON valid / LF endings / duplicate id verify OK。mtime 復元により既存 1,469 ページの sitemap lastmod は不変で、変更は 12 ファイル（`sitemap.xml` は v2026.06.03-5 の URL 追加のみ）。
- **既知の注意点**: corpus は release notes 追加で 1,471 → 1,472 files、領域数 23 は不変。`.txt` は wiki link audit の対象外のため、今後 entry を改名・再編する際は `llms-tasks.txt` の link も併せて更新する必要がある。clone 環境では mtime 復元を行わないと discovery surface の lastmod が全件 clone 日付へ汚染される点に注意する。
- **次の作業**: push 後に remote HEAD、GitHub Actions、GitHub Release（同 tag）、公開 `/llms-tasks.txt` を確認する。将来的には `.txt` 系入口の link target 実在性も release gate に組み込み、`.md` 以外の AI surface でも死リンクを自動検出することを検討する。
- **EN**: Repaired 10 latent dead links in the manually maintained `llms-tasks.txt` (renamed/reorganized entries across money-market / derivatives / payments / policy-finance / securities / systems) and updated its snapshot line to `1,472 files / 1,411 link-audited entries / 23 domains / ~9.99M non-space chars` (2026-06-03 JST). Adding `releases/v2026.06.03-5.md` moves the corpus from 1,471 to 1,472 files, so discovery surfaces were regenerated and README / index.html / ai-index / llms counts synced. Because a fresh clone resets fs mtimes (which drive sitemap/api lastmod), page mtimes were first restored from the committed `sitemap.xml` lastmod values so the 1,469 existing pages keep their lastmod; only README / CHANGELOG / the new release note are dated 2026-06-03. Regeneration also repaired the broken (all-null) frontmatter of `systems/cross-chain-five-pole-comparison-matrix`. Verification: link targets 160 / 0 missing; `release.ts --check --strict` EXIT=0, audit PASS (entries=1411 / issues=0), counts in sync; 12 files changed.
- **中文**: 修复手动维护的 `llms-tasks.txt` 中 10 个潜伏死链（money-market / derivatives / payments / policy-finance / securities / systems 下被改名 / 重组的 entry），并把其 snapshot 行更新为 `1,472 files / 1,411 link-audited entries / 23 domains / ~9.99M non-space chars`（2026-06-03 JST）。新增 `releases/v2026.06.03-5.md` 使 corpus 从 1,471 增至 1,472 files，因此重新生成 discovery surface 并同步 README / index.html / ai-index / llms 的 counts。由于 fresh clone 会重置 fs mtime（sitemap/api lastmod 依赖它），本次先从已提交 `sitemap.xml` 的 lastmod 恢复各页面 mtime，使 1,469 个既有页面 lastmod 不变，仅 README / CHANGELOG / 新 release note 标记为 2026-06-03。重新生成还修复了 `systems/cross-chain-five-pole-comparison-matrix` 损坏的（全 null）frontmatter。验证：link target 160 / 0 missing；`release.ts --check --strict` EXIT=0、audit PASS（entries=1411 / issues=0）、counts in sync；12 文件变更。
...
#### 日本語記録 / English / 中文
- **JST 時刻**: 2026-06-02 18:19 JST。
- **背景**: `finwiki.zksc.io` を GitHub Pages と Vercel の両方で同じ静的成果物として扱うため、Astro 側が `site/src/content/entries` mirror に依存する構造を整理した。mirror sync は内容重複と route drift の温床になりやすく、frontmatter `slug` が Astro collection `id` に混入した場合に flat route や偽 domain route を作るリスクもあった。
- **範囲**: `site/src/content.config.ts`、`site/scripts/*translate*.mjs`、`site/scripts/sync-corpus.ts`、`site/scripts/corpus-roots.mjs`、`site/src/lib/routes.ts`、`site/src/lib/siteIndex.mjs`、`site/src/plugins/remark-wikilink.mjs`、`site/src/plugins/localize-wikilinks.mjs`、`site/astro.config.mjs`、`site/src/pages/[lang]/**`、`site/src/layouts/*`、`.github/workflows/deploy.yml`、`tools/vercel_build.ts`、`README.md`、`CHANGELOG.md`、`docs/04-architecture/astro-site-architecture.md`、`releases/v2026.06.02-8.md`。
- **主要変更**: Astro content loader は root domain corpus を `glob({ base: '../' })` で直接読む。`site/src/content/entries` は deprecated generated mirror とし、build input と CI / Vercel build step から外した。`entryRoute()` / `entryDomain()` / `i18nEntryId()` を追加し、frontmatter `slug` 由来の route collapse を防ぐ。`/[lang]/domains/` と `/[lang]/domains/[domain]/` の landing pages を追加し、domain navigation を entry route helper に統一した。`remark-wikilink` は href を静的生成し、build hook `localize-wikilinks` が en / zh HTML の wikilink href を言語別 URL に置換する。
- **実行手順**: content loader と translate prep scripts を root corpus walker へ移行した。entry pages、browse、home、domain pages、layout navigation を canonical route helper へ接続した。Vercel / Pages build から corpus sync step を削除した。wikilink plugin と build hook を入れ、client-side href rewrite を撤去した。最後に AI discovery / release surfaces を同じ Bun pipeline で再生成する。
- **検証結果**: `bun x tsc --noEmit --pretty false` は PASS。`bun run build` は 4219 pages を生成し、domain landing は ja / en / zh 各 23 domains。`zh/bytecode-forensic-three-tier-verify/` は生成されず、`zh/security/bytecode-forensic-three-tier-verify/` は生成されることを確認した。sample zh / en pages に `class="wl" href="/ja/` は残らず、href なし wikilink も 0 件。`bun tools/wiki_link_audit.ts --report wiki-link-improvement-plan.md --fail-on-issues` は entries_checked=1411 / entries_with_issues=0 / dead_wikilink_references=0 / dead_wikilink_targets=0。`bun tools/vercel_build.ts` は PASS し、Astro 4219 pages、duplicate_ids=0、Pagefind 4219 pages / 3 languages / 132788 words、static assembly astro_files=8846 / root_surface_files=2838 を確認した。
- **既知の注意点**: 過去の ignored worktree には `site/src/content/entries/` が残る場合があるが、現在の build input ではない。過去の CHANGELOG / release notes には当時実行した sync / Python / Node command が historical evidence として残る。
- **次の作業**: push 後に remote HEAD、GitHub Release、GitHub Actions deployment、public URL の `/`、`/zh/domains/exchanges/`、代表 entry、`/llms.txt`、`/pagefind/pagefind.js` を確認する。Vercel project 設定後は preview URL でも同じ `bun tools/vercel_build.ts` 成果物を確認してから DNS cutover を行う。
- **EN**: Refactored FinWiki's static architecture so Astro reads the canonical root domain corpus directly instead of a generated `site/src/content/entries` mirror. Added canonical route helpers to prevent frontmatter `slug` from collapsing nested routes, generated language-scoped domain landing pages, removed the corpus sync step from Pages / Vercel builds, and moved wikilink href localization into the build output. Validation passed through TypeScript, Astro 4219-page build, 23 domain landings per language, dead wikilink audit 1411/0, localized en / zh wikilinks, duplicate_ids=0, Pagefind 4219 pages / 132788 words, and static publish assembly.
- **中文**: 完成静态站点架构 Phase 1。Astro 现在直接读取 root domain corpus，不再把 `site/src/content/entries` mirror 当作构建输入；路由统一通过 `entryRoute()` / `entryDomain()` / `i18nEntryId()` 生成，避免 frontmatter `slug` 把嵌套路由压平成错误路径；新增三语 domain landing pages；CI / Vercel 构建移除 corpus sync；wikilink href 在构建产物阶段本地化。验证通过 TypeScript、Astro 4219 pages、每种语言 23 个 domain landing、wikilink audit 1411/0、en / zh wikilink 无 `/ja/` 残留、duplicate_ids=0、Pagefind 4219 pages / 132788 words 与静态发布组装。
...
- Remote repository: `origin repository`
