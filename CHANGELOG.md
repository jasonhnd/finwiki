...
### Entity graph Packet 3 - objective typed edges wave 1 (#38) / Add public-source-backed typed edges / 公开来源类型化边第一波

#### 日本語 / English / 中文
- **JST time**: 2026-06-22 12:38 JST.
- **Background**: After #36 added the entity graph infrastructure and #37 backfilled 89 Tier-1 `entity_node` anchors, #38 implements Packet 3 / Phase D from `docs/04-architecture/canonical-entity-graph-design.md`: the first objective relationship-edge wave.
- **Scope**: 14 selected source pages under `megabanks/`, `securities-firms/`, `asset-managers/`, `non-life-insurers/`, and `payment-firms/`; `CHANGELOG.md`; and generated release surfaces from `bun tools/release.ts --write`.
- **Main changes**: Added 34 declared `entity_edges` in frontmatter only. The wave uses only `subsidiary_of`, `regulated_by`, and `member_of_sro`; every edge includes `target`, `source`, `as_of`, and `confidence`; all targets resolve to existing FinWiki routes; deterministic inverse edges are generated by tooling.
- **Boundary**: No `competes_with`, peer, alliance, or soft relationship edges were added. No `entity_node` scope expansion, no body edits, and no `CONTRIBUTING.md`.
- **Validation**: `bun run entity:audit`, `bun tools/wiki_link_audit.ts --fail-on-issues`, `bun tools/release.ts --check --strict`, `bun run surface:drift`, and `git diff --check` were used as the #38 gate bundle, with `entity_edges_declared=34`, `entity_edges_derived=34`, and `entity_graph_issues=0`.
- **Next work**: Stop after the #38 PR for maintainer review and merge; do not start any later entity graph or rendering work until instructed.
