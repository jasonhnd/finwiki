  ...
  for (const entry of entries) {
    ...
    const record = {
      ...
      frontmatter: {
        ...
        aliases: stringArray(fm.aliases),
        domain: stringOrNull(fm.domain),
        created: stringOrNull(fm.created),
        last_updated: stringOrNull(fm.last_updated),
        last_tended: stringOrNull(fm.last_tended),
        review_by: stringOrNull(fm.review_by),
        confidence: stringOrNull(fm.confidence),
        tags: stringArray(fm.tags),
        status: stringOrNull(fm.status),
        sources: stringArray(fm.sources),
        canonical_anchor: stringOrNull(fm.canonical_anchor),
        related: stringArray(fm.related),
        note: stringOrNull(fm.note),
        type: stringOrNull(fm.type),
        entity_node: entry.entity_node,
        entity_edges: entry.entity_edges,
      },
