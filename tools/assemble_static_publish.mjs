...
async function copyTree(src, dest, { ignoreExisting = false } = {}) {
  let files = 0;
  for (const entry of await readdir(src, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      ...
      files += await copyTree(from, to, { ignoreExisting });
    } else if (entry.isFile()) {
      if (ignoreExisting && await exists(to)) continue;
...
async function copyRootSurface(outDir) {
  let files = 0;
  for (const entry of await readdir(ROOT, { withFileTypes: true })) {
    if (entry.name === 'index.html') continue;
    if (entry.isDirectory()) {
      if (SKIP_DIRS.has(entry.name) || entry.name === path.basename(outDir)) continue;
