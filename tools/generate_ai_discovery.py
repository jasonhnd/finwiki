...
def iter_markdown_files() -> Iterable[Path]:
    for path in ROOT.rglob("*.md"):
        ...
        if ".git" in rel.parts or ".opinions" in rel.parts:
            continue
        yield path
...
def write_robots(model: dict[str, object]) -> None:
    ...
    # Explicit allowlist for major AI / LLM crawlers. These are already covered
    # by `User-agent: *` Allow, but listing them by name is an explicit signal
    # that this site is intended for AI consumption and a courtesy to operators
    # who treat per-bot blocks differently from a wildcard.
    ai_bots = [
        "GPTBot",
        "ChatGPT-User",
        "OAI-SearchBot",
    ...
    lines.extend(
        [
            ...
            f"# LLM task guide: {SITE_URL}llms-tasks.txt",
            f"# LLM full manifest: {nav['llms_full']}",
            f"# Machine index: {nav['ai_index_json']}",
            f"# Per-entry JSON API: {SITE_URL}api/entries/index.json",
            "",
        ]
    )
...
def write_llms(model: dict[str, object]) -> None:
    ...
    lines = [
        ...
        f"- Task-oriented LLM entry guide (16 research tasks): {SITE_URL}llms-tasks.txt",
        f"- Machine JSON index: {nav['ai_index_json']}",
        f"- Per-entry JSON API (1 file per entry, frontmatter + summary + links + inbound count): {SITE_URL}api/entries/index.json",
        f"- Full LLM page manifest: {nav['llms_full']}",
        f"- Reader-profile navigation guide: {SITE_URL}HOW-TO-NAVIGATE",
        f"- Topic cluster reference (14 thematic clusters): {SITE_URL}topic-cluster-reference",
        f"- Domain bridge navigation guide (14 reader journeys): {SITE_URL}domain-bridge-navigation-guide",
