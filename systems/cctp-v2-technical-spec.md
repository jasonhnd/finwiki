...
**与 §501 denylist 的集成**:Attestation Service 在签名前查询 Circle 的 denylist——若 recipient 或 source 地址在 list,拒绝签名,USDC 在源链已 burn 但目标链无法 mint。**这是 protocol-level 的合规执行通道**(对照 [[fintech/usd-stablecoin-interchange|USD 稳定币跨链互换]] 的合规节点设计)。
...
Hooks 的设计灵感部分来自 [[systems/erc-4337-overview|ERC-4337]] 的 UserOp 模型——把"链 A 操作 + 链 B 操作"打包成原子语义,降低 UX 复杂度。
...
- CCTP V2 是否会引入 stake-based challenger 增加去中心化?(对比 [[systems/cross-chain-five-pole-comparison-matrix|跨链五极对比矩阵]] 中各方案的信任假设)
