...
5. **文档**：ADR-008（推翻 ADR-004 的理由 + 40 领域）、roadmap/backlog/domains 更新、日文→英文的 README/CHANGELOG/release notes。
...
按 `AGENTS.md` 与当前发布规则：同 session 内按日文→英文更新 README + CHANGELOG，运行 `bun run release:docs` 和 discovery 生成，写日英 `releases/vX.md`，push，更新同顺序的 GitHub Release。本设计文档在 `docs/`（corpus 排除），不计入公开计数；人类站点仍只支持 ja/en。
