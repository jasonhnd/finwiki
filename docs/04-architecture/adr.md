...
- **影响**：确立并行开发标准流程（见 [parallel-development.md](../06-implementation/parallel-development.md)）。内容由 agent 生成 → 结构 / 链接可 audit，但**事实精度需人工抽查**。rate-limit + 重试可能产生双批 entry（v12 的 5 领域各得 6 个互补 entry），Phase 2 收尾必须按 **disk 实数**对账 root INDEX。
...
- **决定**：选 **B**。`tools/wiki_link_audit.ts` 加 `--fail-on-canonical-drift`（drift>0 → EXIT=1），`tools/release.ts` 的 `runWikiLinkAudit` 始终带此 flag → 每次 `--check` / `--write` 都 gate `canonical_anchor_drift=0`。「mirror 必填」写进 [`SCHEMA.md`](../../SCHEMA.md) + [entry-authoring.md](../06-implementation/entry-authoring.md)，靠 review 兜底。
...
## ADR-009：模型分工采用规格优先 + 代码任务包

- **背景**：后续开发目标是让高推理规格模型写 BRD/PRD/ARD/FSD/NFR/RTM 和验收文档，让快速代码实现模型写代码。若代码 agent 同时解释需求、改规格、改实现，容易造成 scope 扩张和反向修改验收口径。
- **选项**：(A) 所有 agent 都可自由改需求和代码；(B) 高推理模型先写规格与 task packet，代码模型只改 allowed files；(C) 主会话不拆分，全部手写。
