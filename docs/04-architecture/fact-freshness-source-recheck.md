...
Future implementation should mirror `i18n:status`:

```bash
bun tools/fact_freshness_status.ts
bun tools/fact_freshness_status.ts --json
bun tools/fact_freshness_status.ts --due-only
bun tools/fact_freshness_status.ts --as-of 2026-06-20
bun tools/fact_freshness_status.ts --fail-on-due
```

Default mode should be report-only and exit 0. CI mode can use `--fail-on-due` later, but should not be wired into the release gate until the team has triaged the first report and chosen acceptable thresholds.
...
ALLOWED FILES:
- tools/fact_freshness_status.ts
- package.json
...
- `--due-only` filters to due rows.
- `--as-of YYYY-MM-DD` makes tests deterministic.
- `--fail-on-due` exits non-zero only when explicitly requested.
...
- It has at least one deterministic negative fixture or temporary seeded case proving non-zero exit with `--fail-on-due`.

VALIDATION:
- `bun tools/fact_freshness_status.ts --as-of 2026-06-20`
- `bun tools/fact_freshness_status.ts --json --as-of 2026-06-20`
