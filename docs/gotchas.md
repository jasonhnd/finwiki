...
`EXCLUDED_WALK_DIRS`（`lib/markdown_helpers.ts`，控计数/发现面）≠ `IGNORED_DIRS`（`tools/wiki_link_audit.ts`，控死链审计）。想让一个目录**完全内部**（如 `docs/`），两个都要加；只加一个会"漏"——比如只加 IGNORED_DIRS，文件仍会进 sitemap 计数。详见 [architecture.md](architecture.md#-双排除机制最易混淆)。
