...
| BRD-003 | [BRD](../01-strategy/brd.md) | `README.md`, `CHANGELOG.md`, `releases/`, `tools/release.ts`, `tools/release_documentation_audit.ts` | `bun run release:docs`, `release.ts --write`, `--check --strict` | release note |
...
| NFR-008 | [NFR](nfr.md) | release docs, `tools/release_documentation_audit.ts`, and GitHub Release workflow | `bun run release:docs` + status + remote HEAD after push | release note |
...
| FSD-008 | [FSD](../05-functional-specs/fsd.md), [UI/UX](../05-functional-specs/ui-ux.md), [Entry Discovery Build Graph](../04-architecture/entry-discovery-build-graph.md), [Visual QA Checklist](../07-quality/visual-qa-checklist.md) | `site/src/layouts/Base.astro`, `site/src/layouts/EntryLayout.astro`, `site/src/lib/entryDiscovery.mjs`, `site/src/styles/global.css`, `site/src/pages/**`, `site/src/i18n/ui.ts` | focused discovery cache/parity tests + visual QA + Astro build when changed | release note |
