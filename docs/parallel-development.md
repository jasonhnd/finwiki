...
> 怎么用并行 subagent 高效、安全地开发 FinWiki。沉淀自 v12（10 领域并行 +46 entry）与 v13（canonical_anchor back-fill）的实战。配套 [entry-authoring.md](entry-authoring.md)（怎么写单个 entry）、[release-process.md](release-process.md)（收尾发布）。
...
- **并发上限 ~5-6**。v12 一次发 9 个 agent（每个含 WebSearch）→ **5 个撞 API rate-limit**（`Server is temporarily limiting requests · not your usage limit`）。
- **分批节奏**：① 探路 1 个确认路径通 → ② 批量但每批 ≤ 5-6 → ③ rate-limited 的降并发重试。
...
2. **LEARN FIRST**：读 `SCHEMA.md` + 该领域 `INDEX.md` + 2-3 样例 entry + `docs/entry-authoring.md`。
...
5. 三语 README/CHANGELOG note + `releases/vX.md`（见 [release-process.md](release-process.md)）。
6. commit / push / tag / `gh release` + `gh run watch`。**commit 要单独跑、message 别含 `verif` 字样**（见 [gotchas.md](gotchas.md) block-no-verify 项）。
