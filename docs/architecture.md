...
```
finwiki/
├── <domain>/                 # 23 个领域目录，每个含 INDEX.md + 若干 entry .md
│   └── INDEX.md              # 领域索引（domain_index）
├── JapanFG/                  # 最大领域（600+ entity），含 9 个 tag 驱动 sub-INDEX
...
├── api/                      # generate_ai_discovery 产出的 JSON API
...
**`entries` 计数 = entry_type 为 `wiki_entry` 的文件数**（当前 1420）。`md` 计数 = 全部 corpus `.md`（当前 1485，含 control/release/index）。`domains` = `INDEX.md` 表里的领域行数（23）。

## domain 体系

- 领域 = 顶层目录名。`INDEX.md` 的 markdown 表（`parseDomainMap()` 解析）是**领域的唯一真相源**：每行 `| domain | count | [[domain/INDEX]] | scope |`。
- 新增/改名领域要同步：目录本身、`INDEX.md` 表、`site/` 三处 allowlist（content.config.ts / siteIndex.mjs / groups.ts）、`tools/wiki_link_audit.ts` 的 per-domain suggested-link map。详见 [gotchas.md](gotchas.md) 与 CHANGELOG 中 v2026.06.03-9（governance→non-profit、manufacturing→manufacturer-finance 物理改名）。
...
4. 走 [release-process.md](release-process.md)。
