...
| Root entry | `site/src/pages/index.astro` renders `.root-masthead`, `.root-entry-paths`, optional `.start-here`, `.root-proof`, and `.root-ai`. | FinWiki masthead and bilingual language entries lead; start-here is the same edited set as `/{lang}/`; corpus stats and AI links stay below. |
| Localized home | `[lang]/index.astro` renders `.home-hero` as a two-column composition: `.home-hero__intro` (headline + search) facing `.start-here`; `.home-proof`, `.review-strip`, `.taxonomy-grid`, `.anchor-grid`, and `.ai-band` stay below the fold. | First screen is one sentence, search, and at most 5–7 edited start-here links filling the facing column. The 40-domain map, latest-touched pages, same-entity pairs, and AI links must not compete with that guide. |
| Domains | `domains/index.astro` uses `.domains__head`, `.domain-section`, `.domain-card`; `domains/[domain]/index.astro` uses `.domain-opener`, a single-column `.domain-brief` read-first list, optional search filter, a one-column entry list, and same-entity anchors after the list. | Grouped taxonomy, localized names/descriptions, muted counts, numbered read-first, no 3-column dashboard brief, filter only when useful. |
| Browse | `browse/index.astro` uses sticky `.browse__bar`, `.browse__sort`, reader-facing `.browse-result` rows with one-line leads, optional update metadata, and client-side filter/sort. Domain jump chips remain available as a secondary aid in domain sort. Route slugs stay on the title `title` attribute, not as a competing visual row. | Full-corpus scan, recommended default sorting, localized sort/filter labels, one-line leads (not INDEX-positioning dumps), localized empty state, no facets in C4, and no page-level overflow. |
...
| ED-002 | Root language entrance | `site/src/pages/index.astro` | `/` must lead with `.root-masthead` and bilingual `.root-entry-paths`. The same edited start-here set as `/{lang}/` may follow. Counts and machine-readable links remain below that entrance, not the first-screen message. |
| ED-003 | Localized start-here list | `site/src/pages/[lang]/index.astro` | `/{lang}/` first screen must show `.start-here` with at most 5–7 edited links. The default set is megabanks, FSA, the banking-license matrix, payment clearing infrastructure, the market-infrastructure map, and the all-domains index. Links are editorial, not “recently touched” files. Omit a link if its target is missing; do not substitute a latest-updated page. |
| ED-004 | Home catalog stays below the fold | `site/src/pages/[lang]/index.astro` | `.home-proof`, `.review-strip`, `.taxonomy-grid`, same-entity `.anchor-grid`, and `.ai-band` may remain on the page, but not in the first screen. Do not restore a `.canonical-strip` that duplicates the start-here set. |
| ED-005 | Guided domain taxonomy | `site/src/pages/[lang]/index.astro`, `domains/index.astro`, `site/src/i18n/groups.ts`, `site/src/i18n/domains.ts` | Domain-list taxonomy must use localized group/domain descriptions. Counts are secondary badges. On home, the taxonomy grid is a below-fold catalog, not the first-screen map. |
| ED-006 | Domain briefings | `site/src/pages/[lang]/domains/[domain]/index.astro` | Domain detail pages must lead with `.domain-opener` and a single-column `.domain-brief` numbered read-first list, then the entry list. Same-entity anchors sit after the list, not as a competing column. Megabanks read-first is MUFG, SMFG, and Mizuho FG when those routes exist. List excerpts use the entry lead (TL;DR / 要約 / Summary), not wiki-positioning sentences. Route slugs must not compete with titles. |
| ED-007 | Evidence strip | `site/src/layouts/EntryLayout.astro` | Entry pages must surface confidence, last updated, review-by when present, source count, machine-translation badge, and original-language link through `.evidence-strip` as one quiet line under the H1. Evidence is visible but quieter than H1 and 要約 / TL;DR. Tags and wiki-placement sit after the article. |
...
| FSD-008-005 | Root entry | `site/src/pages/index.astro` | Provide bilingual reader entry paths first. Optional start-here links, corpus proof, and AI/crawler links follow without becoming a marketing-only landing page. |
| FSD-008-006 | Localized home | `site/src/pages/[lang]/index.astro` | First screen is a two-column `.home-hero`: one editorial sentence and search facing `.start-here`. Corpus proof, latest-reviewed, the domain map, same-entity pairs, and AI metadata stay below the fold. |
| FSD-008-007 | Domain list | `site/src/pages/[lang]/domains/index.astro` | Group domains by product taxonomy and show localized group/domain names and descriptions plus secondary counts. Do not render English `group.id` as a visible eyebrow on Japanese pages. |
| FSD-008-008 | Domain detail | `site/src/pages/[lang]/domains/[domain]/index.astro` | Show breadcrumbs, domain opener, a numbered read-first list, counts, optional filter, a single-column entry list with titles and readable leads, and same-entity anchors after the list. Route slugs stay quiet citation metadata. Do not render a 3-column dashboard brief. |
| FSD-008-009 | Browse page | `site/src/pages/[lang]/browse/index.astro` | Provide full corpus scan with default `recommended` sorting, plus `recent`, `title`, and `domain` sort controls. Each `.browse-result` must show localized title, localized domain, optional `last_updated`, and a one-line lead. Route slugs stay on the title `title` attribute and must not compete visually. Keep the sticky filter and localized no-match state; keep domain jumps as a secondary aid for domain grouping; do not add facets in C4. |
| FSD-008-010 | Entry page | `EntryLayout.astro` | Show breadcrumb, title, a one-line evidence strip, then the article starting at 要約 / TL;DR. Machine translation badge stays in the evidence line. Tags and wiki-placement sit after the article, not above 要約. Current-group left domain rail, right TOC or mobile inline TOC remain. Wiki-placement headings are not chapter one and are not in the H2/H3 TOC. Discovery cards use readable leads; route slugs do not compete with titles. |
...
| FSD-008-014 | Article-end discovery | `site/src/pages/[lang]/[...slug].astro`, `site/src/layouts/EntryLayout.astro`, `site/src/lib/entryDiscovery.mjs`, `site/src/lib/reverseLinks.mjs`, `global.css` | Build one immutable route graph and reverse-link index from resolved wikilinks during static-path generation, then pass localized per-route lanes to `EntryLayout` without a per-page collection scan. The graph fingerprint must detect intermediate route, title, tag, and curated-link changes. Render `.entry-discovery` after `.prose` with related, read-next, and backlinks lanes capped to compact lists. Candidate de-duplication prioritizes explicit frontmatter `related`, backlinks, shared tags, and same-domain siblings, while read-next stays deterministic within the same domain. |
...
| `.root-masthead`, `.root-entry-paths`, `.root-entry`, `.start-here`, `.root-proof`, `.root-ai` | `index.astro` | Root bilingual entrance, edited start-here set, corpus proof, and technical links. |
| `.home-hero`, `.home-hero__intro`, `.home-search`, `.pf-hero`, `.start-here`, `.start-here__item`, `.home-proof`, `.home-block`, `.home-catalog`, `.review-strip`, `.review-card`, `.taxonomy-grid`, `.taxonomy-group`, `.anchor-grid`, `.ai-band` | `[lang]/index.astro` | Localized editorial home: two-column first-screen start-here guide, then below-fold catalog surfaces. |
...
| `.doc`, `.drail`, `.drail--left`, `.drail--right`, `.doc-article`, `.entry-head`, `.entry-lead`, `.evidence-strip`, `.evidence-item`, `.evidence-badge`, `.tagrow`, `.toc`, `.toc-inline`, `.wiki-placement__kicker`, `.wiki-placement__body` | `EntryLayout.astro`, quiet-wiki-placement plugin, `global.css` | Entry page editorial reference layout, summary lead, quiet wiki placement, evidence metadata, tags, and responsive rails. |
...
- Root start-here and technical links reduce below `720px`; root stats reduce below `560px`.
...
| Page | Required UX |
|---|---|
| `/` | Editorial bilingual entrance. Masthead and language paths lead. Start-here, corpus proof, and AI/crawler links stay below that entrance. |
| `/{lang}/` | Actual reader entry, not a marketing landing page. First viewport is headline, search, and start-here. Catalog surfaces stay below the fold. |
...
| `/{lang}/domains/{domain}/` | Domain opener and briefing lead; read-first is editorial where named; long lists use filter when needed; titles and leads outrank slugs. |
...
| `/{lang}/{domain}/{slug}/` | Title and summary lead. Wiki placement is a quiet note, not chapter one. Rails help orientation on desktop and collapse cleanly on mobile. |
...
- Root entry cards, start-here rows, home strips, taxonomy groups, domain cards, evidence strips, and route rows must wrap without clipping text.
