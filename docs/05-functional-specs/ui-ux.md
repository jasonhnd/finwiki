...
| Localized home | `[lang]/index.astro` renders `.home-hero`, `.home-search`, and `.start-here` in the first screen; `.home-proof`, `.review-strip`, `.taxonomy-grid`, `.anchor-grid`, and `.ai-band` stay below the fold. | First screen is one sentence, search, and at most 5–7 edited start-here links. The 40-domain map, latest-touched pages, same-entity pairs, and AI links must not compete with that guide. |
...
| ED-006 | Domain briefings | `site/src/pages/[lang]/domains/[domain]/index.astro` | Domain detail pages must lead with `.domain-opener` and `.domain-brief`: what the domain covers, an edited or fallback read-first list, optional same-entity anchors, then the entry list. Megabanks read-first is MUFG, SMFG, and Mizuho FG when those routes exist. List excerpts use the entry lead (TL;DR / 要約 / Summary), not wiki-positioning sentences. Route slugs must not compete with titles. |
...
| FSD-008-006 | Localized home | `site/src/pages/[lang]/index.astro` | First screen exposes one editorial sentence (headline), search, and `.start-here`. Corpus proof, latest-reviewed, the domain map, same-entity pairs, and AI metadata stay below the fold. |
...
| FSD-008-008 | Domain detail | `site/src/pages/[lang]/domains/[domain]/index.astro` | Show breadcrumbs, domain opener, coverage briefing, read-first links, optional same-entity anchors, counts, optional filter, entry titles, readable leads, and translated markers. Route slugs stay quiet citation metadata. |
...
| FSD-008-010 | Entry page | `EntryLayout.astro` | Show breadcrumb, title, a TL;DR / 要約 lead when one exists, confidence/review/source evidence strip, machine translation badge, tags, current-group left domain rail with All domains link, right TOC or mobile inline TOC, and prose. Wiki-placement headings are not chapter one and are not in the H2/H3 TOC. Discovery cards use readable leads; route slugs do not compete with titles. |
...
| `.home-hero`, `.home-search`, `.pf-hero`, `.start-here`, `.start-here__item`, `.home-proof`, `.home-block`, `.home-catalog`, `.review-strip`, `.review-card`, `.taxonomy-grid`, `.taxonomy-group`, `.anchor-grid`, `.ai-band` | `[lang]/index.astro` | Localized editorial home: first-screen start-here guide, then below-fold catalog surfaces. |
