...
- Per-entry API frontmatter includes safe existing source fields for agent routing: `canonical_anchor`, `related`, `note`, `type`, and `sources`, alongside the existing title / aliases / tags / lifecycle fields.
...
- Targeted JSON inspection for per-entry `frontmatter.canonical_anchor`, `frontmatter.related`, `frontmatter.note`, `frontmatter.type`, and `frontmatter.sources` when changing API enrichment.
