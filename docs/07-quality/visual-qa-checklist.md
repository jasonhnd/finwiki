...
- Domain detail pages use `.domain-opener` and `.domain-brief` before the entry list, with editorial or fallback read-first links. Titles and leads outrank route slugs.
...
| Domain detail | `ja`, `en` | `/{lang}/domains/megabanks/` and one high-count domain such as `/{lang}/domains/regional-banks/` | `.domain-opener` and `.domain-brief` appear before the entry list; megabanks read-first is MUFG / SMFG / Mizuho FG; list excerpts are leads, not INDEX-positioning sentences; titles outrank slugs; filter appears only when useful. |
| Long entry | `ja`, `en` | `/{lang}/megabanks/mufg/` and `/{lang}/banking/japan-regional-bank-m-a-consolidation-family-tree-matrix/` | H1 and `.entry-lead` come before wiki placement; TOC does not start with Wiki route / ウィキ上の位置づけ; `.evidence-strip`, tags, rails, tables, source rows, and discovery leads remain readable. Discovery cards do not lead with INDEX-positioning sentences or competing slugs. |
