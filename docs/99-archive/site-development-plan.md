...
2. **人类网站 = 全量自动渲染 + 全量 LLM 翻译** —— 每一页都走同一条管线：MD → 自动渲染成可读页 → LLM 翻成 `ja/en`。**没有精选层、不分质量层，一视同仁。** LLM 翻译让"全量双语"本就可行，不靠人工。
...
MD corpus (手写, 1000→10000)              ← 唯一事实源
  ├─[全量自动渲染 + 全量 LLM 翻译]──→ 人类网站（每页, ja/en, 保真）
  ├─[直接喂]────────────────────→ AI 内容 = MD 本身
...
**目标**：每一页双语可切换，统一质量，事实保真。

- **全量 LLM 翻译管线**：每页检测原文语言 → 翻成另一语（多数 ja → en）
...
**验收**：任意页可切双语；**抽查数字 / 专名 / 日期零误译**；切换无空白页；增量翻译可用。
