...
> 面向**维护者 / agent**：如何写一条高质量的 wiki entry。本文是 [system-architecture.md](../04-architecture/system-architecture.md) 「如何新增一个 wiki entry」一节的**操作化展开**，不重复其内容。
...
> 死链与 body-link 密度的最终裁判是 `bun tools/wiki_link_audit.ts`（经 `release.ts --check --strict`）；不要信裸 grep（见 [gotchas.md](../07-quality/gotchas.md#4-假死链中危)）。
...
| 字段 | 约定 |
|---|---|
| `title` | 机制/框架类用术语标题；案例类常用「实体 — 一句话定位」长标题（如 Aeon 案例）。日文术语条目可把术语放标题首（如 `合併 (Japan corporate merger) — …`）。 |
| `aliases` | 用 `[]` 表示无。日文实体**必须**含假名或汉字读法别名，否则 `[[别名]]` 无法解析。建议同时放 `domain/slug` 全路径与裸 `slug`。 |
| `domain` | 等于所在父目录名，**不可臆造**。领域是根 `INDEX.md` 表的唯一真相源（见 [System Architecture](../04-architecture/system-architecture.md)）。 |
...
`canonical_anchor` = 某个**跨领域实体或 mirror page** 的「单一真相源锚点条目」的 vault-root 路径（如 `non-life-insurers/saison-automobile-fire-insurance`）。

- **何时设（mirror page 必填 / hard requirement，Phase 4）**：同一实体在多个领域有页面时。**实体主页（anchor）不设** `canonical_anchor`；各**镜像/视角页**（domain-specific view）**必须**设 `canonical_anchor` 指向那个 anchor。这是 v2026.06.04-3 起的 hard requirement——新建 mirror page 不设会在 review 退回（mirror 与否是语义判断、无法机检，故靠写作规范 + review 兜底，依据见 [adr.md](../04-architecture/adr.md) ADR-007）。
  例：`insurance/saison-automobile-fire.md` 是 SOMPO Direct 的**保险领域视角页**，它 `canonical_anchor: non-life-insurers/saison-automobile-fire-insurance`（指向 non-life-insurers 实体 anchor）。
- **如何避免 drift（现为硬门禁）**：设了 `canonical_anchor` 后，**必须在 core body（通常 `## Wiki route`）里也用 `[[anchor|label]]` 显式交叉链接同一个 anchor**。canonical_anchor 审计会核对 frontmatter 锚点是否（a）resolve 到实在条目、（b）在正文里被链回——只写 frontmatter、正文不链，会被判 drift。**v2026.06.04-3 起 drift 是硬门禁**：`release.ts` 用 `--fail-on-canonical-drift` 跑审计，任何 `canonical_anchor_drift>0` 都会 `EXIT≠0` 阻断发布。saison 页的做法：frontmatter `canonical_anchor` + Wiki route 里 `The canonical entity anchor for this insurer is [[non-life-insurers/saison-automobile-fire-insurance|its non-life-insurers entity page]]`，两者一致。
- 普通单一领域条目**不需要** `canonical_anchor`，整行省略。

### `entity_node` / `entity_edges`——typed entity graph rails

这两个字段是可选的增量 graph rails，只在已批准的 entity-graph packet 中使用。

- `entity_node` 把页面标记为 typed graph node，必须包含 `kind`、`scope`、`status`。
- `entity_edges` 声明公开来源支撑的关系事实。Wave 1 只允许 `subsidiary_of`、`regulated_by`、`registered_as`、`holds_license`、`member_of_sro`、`predecessor_of`、`successor_of`。
- 每条 edge 必须包含 `target`、`source`、`as_of`、`confidence`；`target` 使用不带 `.md` 的 vault-root 路径。
- Packet 2 backfill 优先只改 frontmatter，避免 body hash 变化导致 i18n mirror 变 stale。
- 修改 graph 字段后必须运行 `bun run entity:audit`。
...
- **`## Wiki route`（开场必写）**：一段话，做两件事——(1) 用 `[[domain/INDEX|…]]` 声明本条所属领域路由；(2) 给出 peer/contrast 与 system/regulatory 的 `[[…|…]]` 链接，并（若是 mirror page）链回 `canonical_anchor`。这是满足 body-link 要求的主力段落。措辞模板见各最佳条目，如 TORF 的 “This entry sits under `[[money-market/INDEX|…]]`. Read it against … for … and … for the supervisory anchor `[[financial-regulators/fsa|…]]`.”
...
- **双语现实（bilingual reality）**：entry **只用一种语言撰写**（通常日文或英文），`ja/en` 译文是 `site/src/content/i18n/{ja,en}/…` 的**机器翻译产物**，**不要手写多语正文**。日文术语保留原文（如 `合併`、`適格合併`、`TONA`）是好的，但整篇正文保持单一撰写语言即可。
...
[[financial-regulators/fsa|the FSA]] for the supervisory / regulatory boundary over
...
- [ ] 最终以 `bun tools/release.ts --check --strict` `EXIT=0`（link audit `issues=0` + counts in sync）为准；之后走 [release-process.md](../08-operations/release-process.md)。
