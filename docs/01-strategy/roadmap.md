...
当前开发入口改为 [next-development-plan.md](next-development-plan.md)。旧 P1/P2/P3/P4 已大多完成，后续优先级不再是继续堆内容，而是先补齐 `.txt` 入口审计、docs link gate、active-doc stale scan、generated-surface drift scan 与 i18n freshness report。部署 runbook 已在 2026-06-08 文档漂移审计中扩写，后续只需随工具变化维护。
...
- docs 自身工具化：docs link checker、active-doc stale scan、generated-surface drift scan、i18n freshness report。CI / Vercel runbook 已有当前基线，后续随实现维护。
