...
| 🟢 | per-entry API `canonical_anchor` | **已落地并在 Issue #281 确认保留。** `tools/generate_ai_discovery.ts` 把 `frontmatter.canonical_anchor` 写成 string 或 `null`；当前约 12 个 mirror 非空。规范见 [AI Discovery Surface](../05-functional-specs/ai-discovery-surface.md)。 |
