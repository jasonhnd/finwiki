...
`generate_ai_discovery.ts` 从 **fs mtime** 生成 `sitemap.xml`/`api` 的 `lastmod`。`git clone` 重置所有 mtime → 直接 `--write` 会把全站 lastmod 污染成 clone 日期。**clone 后第一次发布前**：从已提交的 `sitemap.xml` 的 `<lastmod>` 把各页 mtime 恢复回去，只给当天真正编辑过的文件设今天。详见 [gotchas.md](gotchas.md)。
