...
JapanFG 是旗舰领域：≈620 个具体金融机构 entity 页 + **58,061 条 `[[JapanFG/…]]` 入链**（corpus-wide）+ ja/zh/en 三语镜像。当前用 9 个 tag 驱动 sub-INDEX 做导航（[ADR-004](decisions.md#adr-004japanfg-用-sub-index-细分而非移文件)：**刻意不移文件**，因为移动会改 URL + 重写数万入链，代价极大且不可逆）。
...
把 9 个 sub-INDEX 升为顶级领域，其中 3 个大/混类按 frontmatter tag 再拆（tag 可派生，故映射基本机械化）：

| 新领域（role-plural） | 来源 sub-INDEX | ≈数 | 拆分依据 tag |
|---|---|--:|---|
| `megabanks` | megabanks-and-fg | 22 | — |
| `regional-banks` | regional-banks | 137 | — |
| `cooperative-banks` | cooperative-finance | 48 | — |
| `trust-banks` | trust | 50 | — |
| `life-insurers` | insurance ✂ | ~51 | `life-insurance` |
| `non-life-insurers` | insurance ✂ | ~53 | `non-life-insurance` |
| `securities-firms` | securities-and-asset-management ✂ | ~43 | `securities` / `fiea-operator` |
| `asset-managers` | securities-and-asset-management ✂ | ~20 | `asset-management` |
| `payment-firms` | payments-cards-leasing-finance ✂ | ~47 | `payments` / `funds-transfer` |
| `card-issuers` | payments-cards-leasing-finance ✂ | ~23 | `credit-card` |
| `leasing-firms` | payments-cards-leasing-finance ✂ | ~24 | `leasing` |
| `foreign-financial-institutions` | foreign-institutions | 60 | — |
| `financial-regulators` | regulators-sro-policy | 53 | — |
| `financial-licenses` | legal-financial-licenses/（主题页，非 entity） | 7 | — |

**领域数：23 → 36**（−1 JapanFG，+13 entity 领域 +1 licenses）。每个新领域有自己的 `INDEX.md`（旧 sub-INDEX 转为该领域 landing page）。

命名与现有 23 领域零冲突（`banking`≠`megabanks`/`regional-banks`；`securities`≠`securities-firms`；`insurance`≠`life-insurers`；`payments`≠`payment-firms`）。
...
| **未分类 / sogo-shosha 母公司**（~9–19 非金融控股 entity） | 默认进 best-fit 领域；纯非金融的 sogo-shosha 母公司（三菱商事等）→ 评估移入现有 `corporate-strategy`/`business`，或单列。**在 ambiguous.tsv 里逐一裁定**，不默认建新域。 |
| **JapanFG/ 完全解散** | 所有 entity 迁出，旧 `JapanFG/*` URL 404（与 v9 governance→non-profit 改名同样接受 404、非 301）。`JapanFG/INDEX` 消失。 |
...
4. 生成 14 个新 `INDEX.md`（landing page）。
5. 同步站点配置 3 处（`site/src/content.config.ts` 的 `ENTRY_DOMAIN_DIRS`+`DOMAIN_TITLES`、`site/src/lib/siteIndex.mjs`、`site/src/i18n/groups.ts` super-group）。
6. 同步 `tools/wiki_link_audit.ts` 的 domain route map。
7. 同步 root `INDEX.md` 的 domain 表（删 JapanFG 行、加 14 行 + count）。
...
- corpus 内 `[[JapanFG/` 引用 = **0**（grep 校验）。
- `sitemap.xml` 内旧 `JapanFG/` URL = 0；新 14 领域 URL 齐全。
- domain 数 = 36；i18n 镜像 ×3 全部迁移（ja/zh/en 各域文件数一致）。
...
1. **盘点 + 映射**：脚本生成 `mapping.tsv` + `ambiguous.tsv`；人工裁定 ambiguous；锁定 14 领域 × entity 清单。
...
5. **文档**：ADR-008（推翻 ADR-004 的理由 + 36 领域）、roadmap/backlog/domains 更新、三语 README/CHANGELOG/release notes。
